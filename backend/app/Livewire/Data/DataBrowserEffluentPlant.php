...
/**
 * DataBrowserEffluentPlant — screen-095--data-browser-effluent-plant-web /
 * "Data Browser Effluent Plant" (Livewire web page, route name
 * `data.effluent-plant`, /data/effluent-plant).
 *
 * Reuses EffluentPlantRecordService — the exact same service the API
 * controller (App\Http\Controllers\Api\EffluentPlantRecordController) uses
 ...
 * business_unit_id.
 *
 * Pagination: calls EffluentPlantRecordService::listRecords() directly and
 ...
 * /api/effluent-plant-records/export?...), NOT an AJAX/blob download —
 * session cookie carries auth on a same-origin navigation.
 */
#[Layout('data.effluent-plant')]
class DataBrowserEffluentPlant extends Component
{
    ...
    protected function exportUrl(string $format): string
    {
        ...
        return url('/api/effluent-plant-records/export').'?'.http_build_query($query);
    }
    ...
    public function render()
    {
        ...
        return view('livewire.data.data-browser-effluent-plant', [
            'records' => $result['data'],
            'meta' => $result['meta'],
