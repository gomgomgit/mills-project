...
/**
 * KernelDispatchRecordService — screen-093--data-browser-kernel-dispatch-web
 ...
 * KernelDispatchRecordController) and the Livewire components (App\
 * Livewire\Data\{DataBrowserKernelDispatch,DetailKernelDispatch,
 * FormKernelDispatch}), mirroring SolidWasteDisposalRecordService's
...
class KernelDispatchRecordService
{
    ...
    /**
     * create() — resolve the active Kernel Dispatch station from
    ...
    public function create(array $data, User $actor): array
    {
        ...
        $record = DB::transaction(function () use ($attributes, $details) {
            $record = KernelDispatchRecord::create($attributes);
            $this->upsertDetails($record, $details);

            return $record;
        });

        $record->load(['station', 'createdBy', 'checkedBy', 'acknowledgedBy', 'kernelDispatchDetails']);
    ...
    protected function validateForm(array $attributes): void
    {
        Validator::make($attributes, [
            'kernel_dispatch_id' => ['required', 'string'],
            'date' => ['required', 'date'],
            'note' => ['nullable', 'string'],
        ], [
            'kernel_dispatch_id.required' => 'Kernel Dispatch ID wajib diisi.',
     ...
     * longer present.
     */
    protected function upsertDetails(KernelDispatchRecord $record, array $details): void
    {
        $validRows = collect($details)->filter(fn ($row) => filled($row['event_date'] ?? null));
        ...
        foreach ($validRows as $row) {
            ...
            $detailAttributes = [
                'kernel_dispatch_record_id' => $record->id,
                ...
                'remarks_gate_status' => filled($row['remarks_gate_status'] ?? null) ? $row['remarks_gate_status'] : null,
            ...
            if (! empty($row['id']) && KernelDispatchDetail::where('id', $row['id'])->where('kernel_dispatch_record_id', $record->id)->exists()) {
                ...
                $detail = KernelDispatchDetail::create($detailAttributes);
                $keptIds[] = $detail->id;
            }
        }

        KernelDispatchDetail::where('kernel_dispatch_record_id', $record->id)
     ...
     * SolidWasteDisposalRecordService.
     */
    ...
    public function listRecords(array $filters, int $page, int $perPage): array
    {
        ...
        $formatted['data'] = collect($formatted['data'])
            ->map(fn (KernelDispatchRecord $record) => $this->toListRow($record))
            ->all();
    ...
    protected function fileMetaFor(string $format): array
    {
        ...
        if ($format === 'excel') {
            return [
                ...
                "kernel-dispatch-records_{$timestamp}.xlsx",
            ];
        }

        return [
            'text/csv',
            "kernel-dispatch-records_{$timestamp}.csv",
        ];
    }
    ...
    protected function toListRow(KernelDispatchRecord $record): array
    {
        return [
            'id' => $record->id,
            'kernel_dispatch_id' => $record->kernel_dispatch_id,
            'date' => optional($record->date)->toDateString(),
            'event_count' => (int) $record->kernel_dispatch_details_count,
    ...
    protected function toDetailRow(KernelDispatchRecord $record): array
    {
        return [
            'id' => $record->id,
