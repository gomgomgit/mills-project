/**
 * boilerRoomRecordRepo.spec.ts — screen-068--monitor-boiler-room /
 * screen-078--form-boiler-room / screen-088--data-preview-boiler-room.
 *
 * Unit tests for src/services/boilerRoomRecordRepo.ts — covers the
 * authoritative unit_test_cases for these 3 screens' repo layer
 * (mobile-only / local-SQLite-only, no REST api_contract). Mirrors
 * engineRoomRecordRepo.spec.ts's structure/pattern exactly, since Boiler
 * Room follows the same hourly-grid (dynamic add-row/remove-row, 1..24
 * rows) pattern as Engine Room — except this station has 15 non-time_slot
 * columns and 2 enum status columns (blowdown_executed,
 * sootblowing_executed), and 3 free-text columns (fuel_feed_rate,
 * id_fan_load, sa_fan_load).
 *
...
function makeDetailRow(overrides: Partial<BoilerRoomDetailRow> = {}): BoilerRoomDetailRow {
  return {
    id: 'detail-1',
    boiler_room_record_id: 'draft-1',
    time_slot: '07:00',
...
function makeHeaderData(overrides: Partial<BoilerRoomHeaderFormData> = {}): BoilerRoomHeaderFormData {
  return {
    boiler_room_id: 'BR-01',
...
function makeDetailFormRow(overrides: Partial<BoilerRoomDetailFormRow> = {}): BoilerRoomDetailFormRow {
  return {
    time_slot: '07:00',
    steam_pressure_bar: null,
    steam_temp_c: null,
...
function makeValidDetailRows(overrides: Partial<BoilerRoomDetailFormRow>[] = []): BoilerRoomDetailFormRow[] {
  if (overrides.length === 0) {
    return [makeDetailFormRow({ time_slot: '07:00', steam_pressure_bar: 12.5 })]
  }

  return overrides.map((o) => makeDetailFormRow(o))
}

// Positional order of the 15 non-time_slot detail columns as written by
// applyDetailRowChanges() — used to build expected INSERT/UPDATE param
// arrays without repeating all 15 `null`s by hand in every test.
function readingParams(overrides: Partial<BoilerRoomDetailFormRow> = {}): (string | number | null)[] {
...
describe('boilerRoomRecordRepo', () => {
  ...
  describe('createDraft()', () => {
    it('inserts a new draft_ongoing boiler_room_record with date auto-filled, and inserts NO boiler_room_detail rows', async () => {
  ...
  describe('getDrafts()', () => {
    it('returns the draft/pause list, most-recently-updated first', async () => {
      const rows = [
        { id: 'draft-newest', status: 'draft_paused' as const, boiler_room_id: 'BR-002', updated_at: '2026-08-31T10:00:00.000Z' },
        { id: 'draft-oldest', status: 'draft_ongoing' as const, boiler_room_id: 'BR-001', updated_at: '2026-08-31T08:00:00.000Z' },
      ]
  ...
  describe('getTodaySummary()', () => {
    it("computes today's record count and a plain COUNT of boiler_room_detail rows belonging to those records (not filtered by filled-ness)", async () => {
      vi.mocked(query)
      ...
      expect(result).toEqual({ countBoilerRoomRecord: 2, detailRowCount: 5 })
      expect(query).toHaveBeenNthCalledWith(1, expect.stringContaining('FROM boiler_room_record'), [USER_ID])
      ...
      expect(countSql).toContain('FROM boiler_room_detail')
      expect(countSql).toContain('WHERE boiler_room_record_id IN (?, ?)')
    ...
    it('returns zero counters and skips the detail query when no records match today', async () => {
      ...
      expect(result).toEqual({ countBoilerRoomRecord: 0, detailRowCount: 0 })
  ...
  describe('getAllRecords()', () => {
    it('returns every record for the user regardless of status', async () => {
      ...
      expect(result).toEqual(rows)
      expect(query).toHaveBeenCalledWith(expect.stringContaining('FROM boiler_room_record WHERE created_by = ?'), [
        USER_ID,
      ])
      expect(query).toHaveBeenCalledWith(expect.stringContaining('ORDER BY updated_at DESC'), [USER_ID])
    })
  ...
  describe('saveDraft()', () => {
    it('throws BoilerRoomDetailRequiredError and calls neither run() nor query() when details is empty', async () => {
    ...
    it('throws BoilerRoomDetailRequiredError when every row lacks a selected time_slot or has zero reading columns filled', async () => {
      const headerData = makeHeaderData()
      const details: BoilerRoomDetailFormRow[] = [
    ...
    it('treats a row with only an enum status column filled as satisfying the minimum-one-row rule', async () => {
      ...
      const headerData = makeHeaderData()
      const details = [makeDetailFormRow({ time_slot: '07:00', blowdown_executed: 'y' })]

      await expect(saveDraft('draft-1', headerData, details, [], 'operator')).resolves.toBeTruthy()
    })

    it('treats a row with only a free-text column filled (Fuel Feed Rate) as satisfying the minimum-one-row rule', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      vi.mocked(query)
        .mockResolvedValueOnce([makeRecord({ status: 'saved' })])
        .mockResolvedValueOnce([makeDetailRow()])
      ...
      const headerData = makeHeaderData()
      const details = [makeDetailFormRow({ time_slot: '07:00', fuel_feed_rate: '45 Hz' })]
    ...
    it('updates the header with status=saved, INSERTs a new (id-less) detail row, and returns the freshly saved draft', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      const saved = makeRecord({ id: 'draft-4', boiler_room_id: 'BR-01', status: 'saved' })
      const savedDetails = [makeDetailRow({ id: 'detail-new-1', boiler_room_record_id: 'draft-4' })]
      vi.mocked(query).mockResolvedValueOnce([saved]).mockResolvedValueOnce(savedDetails)

      const headerData = makeHeaderData()
      const details = makeValidDetailRows([{ time_slot: '09:00', steam_pressure_bar: 13 }])
      ...
      const [headerSql, headerParams] = vi.mocked(run).mock.calls[0]
      expect(headerSql).toContain('UPDATE boiler_room_record')
      expect(headerSql).toContain("status = 'saved'")
      expect(headerParams).toEqual(
        expect.arrayContaining(['BR-01', expect.any(String), null, null, expect.any(String), 'draft-4']),
      )

      const [detailSql, detailParams] = vi.mocked(run).mock.calls[1]
      expect(detailSql).toContain('INSERT INTO boiler_room_detail')
      expect(detailParams).toEqual([
        expect.any(String), // freshly generated detail id
        'draft-4',
        '09:00',
        ...readingParams({ time_slot: '09:00', steam_pressure_bar: 13 }),
        expect.any(String),
        expect.any(String),
      ])
    ...
    it('UPDATEs an existing detail row (has an id) instead of inserting it', async () => {
      ...
      const headerData = makeHeaderData()
      const details: BoilerRoomDetailFormRow[] = [
        { ...makeDetailFormRow(), id: 'detail-existing-1', time_slot: '11:00', steam_pressure_bar: 14 },
      ]
      ...
      expect(run).toHaveBeenCalledTimes(2)
      const [detailSql, detailParams] = vi.mocked(run).mock.calls[1]
      expect(detailSql).toContain('UPDATE boiler_room_detail')
      expect(detailParams).toEqual([
        '11:00',
        ...readingParams({ time_slot: '11:00', steam_pressure_bar: 14 }),
        expect.any(String),
        'detail-existing-1',
      ])
    })
    ...
    it('upserts multiple detail rows sequentially, in details order (mixed insert + update)', async () => {
      ...
      const headerData = makeHeaderData()
      const details: BoilerRoomDetailFormRow[] = [
        { ...makeDetailFormRow(), id: 'detail-existing-2', time_slot: '07:00', steam_pressure_bar: 1 },
        makeDetailFormRow({ time_slot: '13:00', steam_pressure_bar: 2 }),
      ]
      ...
      expect(firstRowSql).toContain('UPDATE boiler_room_detail')
      expect(secondRowSql).toContain('INSERT INTO boiler_room_detail')
    })
    ...
    it('DELETEs every id in detailIdsToDelete, after the upserts', async () => {
      ...
      expect(deleteSql1).toContain('DELETE FROM boiler_room_detail WHERE id = ?')
      expect(deleteParams1).toEqual(['detail-removed-1'])
      expect(deleteSql2).toContain('DELETE FROM boiler_room_detail WHERE id = ?')
    ...
    it('coerces empty-string enum values to null when upserting a detail row (both enum columns)', async () => {
      ...
      // positions: id, recordId, time_slot, then the 15 reading params —
      // blowdown_executed is index 12 (0-based) of the 15 reading
      // params, i.e. index 15 of the full array; sootblowing_executed is
      // index 13, i.e. full-array index 16.
      expect(detailParams[15]).toBeNull()
    ...
    it('nulls/strips checked_by for a non-supervisor role', async () => {
      ...
      // checked_by is the 4th positional SET param (index 3): boiler_room_id,
    ...
    it('throws when getDraftWithDetails() re-read after save unexpectedly finds nothing', async () => {
      ...
      await expect(saveDraft('draft-8', headerData, details, [], 'operator')).rejects.toThrow(
        'Gagal memuat ulang data boiler room setelah disimpan.',
      )
    })
  })
  ...
  describe('pauseDraftWithFormData()', () => {
    it('saves with status=draft_paused even when details is empty (no validation gate)', async () => {
      ...
      const headerData = makeHeaderData({ boiler_room_id: '' })
    ...
    it('upserts detail rows and applies pending deletions, same contract as saveDraft()', async () => {
      ...
      expect(deleteSql).toContain('DELETE FROM boiler_room_detail WHERE id = ?')
    ...
    it('throws when getDraftWithDetails() re-read after pause unexpectedly finds nothing', async () => {
      ...
      await expect(pauseDraftWithFormData('draft-1', headerData, [], [], 'operator')).rejects.toThrow(
        'Gagal menyimpan progres boiler room setelah pause.',
      )
    })
  })

  describe('deleteDraft()', () => {
    it('deletes boiler_room_detail rows before deleting the boiler_room_record row', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      ...
      expect(run).toHaveBeenNthCalledWith(
        1,
        expect.stringContaining('DELETE FROM boiler_room_detail WHERE boiler_room_record_id = ?'),
        ['draft-1'],
      )
      expect(run).toHaveBeenNthCalledWith(2, expect.stringContaining('DELETE FROM boiler_room_record WHERE id = ?'), [
        'draft-1',
      ])
    })
  })
