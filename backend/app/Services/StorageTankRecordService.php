 ...
 * StorageTankRecordController) and the Livewire components (App\Livewire\
 * Data\{DataBrowserStorageTank,DetailStorageTank,FormStorageTank}), mirroring
 * EffluentPlantRecordService's structure exactly — Storage Tank follows the
 * same "hourly grid" pattern as Effluent Plant/Process Water/Threshing/
 * Pressing/Depricarping/Kernel Plant: `details` may contain any number of
 * rows (1..24), each with a unique `time_slot` in strictly ascending
 * canonical order (validateDetails()); create() INSERTs exactly the given
 * rows; update() upserts (rows with an existing `id` kept in `$details` are
 * UPDATEd, rows without one are INSERTed, and any existing row whose id is
 * no longer present in `$details` is DELETEd).
 *
 * UNLIKE Threshing/Pressing/Depricarping/Kernel Plant: Storage Tank has NO
 ...
 * storage_tank_detail row counts as filled when at least one of its 17
 * non-time_slot columns is non-null — like Effluent Plant, this station has
 * NO identifying/context columns (no shift/inspector_id equivalent), so all
 * 17 remaining columns are reading/status/text columns and all participate
 * in the "filled" check.
 *
 * IMPORTANT — enum empty-string-vs-null: 1 of the 17 columns
 * (steam_heating_valve_status) is backed by a SQLite CHECK constraint
 * (Laravel enum()) that rejects an empty string. normalizeDetails() coerces
 * '' to null for this column specifically before validation/persistence —
 * the same mitigation Kernel Dispatch/Effluent Plant needed for their own
 * enum columns.
 */
class StorageTankRecordService
{
    ...
    protected const FORM_FIELDS = ['storage_tank_id', 'date', 'note'];
     ...
     * storageTankRecordRepo.ts's canonicalTimeSlots() exactly (`for i in
     ...
     * storage_tank_detail rows inside a DB transaction.
     */
    public function create(array $data, User $actor): array
    {
        ...
        $record = DB::transaction(function () use ($attributes, $details) {
            // StorageTankRecord may carry the same "minimal satu detail row
        ...
        $record->load(['station', 'createdBy', 'checkedBy', 'acknowledgedBy', 'storageTankDetails']);
     ...
     * then upsert its storage_tank_detail rows (insert rows without an id,
     ...
     * EffluentPlantRecordService::update() exactly.
     */
    public function update(string $id, array $data, User $actor): array
    {
        $record = StorageTankRecord::findOrFail($id);
        ...
        return collect($rawDetails)
            ->map(function ($row) {
                ...
                foreach (self::DETAIL_FIELDS as $field) {
                    ...
                    // Empty-string-to-null coercion for the enum column —
                    // SQLite's CHECK constraint (Laravel enum()) rejects '',
                    // so an empty dropdown selection must become null, not
                    // ''. Same fix Kernel Dispatch/Effluent Plant needed for
                    // their own enum columns.
    ...
    protected function validateForm(array $attributes): void
    {
        Validator::make($attributes, [
            'storage_tank_id' => ['required', 'string'],
            'date' => ['required', 'date'],
            'note' => ['nullable', 'string'],
        ], [
            'storage_tank_id.required' => 'Storage Tank ID wajib diisi.',
     ...
     * EffluentPlantRecordService::validateDetails() exactly.
     */
    protected function validateDetails(array $details): void
    {
        ...
        if ($validRows->isEmpty()) {
            throw ValidationException::withMessages([
                'details' => 'Minimal satu baris Storage Tank Detail (Time-Slot terpilih + minimal 1 kolom bacaan terisi) harus diisi.',
            ]);
        }
        ...
        if ($slots->contains(fn ($slot) => ! array_key_exists($slot, $canonicalOrder))) {
            throw ValidationException::withMessages([
                'details' => 'Time-Slot Storage Tank Detail harus salah satu dari 24 slot kanonis (07:00-06:00).',
            ]);
        }

        if ($slots->duplicates()->isNotEmpty()) {
            throw ValidationException::withMessages([
                'details' => 'Time-Slot tiap baris Storage Tank Detail tidak boleh duplikat.',
            ]);
        }
        ...
        foreach ($sortedIndexes as $index => $slotIndex) {
            if ($index > 0 && $slotIndex <= $sortedIndexes[$index - 1]) {
                throw ValidationException::withMessages([
                    'details' => 'Time-Slot tiap baris Storage Tank Detail harus lebih besar dari baris sebelumnya (urutan menaik).',
                ]);
            }
        }
    }
     ...
     * no longer present — mirrors EffluentPlantRecordService::upsertDetails()
     * exactly.
     */
    protected function upsertDetails(StorageTankRecord $record, array $details): void
    {
        $validRows = collect($details)->filter(
            fn ($row) => $row['time_slot'] !== null && $row['time_slot'] !== '' && $this->isRowFilled($row)
        );
     ...
     * EffluentPlantRecordService::applyVerification() exactly.
     */
    ...
    public function listRecords(array $filters, int $page, int $perPage): array
    {
        ...
        $query = $this->buildFilteredQuery($filters)
            ->withCount(['storageTankDetails as filled_slot_count' => function (Builder $detailQuery) use ($readingFields) {
        ...
        $formatted['data'] = collect($formatted['data'])
            ->map(fn (StorageTankRecord $record) => $this->toListRow($record))
            ->all();
    ...
    protected function fileMetaFor(string $format): array
    {
        ...
        if ($format === 'excel') {
            return [
                ...
                "storage-tank-records_{$timestamp}.xlsx",
            ];
        }

        return [
            'text/csv',
            "storage-tank-records_{$timestamp}.csv",
        ];
    }
    ...
    /**
     * Maps a StorageTankRecord (with filled_slot_count pre-loaded via
     * withCount()) to the list endpoint's success_schema row shape.
     */
    protected function toListRow(StorageTankRecord $record): array
    {
        return [
            'id' => $record->id,
     ...
     * and the storageTankDetails grid, sorted into canonical time-slot
     * order (not alphabetical — '00:00' would otherwise sort before
     * '07:00').
     */
    public function getDetail(string $id): array
    {
        $record = StorageTankRecord::with([
            'station',
            'createdBy',
            'checkedBy',
            'acknowledgedBy',
    ...
    /**
     * Maps a StorageTankRecord to the detail endpoint's success_schema —
     ...
     */
    protected function toDetailRow(StorageTankRecord $record): array
    {
        $canonicalOrder = array_flip(self::canonicalTimeSlots());

        $details = $record->storageTankDetails
            ->sortBy(fn (StorageTankDetail $row) => $canonicalOrder[$row->time_slot] ?? 999)
            ->values()
            ->map(function (StorageTankDetail $row) {
