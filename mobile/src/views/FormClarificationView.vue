...
/**
 * FormClarificationView — screen-079--form-clarification /
 * usecase-110--form-clarification (mounted at
 * /stations/clarification/form/:id, meta.public = false — requires an
 ...
 * local (offline) `clarification_record` + `clarification_detail` tables
 * via clarificationRecordRepo.ts.
 *
 ...
 * FormBoilerRoomView.vue exactly, including its dynamic add-row/
 * remove-row detail grid pattern:
 *
 *  - "Identitas Clarification" section: `clarification_id` (required,
 *    freely editable), `date` (auto-set once, in memory, ONLY for a
 *    brand-new draft — clarificationRecordRepo.ts's createDraft() already
 ...
 *    SearchableSelect dropdown; the 6 numeric reading fields
 *    (Clarification Tank Temp, Oil Tank Temperature, Sludge Tank Temp,
 *    Buffer Tank Level, Pure Oil Production Rate, Downtime) and 1 free-text
 *    field (Findings) stay freely editable at all times — 7 non-time_slot
 *    columns total. "Tambah baris" is disabled once all 24 canonical slots
 *    are already used (`detailRows.length >= 24`) or while an action is in
 *    progress.
 *
 *  - This is the SIMPLEST station in the whole project — NO
 *    operational-target reference table, NO enum `<select>` dropdowns
 *    anywhere on this screen.
 *
 *  - 'Simpan': validates clarification_id (required) — inline error — AND
 *    at least one valid Clarification Detail row (a selected Time-Slot AND
 ...
 *    the repo level (ClarificationDetailRequiredError) — mirrors
 *    FormBoilerRoomView.vue's grid validation exactly. On success: calls
 *    clarificationRecordRepo.saveDraft() (status='saved'), navigates to
 *    `monitor-clarification`.
 *  - 'Pause': no validation at all, persists current field values as-is via
 *    clarificationRecordRepo.pauseDraftWithFormData()
 *    (status='draft_paused'), navigates to `monitor-clarification`.
 *  - 'Clear': ConfirmDialog.vue first; on confirm, calls
 *    clarificationRecordRepo.deleteDraft() (cascades delete to every
 *    clarification_detail row for the record, however many exist), then
 *    navigates to `monitor-clarification`; on cancel, no change.
 ...
 *
 * Header/breadcrumb/nav-menu: copied verbatim from FormBoilerRoomView.vue.
 * Breadcrumb is 4 segments deep (Home > Production Process Activity >
 * Clarification > Form), the first 3 tappable.
 */
...
import clarificationRecordRepo, {
  ClarificationDetailRequiredError,
  type ClarificationDetailFormRow,
  type ClarificationDraftWithDetails,
...
const REQUIRED_FIELDS: (keyof ClarificationHeaderFormData)[] = ['clarification_id']
...
// reasoning as FormBoilerRoomView.vue excluding date).
const DIRTY_CHECK_FIELDS: (keyof ClarificationHeaderFormData)[] = [
  'clarification_id',
  'note',
  'checked_by',
  'acknowledged_by',
...
function emptyFormData(): ClarificationHeaderFormData {
  return {
    clarification_id: '',
    date: '',
    note: '',
...
function emptyDetailRow(): ClarificationDetailFormRow {
  return {
    time_slot: null,
...
const form = reactive<ClarificationHeaderFormData>(emptyFormData())
const errors = reactive<Partial<Record<keyof ClarificationHeaderFormData, string>>>({})
const detailRows = ref<ClarificationDetailFormRow[]>([])
...
// next Simpan/Pause call (mirrors FormBoilerRoomView.vue's
...
// surfaced on the Clarification Detail section rather than mixed into the
...
// FormBoilerRoomView.vue's `isDirty`.
let loadedSnapshot = dirtySnapshot()

function dirtySnapshot(): string {
  const header: Partial<Record<keyof ClarificationHeaderFormData, string>> = {}
 ...
 * device's local time. Mirrors FormBoilerRoomView.vue's
...
// untouched.
function populateForm(record: ClarificationRecord): void {
  form.clarification_id = record.clarification_id ?? ''
...
function populateDetailRows(rows: ClarificationDraftWithDetails['details']): void {
  detailRows.value = rows.map((row) => ({
    id: row.id,
    time_slot: row.time_slot ?? null,
    clarification_tank_temp_c: row.clarification_tank_temp_c ?? null,
    oil_tank_temperature_c: row.oil_tank_temperature_c ?? null,
    sludge_tank_temp_c: row.sludge_tank_temp_c ?? null,
    buffer_tank_level_percent: row.buffer_tank_level_percent ?? null,
    pure_oil_production_rate_ton_hour: row.pure_oil_production_rate_ton_hour ?? null,
    downtime_mins: row.downtime_mins ?? null,
...
async function loadDraft(): Promise<void> {
  ...
  try {
    const draft = await clarificationRecordRepo.getDraftWithDetails(recordId)
    ...
    loadErrorMessage.value = err instanceof Error ? err.message : 'Gagal memuat draft clarification.'
  } finally {
    loading.value = false
  }
}
 ...
 * mirrors FormBoilerRoomView.vue's `availableTimeSlotOptions()` exactly.
...
function availableTimeSlotOptions(index: number): string[] {
  ...
  const order = clarificationRecordRepo.canonicalTimeSlots()
...
// FormBoilerRoomView.vue's timeSlotSelectOptions() uses for its own
// per-row dropdown.
...
// — mirrors FormBoilerRoomView.vue's removeDetailRow() exactly. Either
...
function validateHeader(): boolean {
  for (const key of Object.keys(errors) as (keyof ClarificationHeaderFormData)[]) {
    delete errors[key]
  }
...
function isRowFilled(row: ClarificationDetailFormRow): boolean {
  return (
...
// grid, not a per-field header error — mirrors FormBoilerRoomView.vue's
// validateDetailRows() exactly.
function isRowValid(row: ClarificationDetailFormRow): boolean {
...
async function onSimpan(): Promise<void> {
  ...
  try {
    await clarificationRecordRepo.saveDraft(
      recordId,
      { ...form },
      [...detailRows.value],
      [...pendingDeletionIds.value],
    ...
    await syncAfterSave('clarification_record', recordId)
    router.push({ name: 'monitor-clarification' })
  } catch (err) {
    if (err instanceof ClarificationDetailRequiredError) {
    ...
    actionErrorMessage.value = err instanceof Error ? err.message : 'Gagal menyimpan data clarification.'
  } finally {
    saving.value = false
  }
}
...
async function onPause(): Promise<void> {
  ...
  try {
    await clarificationRecordRepo.pauseDraftWithFormData(
      recordId,
      { ...form },
      [...detailRows.value],
      [...pendingDeletionIds.value],
...
async function onClearConfirm(): Promise<void> {
  ...
  try {
    await clarificationRecordRepo.deleteDraft(recordId)
    router.push({ name: 'monitor-clarification' })
  } catch (err) {
    actionErrorMessage.value = err instanceof Error ? err.message : 'Gagal menghapus draft clarification.'
  } finally {
...
function goToMonitorClarification(): void {
  router.push({ name: 'monitor-clarification' })
}
</script>

<template>
  <main class="form-clarification-view">
    ...
    <div class="form-clarification-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <button type="button" class="breadcrumb-link" data-testid="breadcrumb-clarification" @click="goToMonitorClarification">
          Clarification
        </button>
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <span class="breadcrumb-current" aria-current="page">Form</span>
      </nav>
      <h1 class="screen-title">Form Clarification</h1>
    </div>

    <p v-if="loading" class="status-text">Memuat draft clarification…</p>
    <p v-else-if="notFound" class="status-text status-text--error" role="alert">
      Draft clarification tidak ditemukan.
    </p>
    ...
    <form v-else class="form-body" novalidate @submit.prevent="onSimpan">
      ...
      <section class="form-section" aria-label="Identitas Clarification">
        <h2 class="form-section-title">Identitas Clarification</h2>

        <FormField
          v-model="form.clarification_id"
          label="Clarification ID"
          required
          :error="errors.clarification_id"
      ...
      <section class="form-section" aria-label="Clarification Detail">
        <h2 class="form-section-title">Clarification Detail</h2>
        ...
        <div
          ...
          class="detail-row"
          data-testid="clarification-detail-row"
        >
          <div class="form-field">
          ...
          <FormField
            v-model="row.clarification_tank_temp_c"
            label="Clarification Tank Temp (°C)"
            type="number"
            :id="`clarification-tank-temp-${index}`"
            :data-testid="`clarification-tank-temp-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.oil_tank_temperature_c"
            label="Oil Tank Temperature (°C)"
            type="number"
            :id="`oil-tank-temperature-${index}`"
            :data-testid="`oil-tank-temperature-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.sludge_tank_temp_c"
            label="Sludge Tank Temp (°C)"
            type="number"
            :id="`sludge-tank-temp-${index}`"
            :data-testid="`sludge-tank-temp-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.buffer_tank_level_percent"
            label="Buffer Tank Level (%)"
            type="number"
            :id="`buffer-tank-level-${index}`"
            :data-testid="`buffer-tank-level-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.pure_oil_production_rate_ton_hour"
            label="Pure Oil Production Rate (Ton/Hour)"
            type="number"
            :id="`pure-oil-production-rate-${index}`"
            :data-testid="`pure-oil-production-rate-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.downtime_mins"
            label="Downtime (Mins)"
            type="number"
            :id="`downtime-mins-${index}`"
            :data-testid="`downtime-mins-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
    ...
    <ConfirmDialog
      ...
      title="Hapus Draft"
      message="Draft clarification ini akan dihapus secara permanen dan tidak dapat dikembalikan. Lanjutkan?"
