...
/**
 * MonitorPressingView — screen-038--monitor-pressing /
 * usecase-038--monitor-pressing (mounted at /stations/pressing/monitor,
 ...
 * local (offline) `pressing_record` table (and its child `pressing_detail`
 * table, for the "filled slot" counter) via pressingRecordRepo.ts.
 * api_contracts.endpoints is empty for this screen.
 *
 * Structural pattern copied verbatim from MonitorThreshingView.vue
 * (screen-037), Pressing's exact structural sibling: a "Hari Ini" 2-card
 ...
 * calls) — those actions live in Form Pressing (screen-042).
 *
 * Counter card labels per this screen's tech spec: "Jumlah Pressing
 * Record" (COUNT of today's pressing_record rows, any status) / "Jumlah
 * Baris Time-Slot Tercatat" (COUNT of pressing_detail rows belonging to
 * those records — REVISED 2026-08-24, entity-catalog v12: since
 * pressing_detail rows are no longer pre-created (see
 * pressingRecordRepo.ts's header comment), row EXISTENCE itself is the
 * meaningful signal now — a plain count of rows the user has added, with
 * no fixed-24 denominator, mirroring MonitorThreshingView.vue's own
 * "count of child rows recorded" counter shape).
 *
 ...
 * (Home > Production Process Activity > Pressing, the first two tappable).
 *
 * Navigation:
 *  - 'New Data' -> pressingRecordRepo.createDraft(userId) (INSERTs a new
 *    status='draft_ongoing' header row ONLY — no pressing_detail rows are
 *    pre-created, see that function's doc comment), then -> route name
 *    `pressing-form` with the new draft's id.
 *  - Tapping a list item -> route name `pressing-form` with that item's
 *    EXISTING id, directly — no status-transition call.
 *  - 'Load Data' -> route name `data-preview-pressing` with NO id param
 *    (list mode).
 *  - 'Back' -> route name `station-list`.
 *
 * known_issues: StationListView.vue (screen-006, out of this task's scope)
 * has not yet been updated to link to `monitor-pressing` — same known
 * issue already documented for Threshing, this screen is reachable once
 * screen-006 is revised in a separate task.
 */
...
import {
  pressingRecordRepo,
  type PressingDraftListItem,
  type PressingTodaySummary,
...
const drafts = ref<PressingDraftListItem[]>([])
...
// today.
const todaySummary = ref<PressingTodaySummary>({ countPressingRecord: 0, detailRowCount: 0 })
...
async function loadDrafts() {
  ...
  try {
    drafts.value = await pressingRecordRepo.getDrafts(userId)
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal memuat daftar draft pressing lokal.'
  } finally {
    loading.value = false
  }
}
...
async function loadTodaySummary() {
  ...
  try {
    todaySummary.value = await pressingRecordRepo.getTodaySummary(userId)
  } catch {
 ...
 * filled in yet.
 */
function draftLabel(draft: PressingDraftListItem): string {
...
async function onNewData() {
  ...
  try {
    const id = await pressingRecordRepo.createDraft(userId)
    router.push({ name: 'pressing-form', params: { id } })
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal membuat draft pressing baru.'
  }
}
...
function onDraftClick(draft: PressingDraftListItem) {
  router.push({ name: 'pressing-form', params: { id: draft.id } })
}
...
// business_logic step 8 — breadcrumb taps. 'Pressing' is the current page
...
<template>
  <main class="monitor-pressing-view">
    ...
    <div class="monitor-pressing-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <span class="breadcrumb-current" aria-current="page">Pressing</span>
      </nav>
      <h1 class="screen-title">Monitor Pressing</h1>
    </div>
    ...
    <section class="today-summary-section" aria-label="Hari Ini">
      ...
      <div class="today-summary-grid">
        <div class="today-summary-card">
          <span class="today-summary-label">Jumlah Pressing Record</span>
          <span class="today-summary-value" data-testid="counter-count-pressing-record">{{ todaySummary.countPressingRecord }}</span>
        </div>
        <div class="today-summary-card">
    ...
    <p v-if="loading" class="status-text">Memuat daftar draft pressing lokal…</p>
    <p v-else-if="error" class="status-text status-text--error" role="alert">{{ error }}</p>

    <section v-else class="draft-list-section" aria-label="Daftar Draft Pressing">
      <p v-if="drafts.length === 0" class="empty-state" data-testid="draft-list-empty">
        Belum ada draft pressing tersimpan.
      </p>
