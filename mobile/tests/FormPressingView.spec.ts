/**
 * FormPressingView.spec.ts — screen-042--form-pressing /
 * usecase-042--form-pressing.
 *
 * REVISED 2026-08-24 (entity-catalog v12): mirrors
 * FormThreshingView.spec.ts's add-row/remove-row interaction helpers and
 * coverage exactly — the Pressing Detail grid is no longer a fixed 24-row
 * grid, it now uses the exact same dynamic add-row/remove-row pattern as
 * Threshing Detail / Cages Tipped Time.
 *
 ...
 *   - Simpan success (status=saved), navigates to Monitor Pressing
 ...
 *     mill_management
 *   - Target Operasional reference table renders 7 read-only rows
 */
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { mount, flushPromises } from '@vue/test-utils'
import FormPressingView from '@/views/FormPressingView.vue'
import { PressingDetailRequiredError, type PressingDetailRow, type PressingRecord } from '@/services/pressingRecordRepo'
...
describe('FormPressingView', () => {
  it('loads existing detail rows (however many) ordered by canonical time-slot on mount', async () => {
    ...
    const wrapper = mount(FormPressingView)
    await flushPromises()

    const rows = wrapper.findAll('[data-testid="pressing-detail-row"]')
  ...
  it('adds one row at a time via "Tambah baris"', async () => {
    ...
    expect(wrapper.findAll('[data-testid="pressing-detail-row"]')).toHaveLength(1)
  ...
  it('shows a grid-specific error and does not save when there are zero valid detail rows', async () => {
    ...
    await wrapper.find('#field-presser-id').setValue('PR-01')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()
  ...
  it('saves successfully and navigates to Monitor Pressing when Presser ID and a valid detail row exist', async () => {
    ...
    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    await chooseSearchableOption(wrapper, 'time-slot-select-0', '07:00')
    await wrapper.find('#digester-temp-0').setValue('92')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()
    ...
    expect(headerData.presser_id).toBe('PR-01')
  ...
  it('falls back to the grid error (defense-in-depth) when the repo throws PressingDetailRequiredError', async () => {
    saveDraftMock.mockRejectedValue(new PressingDetailRequiredError())
    ...
    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    await chooseSearchableOption(wrapper, 'time-slot-select-0', '07:00')
    await wrapper.find('#digester-temp-0').setValue('1')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()
  ...
  it('shows a confirm dialog on Back when the form is dirty', async () => {
    ...
    await wrapper.find('#field-presser-id').setValue('PR-99')
  ...
  it('renders the Target Operasional reference table with 7 read-only rows', async () => {
    const wrapper = mount(FormPressingView)
    await flushPromises()

    const table = wrapper.find('[data-testid="operational-target-table"]')
    expect(table.exists()).toBe(true)
    expect(table.findAll('tbody tr')).toHaveLength(7)
    expect(table.text()).toContain('Digester Temperature')
    expect(table.text()).toContain('90°C - 95°C')
  })
