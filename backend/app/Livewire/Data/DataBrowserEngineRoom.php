...
/**
 * DataBrowserEngineRoom — screen-097--data-browser-engine-room-web /
 * "Data Browser Engine Room" (Livewire web page, route name
 * `data.engine-room`, /data/engine-room).
 *
 * Reuses EngineRoomRecordService — the exact same service the API
 * controller (App\Http\Controllers\Api\EngineRoomRecordController) uses —
 * so filtering/pagination rules stay identical between the web and API
 * entry points (mirrors DataBrowserStorageTank's pattern).
 *
 * Filter state mirrors the tech-spec exactly: date_from, date_to,
 * business_unit_id.
 *
 * Pagination: calls EngineRoomRecordService::listRecords() directly and
 * keeps its own $page/$perPage state, rather than Livewire's
 * WithPagination trait — same approach as DataBrowserStorageTank.
 *
 * Export: dispatched as a plain browser-navigable link (GET
 * /api/engine-room-records/export?...), NOT an AJAX/blob download —
 * session cookie carries auth on a same-origin navigation.
 */
#[Layout('data.engine-room')]
class DataBrowserEngineRoom extends Component
{
    ...
    protected function exportUrl(string $format): string
    {
        ...
        return url('/api/engine-room-records/export').'?'.http_build_query($query);
    }
    ...
    public function render()
    {
        ...
        return view('livewire.data.data-browser-engine-room', [
            'records' => $result['data'],
            'meta' => $result['meta'],
