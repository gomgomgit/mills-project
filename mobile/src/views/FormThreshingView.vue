...
/**
 * FormThreshingView — screen-041--form-threshing /
 * usecase-041--form-threshing (mounted at /stations/threshing/form/:id,
 ...
 * local (offline) `threshing_record` + `threshing_detail` tables via
 * threshingRecordRepo.ts.
 *
 ...
 * FormCagesTrackView.vue as closely as sensible, adapted for Threshing's
 * very different detail grid:
 *
 *  - "Identitas Threshing" section: `thresher_id` (required, freely
 *    editable), `date` (auto-set once, in memory, ONLY for a brand-new
 *    draft — threshingRecordRepo.ts's createDraft() already auto-fills it
 ...
 *    level (threshingRecordRepo.ts's saveDraft()/pauseDraftWithFormData()
 *    role-stripping) as defense-in-depth. `note` (optional free text) also
 *    lives in this section.
 *
 *  - "Threshing Detail" section — the dynamic grid. REVISED 2026-08-24
 ...
 *    EXACT pattern FormCagesTrackView.vue's Cages Tipped Time grid already
 *    uses: `detailRows` (in-memory, rows without a saved `id` are new),
 ...
 *    logic as FormCagesTrackView.vue's `availableHourOptions()`, but over
 *    `canonicalTimeSlots()` array indices instead of raw hour numbers,
 ...
 *
 *  - "Target Operasional" section — read-only reference table (6 rows,
 *    from the static THRESHING_OPERATIONAL_TARGETS constant — see that
 ...
 *
 *  - 'Simpan': validates thresher_id (required) — inline error — AND at
 *    least one valid Threshing Detail row (a selected Time-Slot AND at
 ...
 *    repo level (ThreshingDetailRequiredError) — mirrors
 *    FormCagesTrackView.vue's grid validation exactly. On success: calls
 *    threshingRecordRepo.saveDraft() (status='saved'), navigates to
 *    `monitor-threshing`.
 *  - 'Pause': no validation at all, persists current field values as-is via
 *    threshingRecordRepo.pauseDraftWithFormData() (status='draft_paused'),
 *    navigates to `monitor-threshing`.
 *  - 'Clear': ConfirmDialog.vue first; on confirm, calls
 *    threshingRecordRepo.deleteDraft() (cascades delete to every
 *    threshing_detail row for the record, however many exist), then
 *    navigates to `monitor-threshing`; on cancel, no change.
 ...
 *
 * Header/breadcrumb/nav-menu: copied verbatim from FormCagesTrackView.vue.
 * Breadcrumb is 4 segments deep (Home > Production Process Activity >
 * Threshing > Form), the first 3 tappable.
 */
...
import threshingRecordRepo, {
  ThreshingDetailRequiredError,
  type ThreshingDetailFormRow,
  type ThreshingDraftWithDetails,
  type ThreshingHeaderFormData,
  type ThreshingRecord,
} from '@/services/threshingRecordRepo'
...
const REQUIRED_FIELDS: (keyof ThreshingHeaderFormData)[] = ['thresher_id']

const REQUIRED_FIELD_LABELS: Record<string, string> = {
  thresher_id: 'Thresher ID',
}
...
// reasoning as FormCagesTrackView.vue excluding date/tippler_start_time).
const DIRTY_CHECK_FIELDS: (keyof ThreshingHeaderFormData)[] = ['thresher_id', 'note', 'checked_by', 'acknowledged_by']

function emptyFormData(): ThreshingHeaderFormData {
  return {
    thresher_id: '',
    date: '',
    note: '',
...
const form = reactive<ThreshingHeaderFormData>(emptyFormData())
const errors = reactive<Partial<Record<keyof ThreshingHeaderFormData, string>>>({})
const detailRows = ref<ThreshingDetailFormRow[]>([])
...
// next Simpan/Pause call (mirrors FormCagesTrackView.vue's
...
// surfaced on the Threshing Detail section rather than mixed into the
...
// FormCagesTrackView.vue's `isDirty`.
let loadedSnapshot = dirtySnapshot()

function dirtySnapshot(): string {
  const header: Partial<Record<keyof ThreshingHeaderFormData, string>> = {}
 ...
 * device's local time. Mirrors FormCagesTrackView.vue's
...
// untouched.
function populateForm(record: ThreshingRecord): void {
  form.thresher_id = record.thresher_id ?? ''
...
function populateDetailRows(rows: ThreshingDraftWithDetails['details']): void {
...
async function loadDraft(): Promise<void> {
  ...
  try {
    const draft = await threshingRecordRepo.getDraftWithDetails(recordId)
    ...
    loadErrorMessage.value = err instanceof Error ? err.message : 'Gagal memuat draft threshing.'
  } finally {
    loading.value = false
  }
}
 ...
 * FormCagesTrackView.vue's `availableHourOptions()`. Both exclusions exempt
 * the row's OWN current selection. Reactive over the entire
...
function availableTimeSlotOptions(index: number): string[] {
  ...
  const order = threshingRecordRepo.canonicalTimeSlots()
...
// FormCagesTrackView.vue's hourSelectOptions() uses for its own per-row
// dropdown.
...
// mirrors FormCagesTrackView.vue's removeTippedTimeRow() exactly. Either
...
function validateHeader(): boolean {
  for (const key of Object.keys(errors) as (keyof ThreshingHeaderFormData)[]) {
    delete errors[key]
  }
...
function isRowFilled(row: ThreshingDetailFormRow): boolean {
  return (
...
// grid, not a per-field header error — mirrors FormCagesTrackView.vue's
// validateTippedTimeRows() exactly.
function isRowValid(row: ThreshingDetailFormRow): boolean {
...
function validateDetailRows(): boolean {
  ...
  if (!hasValidRow) {
    detailRowsError.value =
      'Minimal 1 baris Threshing Detail (Time-Slot terpilih + minimal 1 kolom bacaan terisi) harus diisi sebelum menyimpan.'
    return false
  }
...
async function onSimpan(): Promise<void> {
  ...
  try {
    await threshingRecordRepo.saveDraft(
      recordId,
      { ...form },
      [...detailRows.value],
      [...pendingDeletionIds.value],
    ...
    await syncAfterSave('threshing_record', recordId)
    router.push({ name: 'monitor-threshing' })
  } catch (err) {
    if (err instanceof ThreshingDetailRequiredError) {
    ...
    actionErrorMessage.value = err instanceof Error ? err.message : 'Gagal menyimpan data threshing.'
  } finally {
    saving.value = false
  }
}
...
async function onPause(): Promise<void> {
  ...
  try {
    await threshingRecordRepo.pauseDraftWithFormData(
      recordId,
      { ...form },
      [...detailRows.value],
      [...pendingDeletionIds.value],
...
async function onClearConfirm(): Promise<void> {
  ...
  try {
    await threshingRecordRepo.deleteDraft(recordId)
    router.push({ name: 'monitor-threshing' })
  } catch (err) {
    actionErrorMessage.value = err instanceof Error ? err.message : 'Gagal menghapus draft threshing.'
  } finally {
...
function goToMonitorThreshing(): void {
  router.push({ name: 'monitor-threshing' })
}
</script>

<template>
  <main class="form-threshing-view">
    ...
    <div class="form-threshing-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <button type="button" class="breadcrumb-link" data-testid="breadcrumb-threshing" @click="goToMonitorThreshing">
          Threshing
        </button>
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <span class="breadcrumb-current" aria-current="page">Form</span>
      </nav>
      <h1 class="screen-title">Form Threshing</h1>
    </div>

    <p v-if="loading" class="status-text">Memuat draft threshing…</p>
    <p v-else-if="notFound" class="status-text status-text--error" role="alert">
      Draft threshing tidak ditemukan.
    </p>
    ...
    <form v-else class="form-body" novalidate @submit.prevent="onSimpan">
      ...
      <section class="form-section" aria-label="Identitas Threshing">
        <h2 class="form-section-title">Identitas Threshing</h2>

        <FormField
          v-model="form.thresher_id"
          label="Thresher ID"
          required
          :error="errors.thresher_id"
      ...
      <section class="form-section" aria-label="Threshing Detail">
        <h2 class="form-section-title">Threshing Detail</h2>
        ...
        <div
          ...
          class="detail-row"
          data-testid="threshing-detail-row"
        >
          <div class="form-field">
          ...
          <FormField
            v-model="row.ffb_throughput_mt_hour"
            label="FFB Throughput (MT/hour)"
            type="number"
            :id="`ffb-throughput-${index}`"
            :data-testid="`ffb-throughput-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.thresher_drum_speed_rpm"
            label="Thresher Drum Speed (RPM)"
            type="number"
            :id="`drum-speed-${index}`"
            :data-testid="`drum-speed-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.motor_current_amps"
            label="Motor Current (Amps)"
            type="number"
            :id="`motor-current-${index}`"
            :data-testid="`motor-current-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.unstripped_bunch_count_percent"
            label="Unstripped Bunch Count (%)"
            type="number"
            :id="`unstripped-bunch-${index}`"
            :data-testid="`unstripped-bunch-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.empty_bunch_oil_loss_percent"
            label="Empty Bunch Oil Loss (%)"
            type="number"
            :id="`oil-loss-${index}`"
            :data-testid="`oil-loss-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
      ...
      <CollapsibleSection title="Target Operasional">
        ...
        <table class="operational-target-table" data-testid="operational-target-table">
          ...
          <tbody>
            <tr v-for="target in THRESHING_OPERATIONAL_TARGETS" :key="target.parameter">
    ...
    <ConfirmDialog
      ...
      title="Hapus Draft"
      message="Draft threshing ini akan dihapus secara permanen dan tidak dapat dikembalikan. Lanjutkan?"
