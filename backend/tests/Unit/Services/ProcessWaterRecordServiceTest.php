...
/**
 * ProcessWaterRecordServiceTest — screen-092--data-browser-process-water-web /
 * screen-102--detail-process-water-web / screen-112--form-process-water-web.
 *
 * Unit tests for App\Services\ProcessWaterRecordService, mirroring
 * ThreshingRecordServiceTest.php's structure — Process Water follows the
...
function processWaterFormPayload(array $overrides = []): array
{
    return array_merge([
...
it('returns an empty data list and meta.total = 0 when no records match the filter', function () {
    ProcessWaterRecord::factory()->forStation($this->station)->onDate('2026-01-01')->create();
...
it('returns a paginated, filtered list with the shared pagination meta shape', function () {
    ...
    ProcessWaterRecord::factory()->forStation($this->station)->onDate('2026-02-05')->count(3)->create();
    ProcessWaterRecord::factory()->forStation($this->station)->onDate('2026-03-01')->create();
    ProcessWaterRecord::factory()->forStation($otherStation)->onDate('2026-02-05')->create();
    ...
    expect(array_keys($result['data'][0]))->toBe(['id', 'process_water_id', 'date', 'filled_slot_count', 'status']);
});

it('computes filled_slot_count as the number of detail rows with at least one non-null reading column', function () {
    $withTwoFilled = ProcessWaterRecord::factory()->forStation($this->station)->onDate('2026-02-05')->create();
    ProcessWaterDetail::factory()->forRecord($withTwoFilled)->timeSlot('07:00')->filled()->create();
    ProcessWaterDetail::factory()->forRecord($withTwoFilled)->timeSlot('08:00')->create(['findings' => 'Perlu cek ulang']);
    ProcessWaterDetail::factory()->forRecord($withTwoFilled)->timeSlot('09:00')->create(); // unfilled

    $withNone = ProcessWaterRecord::factory()->forStation($this->station)->onDate('2026-02-05')->create();
    ProcessWaterDetail::factory()->forRecord($withNone)->timeSlot('07:00')->create();
...
it('does not count shift or inspector_id alone as a filled reading column', function () {
    $record = ProcessWaterRecord::factory()->forStation($this->station)->onDate('2026-02-05')->create();
    ProcessWaterDetail::factory()->forRecord($record)->timeSlot('07:00')->create(['shift' => 'Pagi', 'inspector_id' => 'INS-01']);

    $result = $this->service->listRecords(['date_from' => '2026-02-01', 'date_to' => '2026-02-10'], 1, 20);
    $rows = collect($result['data'])->keyBy('id');

    expect($rows[$record->id]['filled_slot_count'])->toBe(0);
});
...
it('throws ExportFailedException when the filtered dataset exceeds the export row limit', function () {
    $limit = ProcessWaterRecordService::EXPORT_ROW_LIMIT;
    $total = $limit + 1;
    ...
    while ($inserted < $total) {
        ...
        for ($i = 0; $i < $batch; $i++) {
            $rows[] = [
                ...
                'process_water_id' => 'PW-BULK-'.($inserted + $i),
        ...
        DB::table('process_water_records')->insert($rows);
        $inserted += $batch;
    }

    expect(ProcessWaterRecord::count())->toBe($total);
...
it('returns a StreamedResponse with the correct content-type for csv and excel formats', function (string $format, string $expectedContentType) {
    ProcessWaterRecord::factory()->forStation($this->station)->onDate('2026-02-05')->create();
    ProcessWaterRecord::factory()->forStation($this->station)->onDate('2026-02-05')->create();
    ...
    expect($body)->toContain('Process Water ID');
...
it('returns details array sorted into canonical time-slot order, not alphabetical', function () {
    $record = ProcessWaterRecord::factory()->forStation($this->station)->create();
    ProcessWaterDetail::factory()->forRecord($record)->timeSlot('00:00')->create();
...
it('returns null checked_by_name and acknowledged_by_name when not set', function () {
    $record = ProcessWaterRecord::factory()->forStation($this->station)->create(['checked_by' => null, 'acknowledged_by' => null]);
...
// screen-112--form-process-water-web: create()/update() tests below.

it('creates record with resolved station_id and only the given detail rows inserted (no forced 24)', function () {
    $result = $this->service->create(
        processWaterFormPayload(['production_line_id' => $this->processWaterStation->production_line_id]),
        $this->creator
    );

    expect($result['station_id'])->toBe($this->processWaterStation->id);
    expect($result['status'])->toBe('saved');
    expect($result['details'])->toHaveCount(1);
    expect(ProcessWaterDetail::where('process_water_record_id', $result['id'])->count())->toBe(1);
});

it('creates a record with multiple detail rows when given several valid, ascending time slots', function () {
    $result = $this->service->create(
        processWaterFormPayload([
            'production_line_id' => $this->processWaterStation->production_line_id,
            'details' => [
                ['time_slot' => '07:00', 'raw_water_flow_m3h' => 10],
                ['time_slot' => '09:00', 'raw_water_flow_m3h' => 20],
...
it('throws ValidationException when a required field is empty', function () {
    expect(fn () => $this->service->create(
        processWaterFormPayload(['production_line_id' => $this->processWaterStation->production_line_id, 'process_water_id' => '']),
        $this->creator
...
it('throws ValidationException when details is empty', function () {
    expect(fn () => $this->service->create(
        processWaterFormPayload(['production_line_id' => $this->processWaterStation->production_line_id, 'details' => []]),
        $this->creator
    ))->toThrow(ValidationException::class);
});
...
it('throws ValidationException when a detail row has a time_slot not in the 24 canonical slots', function () {
    expect(fn () => $this->service->create(
        processWaterFormPayload([
            'production_line_id' => $this->processWaterStation->production_line_id,
            'details' => [['time_slot' => '07:30', 'raw_water_flow_m3h' => 1]],
        ]),
        $this->creator
    ))->toThrow(ValidationException::class);
});
...
it('throws ValidationException when two detail rows share the same time_slot', function () {
    expect(fn () => $this->service->create(
        processWaterFormPayload([
            'production_line_id' => $this->processWaterStation->production_line_id,
            'details' => [
                ['time_slot' => '07:00', 'raw_water_flow_m3h' => 1],
                ['time_slot' => '07:00', 'raw_water_flow_m3h' => 2],
            ],
        ]),
        $this->creator
...
it('throws NoActiveProcessWaterStationException when production_line_id has no active process-water station', function () {
    $otherProductionLine = ProductionLine::factory()->create();

    expect(fn () => $this->service->create(
        processWaterFormPayload(['production_line_id' => $otherProductionLine->id]),
        $this->creator
    ))->toThrow(NoActiveProcessWaterStationException::class);
});
...
it('sets checked_by to requester id when checked=true and requester role=supervisor', function () {
    ...
    $result = $this->service->create(
        processWaterFormPayload(['production_line_id' => $this->processWaterStation->production_line_id, 'checked' => true]),
        $supervisor
    );
...
it('sets acknowledged_by to requester id when acknowledged=true and requester role=mill_management', function () {
    ...
    $result = $this->service->create(
        processWaterFormPayload(['production_line_id' => $this->processWaterStation->production_line_id, 'acknowledged' => true]),
        $millManagement
    );
...
it('updates record and upserts details: inserts new row, updates existing row, deletes removed row', function () {
    ...
    $keptDetail = ProcessWaterDetail::factory()->forRecord($record)->timeSlot('07:00')->create(['raw_water_flow_m3h' => 10]);
    ProcessWaterDetail::factory()->forRecord($record)->timeSlot('12:00')->filled()->create();

    $result = $this->service->update(
        $record->id,
        processWaterFormPayload([
            'process_water_id' => 'PW-EDITED',
            'details' => [
                ['id' => $keptDetail->id, 'time_slot' => '07:00', 'raw_water_flow_m3h' => 88.8],
    ...
    expect($result['process_water_id'])->toBe('PW-EDITED');
    expect($result['details'])->toHaveCount(2);
    expect(ProcessWaterDetail::where('process_water_record_id', $record->id)->count())->toBe(2);
    expect(ProcessWaterDetail::find($keptDetail->id)->raw_water_flow_m3h)->toBe(88.8);
    expect(ProcessWaterDetail::where('time_slot', '12:00')->exists())->toBeFalse();
});
