...
/**
 * DataBrowserGrading — screen-017--data-browser-grading-web /
 * "Data Browser Grading" (Livewire web page, route name
 * `data.grading`, /data/grading).
 *
 * Reuses GradingRecordService — the exact same service the API
 * controller (App\Http\Controllers\Api\GradingRecordController) uses —
 * so filtering/pagination rules stay identical between the web and API
 * entry points (mirrors screen-016's DataBrowserWeighbridge /
 * WeighbridgeRecordService pattern).
 *
 * Filter state mirrors the tech-spec exactly: date_from, date_to,
 * business_unit_id.
 *
 * Pagination: same approach as DataBrowserWeighbridge — calls
 * GradingRecordService::listRecords() directly (same call the API
 ...
 * /api/grading-records/export?...), NOT an AJAX/blob download — same
 * rationale as DataBrowserWeighbridge (session cookie carries auth on a
 * same-origin navigation, so a plain <a href> triggers the browser's
 * native file-download flow).
 */
#[Layout('data.grading')]
class DataBrowserGrading extends Component
{
    ...
    protected function exportUrl(string $format): string
    {
        ...
        return url('/api/grading-records/export').'?'.http_build_query($query);
    }
    ...
    public function render()
    {
        ...
        return view('livewire.data.data-browser-grading', [
            'records' => $result['data'],
            'meta' => $result['meta'],
