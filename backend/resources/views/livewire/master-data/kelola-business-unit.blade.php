<div class="kc-page" wire:loading.class="kc-page--busy" wire:target="nextPage,previousPage,save,confirmDelete">
    <div class="kc-page__header">
        <div>
            <h2 class="kc-page__title">Kelola Business Unit</h2>
            <p class="kc-page__subtitle">Kelola daftar Business Unit (di bawah Company) dalam struktur organisasi</p>
        </div>
        ...
        <table class="kc-table">
            ...
            <tbody>
                @forelse ($businessUnits as $businessUnit)
                    <tr class="kc-table__row" wire:key="business-unit-{{ $businessUnit['id'] }}">
                        <td>
                            @if ($businessUnit['logo_url'])
                                <img src="{{ $businessUnit['logo_url'] }}" alt="Logo {{ $businessUnit['name'] }}" class="kc-table__logo">
                            @else
                        ...
                        <td>{{ $businessUnit['company_name'] ?? '-' }}</td>
                        <td>{{ $businessUnit['station_count'] }}</td>
                        <td>{{ $businessUnit['created_at'] ? \Illuminate\Support\Carbon::parse($businessUnit['created_at'])->format('d/m/Y H:i') : '-' }}</td>
                        <td class="kc-table__actions">
                            @if ($confirmingDeleteId === $businessUnit['id'])
                            ...
                            @else
                                <button type="button" wire:click="openEditForm('{{ $businessUnit['id'] }}')" class="kc-button kc-button--ghost kc-button--sm">
                                    Edit
                                </button>
                                <button type="button" wire:click="askDelete('{{ $businessUnit['id'] }}')" class="kc-button kc-button--ghost kc-button--sm kc-button--danger-text">
                                    Hapus
                                </button>
                            @endif
                        </td>
                    ...
                    <tr class="kc-table__row kc-table__row--static">
                        <td colspan="7">
                            <div class="kc-empty">
                                ...
                                <p class="kc-empty__title">Belum ada Business Unit</p>
                                <p class="kc-empty__subtitle">Klik "Tambah Business Unit" untuk menambahkan data baru.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    @if ($showForm)
        <div class="kc-modal-backdrop" wire:key="business-unit-form-backdrop">
            <div class="kc-modal kc-modal--wide" role="dialog" aria-modal="true">
                <h3 class="kc-modal__title">
                    {{ $editingId !== null ? 'Edit Business Unit' : 'Tambah Business Unit' }}
                </h3>
                        ...
                        <div class="kc-form-section">
                            ...
                            <div class="kc-form-grid">
                                <div class="kc-form-field">
                                    <label for="company_id" class="kc-form-field__label">
                                        Company <span class="kc-form-field__required">*</span>
                                    </label>
                                    <x-searchable-select
                                        id="company_id"
                                        ...
                                        empty-message="Belum ada Company. Buat Company terlebih dahulu."
                                        :class="'kc-form-field__input'.($errors->has('company_id') ? ' kc-form-field__input--error' : '')"
                                    />
                                    @error('company_id')
                                        <p class="kc-form-field__error">{{ $message }}</p>
                                    @enderror
                                ...
                                <div class="kc-form-field">
                                    <label for="code" class="kc-form-field__label">
                                        Kode Business Unit <span class="kc-form-field__required">*</span>
                                    </label>
                                    <input
                                        type="text"
                                        id="code"
                                ...
                                <div class="kc-form-field">
                                    <label for="name" class="kc-form-field__label">
                                        Nama Business Unit <span class="kc-form-field__required">*</span>
                                    </label>
                                    <input
                                        type="text"
                                        id="name"
                                ...
                                <div class="kc-form-field">
                                    <label for="business_unit_type_code" class="kc-form-field__label">Tipe Business Unit</label>
                                    <input type="text" id="business_unit_type_code" wire:model="form.business_unit_type_code" class="kc-form-field__input @error('form.business_unit_type_code') kc-form-field__input--error @enderror">
                                    @error('form.business_unit_type_code')
                                        <p class="kc-form-field__error">{{ $message }}</p>
                                    @enderror
                                </div>
    ...
    <style>
        /* Design tokens — uiux-spec: brand #249360. Inlined here (same
           ...
           and resources/views/livewire/master-data/kelola-company.blade.php)
           since the backend scaffold has no frontend build pipeline yet —
           see implementation_notes. `kc-` prefix (Kelola Corporate/
           Company/Business-Unit — shared prefix reused across all three
           master-data screens) keeps these rules scoped/non-colliding with
