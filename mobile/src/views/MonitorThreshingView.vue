...
/**
 * MonitorThreshingView — screen-037--monitor-threshing /
 * usecase-037--monitor-threshing (mounted at /stations/threshing/monitor,
 ...
 * local (offline) `threshing_record` table (and its child `threshing_detail`
 * table, for the "filled slot" counter) via threshingRecordRepo.ts.
 * api_contracts.endpoints is empty for this screen.
 *
 * Structural pattern copied from MonitorCagesTrackView.vue (its own
 * entity-catalog v3 list-view rewrite): a "Hari Ini" 2-card counter row
 * plus a full scrollable LIST of the current user's ongoing/paused drafts,
 * every row labeled uniformly "Pause" (StatusBadge.vue with a fixed
 * status="paused" + label="Pause" override) — no ongoing-vs-paused
 * distinction is surfaced. This screen no longer mutates draft status from
 * here (no resumeDraft()/pauseDraft()/deleteDraft() calls) — those actions
 * live in Form Threshing (screen-041).
 *
 * Counter card labels per this screen's tech spec: "Jumlah Threshing
 * Record" (COUNT of today's threshing_record rows, any status) / "Jumlah
 * Baris Time-Slot Tercatat" (COUNT of threshing_detail rows belonging to
 * those records — REVISED 2026-08-24, entity-catalog v12: since
 * threshing_detail rows are no longer pre-created (see
 * threshingRecordRepo.ts's header comment), row EXISTENCE itself is the
 * meaningful signal now — a plain count of rows the user has added, with
 * no fixed-24 denominator, mirroring Cages Track's own "count of child
 * rows recorded" counter shape).
 *
 ...
 * (Home > Production Process Activity > Threshing, the first two tappable).
 *
 * Navigation:
 *  - 'New Data' -> threshingRecordRepo.createDraft(userId) (INSERTs a new
 *    status='draft_ongoing' header row ONLY — no threshing_detail rows are
 *    pre-created, see that function's doc comment), then -> route name
 *    `threshing-form` with the new draft's id.
 *  - Tapping a list item -> route name `threshing-form` with that item's
 *    EXISTING id, directly — no status-transition call.
 *  - 'Load Data' -> route name `data-preview-threshing` with NO id param
 *    (list mode).
 *  - 'Back' -> route name `station-list`.
 *
 * known_issues: StationListView.vue (screen-006, out of this task's scope)
 * has not yet been updated to link to `monitor-threshing` — this screen is
 * reachable once screen-006 is revised in a separate task, per the
 * coordinating session's instructions.
 */
...
import {
  threshingRecordRepo,
  type ThreshingDraftListItem,
  type ThreshingTodaySummary,
...
const drafts = ref<ThreshingDraftListItem[]>([])
...
// today.
const todaySummary = ref<ThreshingTodaySummary>({ countThreshingRecord: 0, detailRowCount: 0 })
...
async function loadDrafts() {
  ...
  try {
    drafts.value = await threshingRecordRepo.getDrafts(userId)
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal memuat daftar draft threshing lokal.'
  } finally {
    loading.value = false
  }
}
...
async function loadTodaySummary() {
  ...
  try {
    todaySummary.value = await threshingRecordRepo.getTodaySummary(userId)
  } catch {
...
/**
 * Thresher ID fallback text for a draft whose `thresher_id` has not been
 * filled in yet.
 */
function draftLabel(draft: ThreshingDraftListItem): string {
  return draft.thresher_id?.trim() ? draft.thresher_id : 'Thresher ID belum diisi'
}
...
async function onNewData() {
  ...
  try {
    const id = await threshingRecordRepo.createDraft(userId)
    router.push({ name: 'threshing-form', params: { id } })
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal membuat draft threshing baru.'
  }
}
...
function onDraftClick(draft: ThreshingDraftListItem) {
  router.push({ name: 'threshing-form', params: { id: draft.id } })
}

// business_logic step 7 — 'Load Data', list mode (no id param).
function onLoadData() {
  router.push({ name: 'data-preview-threshing' })
}
...
// business_logic step 8 — breadcrumb taps. 'Threshing' is the current page
...
<template>
  <main class="monitor-threshing-view">
    ...
    <div class="monitor-threshing-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <span class="breadcrumb-current" aria-current="page">Threshing</span>
      </nav>
      <h1 class="screen-title">Monitor Threshing</h1>
    </div>
    ...
    <section class="today-summary-section" aria-label="Hari Ini">
      ...
      <div class="today-summary-grid">
        <div class="today-summary-card">
          <span class="today-summary-label">Jumlah Threshing Record</span>
          <span class="today-summary-value" data-testid="counter-count-threshing-record">{{ todaySummary.countThreshingRecord }}</span>
        </div>
        <div class="today-summary-card">
    ...
    <p v-if="loading" class="status-text">Memuat daftar draft threshing lokal…</p>
    <p v-else-if="error" class="status-text status-text--error" role="alert">{{ error }}</p>

    <section v-else class="draft-list-section" aria-label="Daftar Draft Threshing">
      <p v-if="drafts.length === 0" class="empty-state" data-testid="draft-list-empty">
        Belum ada draft threshing tersimpan.
      </p>
