...
/**
 * SterilizerRecordController — screen-124--data-browser-sterilizer-web,
 * screen-125--detail-sterilizer-web, screen-126--form-sterilizer-web.
 * Mirrors CpoDispatchRecordController's structure exactly, delegating
 * all business logic to SterilizerRecordService (shared with the
 * Livewire components) so both web and API entry points apply the same
 * rules.
 */
class SterilizerRecordController extends Controller
{
    public function __construct(protected SterilizerRecordService $service) {}
    ...
    {
        $data = $request->only([
            'production_line_id', 'sterilizer_id', 'date', 'note',
    ...
    {
        $data = $request->only([
            'sterilizer_id', 'date', 'note', 'checked', 'acknowledged', 'details',
        ]);
