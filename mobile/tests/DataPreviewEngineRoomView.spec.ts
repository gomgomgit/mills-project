/**
 * DataPreviewEngineRoomView.spec.ts — screen-086--data-preview-engine-room
 * / usecase-093--data-preview-engine-room.
 *
...
import DataPreviewEngineRoomView from '@/views/DataPreviewEngineRoomView.vue'
import { canonicalTimeSlots, type EngineRoomDetailRow, type EngineRoomRecord } from '@/services/engineRoomRecordRepo'
...
function makeAllDetailRows(): EngineRoomDetailRow[] {
  return canonicalTimeSlots().map((timeSlot, index) => ({
    id: `detail-${index}`,
    engine_room_record_id: 'rec-1',
    time_slot: timeSlot,
...
describe('DataPreviewEngineRoomView — list mode', () => {
  ...
  it('navigates to Form Engine Room when a draft/pause item is tapped', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'engine-room-form', params: { id: 'rec-draft' } })
  })
  ...
  it('navigates to detail mode when a saved/synced item is tapped', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'data-preview-engine-room', params: { id: 'rec-saved' } })
  })

  it('navigates to Monitor Engine Room when Back is pressed in list mode', async () => {
    const wrapper = mount(DataPreviewEngineRoomView)
    await flushPromises()
...
describe('DataPreviewEngineRoomView — detail mode', () => {
  it('renders the record read-only with all of its detail rows (a full 24, when all canonical slots were added)', async () => {
    ...
    expect(wrapper.find('[data-testid="engine-room-detail-rows-list"]').exists()).toBe(true)
    expect(wrapper.findAll('[data-testid^="engine-room-detail-row-"]')).toHaveLength(24)
  })
  ...
  it('shows the empty-state message when a record has zero detail rows', async () => {
    ...
    expect(wrapper.find('[data-testid="engine-room-detail-rows-empty"]').exists()).toBe(true)
  ...
  it('navigates back to list mode (not Monitor Engine Room) when Back is pressed in detail mode', async () => {
    routeState.params = { id: 'rec-1' }
    getDraftWithDetailsMock.mockResolvedValue({ record: makeRecord(), details: makeAllDetailRows() })
