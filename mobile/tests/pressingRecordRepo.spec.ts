/**
 * pressingRecordRepo.spec.ts — screen-038--monitor-pressing /
 * screen-042--form-pressing / screen-046--data-preview-pressing.
 *
 * Unit tests for src/services/pressingRecordRepo.ts — covers the
 * authoritative unit_test_cases for these 3 screens' repo layer (mobile-only
 * / local-SQLite-only, no REST api_contract). Mirrors
 * threshingRecordRepo.spec.ts's structure/pattern, adapted for Pressing's
 ...
 * pauseDraftWithFormData() upsert/delete pressing_detail rows via
 * applyDetailRowChanges(), mirroring threshingRecordRepo.ts's
...
function makeDetailRow(overrides: Partial<PressingDetailRow> = {}): PressingDetailRow {
  return {
    id: 'detail-1',
    pressing_record_id: 'draft-1',
    time_slot: '07:00',
...
function makeHeaderData(overrides: Partial<PressingHeaderFormData> = {}): PressingHeaderFormData {
  return {
    presser_id: 'PR-01',
...
function makeDetailFormRow(overrides: Partial<PressingDetailFormRow> = {}): PressingDetailFormRow {
  return {
    time_slot: '07:00',
    digester_temp_c: null,
    digester_level_percent: null,
...
function makeValidDetailRows(overrides: Partial<PressingDetailFormRow>[] = []): PressingDetailFormRow[] {
  if (overrides.length === 0) {
    return [makeDetailFormRow({ time_slot: '07:00', digester_temp_c: 92 })]
  }

  return overrides.map((o) => makeDetailFormRow(o))
}

describe('pressingRecordRepo', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  describe('createDraft()', () => {
    it('inserts a new draft_ongoing pressing_record with date auto-filled, and inserts NO pressing_detail rows', async () => {
      ...
      expect(headerSql).toContain('INSERT INTO pressing_record')
  ...
  describe('getDrafts()', () => {
    it('returns the draft/pause list, most-recently-updated first', async () => {
      const rows = [
        { id: 'draft-newest', status: 'draft_paused' as const, presser_id: 'PR-002', updated_at: '2026-08-24T10:00:00.000Z' },
        { id: 'draft-oldest', status: 'draft_ongoing' as const, presser_id: 'PR-001', updated_at: '2026-08-24T08:00:00.000Z' },
      ]
  ...
  describe('getTodaySummary()', () => {
    it("computes today's record count and a plain COUNT of pressing_detail rows belonging to those records (not filtered by filled-ness)", async () => {
      vi.mocked(query)
      ...
      expect(result).toEqual({ countPressingRecord: 2, detailRowCount: 5 })
      expect(query).toHaveBeenNthCalledWith(1, expect.stringContaining('FROM pressing_record'), [USER_ID])
      ...
      expect(countSql).toContain('FROM pressing_detail')
      expect(countSql).toContain('WHERE pressing_record_id IN (?, ?)')
    ...
    it('returns zero counters and skips the detail query when no records match today', async () => {
      ...
      expect(result).toEqual({ countPressingRecord: 0, detailRowCount: 0 })
  ...
  describe('getAllRecords()', () => {
    it('returns every record for the user regardless of status', async () => {
      ...
      expect(result).toEqual(rows)
      expect(query).toHaveBeenCalledWith(expect.stringContaining('FROM pressing_record WHERE created_by = ?'), [
        USER_ID,
      ])
      expect(query).toHaveBeenCalledWith(expect.stringContaining('ORDER BY updated_at DESC'), [USER_ID])
    })
  ...
  describe('saveDraft()', () => {
    it('throws PressingDetailRequiredError and calls neither run() nor query() when details is empty', async () => {
    ...
    it('throws PressingDetailRequiredError when every row lacks a selected time_slot or has zero reading columns filled', async () => {
      const headerData = makeHeaderData()
      const details: PressingDetailFormRow[] = [
    ...
    it('updates the header with status=saved, INSERTs a new (id-less) detail row, and returns the freshly saved draft', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      const saved = makeRecord({ id: 'draft-4', presser_id: 'PR-01', status: 'saved' })
      const savedDetails = [makeDetailRow({ id: 'detail-new-1', pressing_record_id: 'draft-4' })]
      vi.mocked(query).mockResolvedValueOnce([saved]).mockResolvedValueOnce(savedDetails)

      const headerData = makeHeaderData()
      const details = makeValidDetailRows([{ time_slot: '09:00', digester_temp_c: 92 }])
      ...
      const [headerSql, headerParams] = vi.mocked(run).mock.calls[0]
      expect(headerSql).toContain('UPDATE pressing_record')
      expect(headerSql).toContain("status = 'saved'")
      expect(headerParams).toEqual(
        expect.arrayContaining(['PR-01', expect.any(String), null, null, expect.any(String), 'draft-4']),
      )

      const [detailSql, detailParams] = vi.mocked(run).mock.calls[1]
      expect(detailSql).toContain('INSERT INTO pressing_detail')
    ...
    it('UPDATEs an existing detail row (has an id) instead of inserting it', async () => {
      ...
      const headerData = makeHeaderData()
      const details: PressingDetailFormRow[] = [
        { id: 'detail-existing-1', time_slot: '11:00', digester_temp_c: 95, digester_level_percent: null, press_motor_current_amps: null, cone_hydraulic_pressure_bar: null, dilution_water_temp_c: null, downtime_reason: null },
      ]
      ...
      expect(run).toHaveBeenCalledTimes(2)
      const [detailSql, detailParams] = vi.mocked(run).mock.calls[1]
      expect(detailSql).toContain('UPDATE pressing_detail')
      expect(detailParams).toEqual(['11:00', 95, null, null, null, null, null, expect.any(String), 'detail-existing-1'])
    })
    ...
    it('upserts multiple detail rows sequentially, in details order (mixed insert + update)', async () => {
      ...
      const headerData = makeHeaderData()
      const details: PressingDetailFormRow[] = [
        { id: 'detail-existing-2', time_slot: '07:00', digester_temp_c: 1, digester_level_percent: null, press_motor_current_amps: null, cone_hydraulic_pressure_bar: null, dilution_water_temp_c: null, downtime_reason: null },
        makeDetailFormRow({ time_slot: '13:00', digester_temp_c: 2 }),
      ]
      ...
      expect(firstRowSql).toContain('UPDATE pressing_detail')
      expect(secondRowSql).toContain('INSERT INTO pressing_detail')
    })
    ...
    it('DELETEs every id in detailIdsToDelete, after the upserts', async () => {
      ...
      expect(deleteSql1).toContain('DELETE FROM pressing_detail WHERE id = ?')
      expect(deleteParams1).toEqual(['detail-removed-1'])
      expect(deleteSql2).toContain('DELETE FROM pressing_detail WHERE id = ?')
    ...
    it('throws when getDraftWithDetails() re-read after save unexpectedly finds nothing', async () => {
      ...
      await expect(saveDraft('draft-8', headerData, details, [], 'operator')).rejects.toThrow(
        'Gagal memuat ulang data pressing setelah disimpan.',
      )
    })
  })
  ...
  describe('pauseDraftWithFormData()', () => {
    ...
    it('upserts detail rows and applies pending deletions, same contract as saveDraft()', async () => {
      ...
      expect(deleteSql).toContain('DELETE FROM pressing_detail WHERE id = ?')
    ...
    it('throws when getDraftWithDetails() re-read after pause unexpectedly finds nothing', async () => {
      ...
      await expect(pauseDraftWithFormData('draft-1', headerData, [], [], 'operator')).rejects.toThrow(
        'Gagal menyimpan progres pressing setelah pause.',
      )
    })
  })

  describe('deleteDraft()', () => {
    it('deletes pressing_detail rows before deleting the pressing_record row', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      ...
      expect(run).toHaveBeenNthCalledWith(
        1,
        expect.stringContaining('DELETE FROM pressing_detail WHERE pressing_record_id = ?'),
        ['draft-1'],
      )
      expect(run).toHaveBeenNthCalledWith(2, expect.stringContaining('DELETE FROM pressing_record WHERE id = ?'), [
        'draft-1',
      ])
    })
  })
