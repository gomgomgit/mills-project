/**
 * MonitorClarificationView.spec.ts — screen-069--monitor-clarification /
 * usecase-109--monitor-clarification.
 *
 * Mirrors MonitorBoilerRoomView.spec.ts's structure/pattern exactly.
 *
 ...
 *   4. 'New Data' creates a new draft and navigates to Form Clarification
 *   5. tapping a list item navigates with the EXISTING draft id
 *   6. 'Load Data' navigates to Data Preview Clarification (list mode, no id)
...
import MonitorClarificationView from '@/views/MonitorClarificationView.vue'
import type { ClarificationDraftListItem } from '@/services/clarificationRecordRepo'
...
function makeDraft(overrides: Partial<ClarificationDraftListItem> & { id: string }): ClarificationDraftListItem {
  return {
...
beforeEach(() => {
  ...
  getTodaySummaryMock.mockResolvedValue({ countClarificationRecord: 0, detailRowCount: 0 })
})

describe('MonitorClarificationView', () => {
  it('loads and renders a populated draft/pause list', async () => {
    getDraftsMock.mockResolvedValue([
      makeDraft({ id: 'draft-1', clarification_id: 'CLR-001' }),
      makeDraft({ id: 'draft-2', clarification_id: 'CLR-002' }),
    ])

    const wrapper = mount(MonitorClarificationView)
    await flushPromises()
  ...
  it('creates a new draft and navigates to Form Clarification when New Data is pressed', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'clarification-form', params: { id: 'new-draft-id' } })
  })

  it('navigates to Form Clarification with the existing draft id when a list item is tapped, without any status-update call', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'clarification-form', params: { id: 'draft-existing' } })
    expect(createDraftMock).not.toHaveBeenCalled()
  })

  it('navigates to Data Preview Clarification in list mode with no id when Load Data is pressed', async () => {
    const wrapper = mount(MonitorClarificationView)
    await flushPromises()
  ...
  it("renders the Hari Ini counter cards from getTodaySummary()'s resolved values", async () => {
    getTodaySummaryMock.mockResolvedValue({ countClarificationRecord: 3, detailRowCount: 7 })

    const wrapper = mount(MonitorClarificationView)
    await flushPromises()

    expect(wrapper.find('[data-testid="counter-count-clarification-record"]').text()).toBe('3')
  ...
  it('renders zero counters when there is no data today', async () => {
    ...
    expect(wrapper.find('[data-testid="counter-count-clarification-record"]').text()).toBe('0')
