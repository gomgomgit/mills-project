/**
 * FormDepricarpingView.spec.ts — screen-043--form-depricarping /
 * usecase-043--form-depricarping.
 *
 * REVISED 2026-08-24 (entity-catalog v12): mirrors
 * FormPressingView.spec.ts's add-row/remove-row interaction helpers and
 * coverage exactly — the Depricarping Detail grid is no longer a fixed
 * 24-row grid, it now uses the exact same dynamic add-row/remove-row
 * pattern as Pressing Detail / Threshing Detail / Cages Tipped Time,
 * adapted for Depricarping's own header field (presser_id) and 8 reading
 * columns (7 numeric + downtime_minutes + findings — where a row is
 ...
 *   - required-field validation (Presser ID)
 *   - "at least one valid row" validation (Time-Slot + reading filled)
 *   - Simpan success (status=saved), navigates to Monitor Depricarping
 ...
 *     mill_management
 *   - Target Operasional reference table renders 6 read-only rows with 4
 *     columns (Parameter/Metric, Target Range, Critical Limit, Operational
 *     Consequence / Justification) — untouched by this revision
 */
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { mount, flushPromises } from '@vue/test-utils'
import FormDepricarpingView from '@/views/FormDepricarpingView.vue'
import {
...
vi.mock('@/services/depricarpingRecordRepo', async () => {
  const actual = await vi.importActual<typeof import('@/services/depricarpingRecordRepo')>(
    '@/services/depricarpingRecordRepo',
  )
  return {
    ...actual,
    default: {
...
// Mirrors FormPressingView.spec.ts's helpers exactly.
...
describe('FormDepricarpingView', () => {
  it('loads existing detail rows (however many) ordered by canonical time-slot on mount', async () => {
    ...
    const wrapper = mount(FormDepricarpingView)
    await flushPromises()

    const rows = wrapper.findAll('[data-testid="depricarping-detail-row"]')
  ...
  it('renders zero detail rows for a brand-new draft (no pre-created rows)', async () => {
    ...
    expect(wrapper.findAll('[data-testid="depricarping-detail-row"]')).toHaveLength(0)
  })
  ...
  it('adds one row at a time via "Tambah baris"', async () => {
    ...
    expect(wrapper.findAll('[data-testid="depricarping-detail-row"]')).toHaveLength(1)

    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    expect(wrapper.findAll('[data-testid="depricarping-detail-row"]')).toHaveLength(2)
  })
  ...
  it('shows inline error and does not save when Presser ID is empty on Simpan', async () => {
    const wrapper = mount(FormDepricarpingView)
    await flushPromises()
    ...
    expect(saveDraftMock).not.toHaveBeenCalled()
    expect(wrapper.text()).toContain('Presser ID wajib diisi')
  })
  ...
  it('shows a grid-specific error and does not save when there are zero valid detail rows', async () => {
    ...
    await wrapper.find('#field-presser-id').setValue('DP-01')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()
  ...
  it('saves successfully and navigates to Monitor Depricarping when Presser ID and a valid detail row exist', async () => {
    ...
    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    await chooseSearchableOption(wrapper, 'time-slot-select-0', '07:00')
    await wrapper.find('#fan-static-pressure-0').setValue('45')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()
    ...
    expect(headerData.presser_id).toBe('DP-01')
    expect(details).toHaveLength(1)
    expect(details[0].time_slot).toBe('07:00')
    expect(idsToDelete).toEqual([])
    expect(pushMock).toHaveBeenCalledWith({ name: 'monitor-depricarping' })
  })
  ...
  it('accepts a row filled only via Downtime (Mins) or Findings as satisfying the minimum-one-row rule', async () => {
    ...
    await wrapper.find('#field-presser-id').setValue('DP-02')
    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    await chooseSearchableOption(wrapper, 'time-slot-select-0', '07:00')
    await wrapper.find('#findings-0').setValue('Fan belt loose')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()
  ...
  it('accepts a row filled only via Downtime (Mins) as satisfying the minimum-one-row rule', async () => {
    ...
    await wrapper.find('#field-presser-id').setValue('DP-03')
    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    await chooseSearchableOption(wrapper, 'time-slot-select-0', '07:00')
  ...
  it('falls back to the grid error (defense-in-depth) when the repo throws DepricarpingDetailRequiredError', async () => {
    saveDraftMock.mockRejectedValue(new DepricarpingDetailRequiredError())
    ...
    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    await chooseSearchableOption(wrapper, 'time-slot-select-0', '07:00')
    await wrapper.find('#fan-static-pressure-0').setValue('1')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()

    expect(wrapper.find('[data-testid="detail-rows-error"]').exists()).toBe(true)
    expect(pushMock).not.toHaveBeenCalledWith({ name: 'monitor-depricarping' })
  })
  ...
  it('shows a confirm dialog on Back when the form is dirty', async () => {
    ...
    await wrapper.find('#field-presser-id').setValue('DP-99')
  ...
  it('renders the Target Operasional reference table with 6 read-only rows and 4 columns', async () => {
    const wrapper = mount(FormDepricarpingView)
    await flushPromises()
    ...
    expect(table.text()).toContain('Fan Static Pressure')
    expect(table.text()).toContain('40 - 50 mmH2O')
    expect(table.text()).toContain('Low pressure drops fibre early')
  })
