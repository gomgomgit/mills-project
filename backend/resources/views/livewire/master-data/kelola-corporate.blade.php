<div class="kc-page" wire:loading.class="kc-page--busy" wire:target="nextPage,previousPage,save,confirmDelete">
    <div class="kc-page__header">
        <div>
            <h2 class="kc-page__title">Kelola Corporate</h2>
            <p class="kc-page__subtitle">Kelola daftar Corporate (level teratas struktur organisasi)</p>
        </div>
        ...
        <table class="kc-table">
            ...
            <tbody>
                @forelse ($corporates as $corporate)
                    <tr class="kc-table__row" wire:key="corporate-{{ $corporate['id'] }}">
                        <td>
                            @if ($corporate['logo_url'])
                                <img src="{{ $corporate['logo_url'] }}" alt="Logo {{ $corporate['name'] }}" class="kc-table__logo">
                            @else
                                <span class="kc-table__logo-placeholder" aria-hidden="true">&#127970;</span>
                            @endif
                        </td>
                        <td>{{ $corporate['corporate_code'] ?? '-' }}</td>
                        <td>{{ $corporate['name'] }}</td>
                        <td>{{ $corporate['company_count'] }}</td>
                        <td>{{ $corporate['created_at'] ? \Illuminate\Support\Carbon::parse($corporate['created_at'])->format('d/m/Y H:i') : '-' }}</td>
                        <td class="kc-table__actions">
                            @if ($confirmingDeleteId === $corporate['id'])
                            ...
                            @else
                                <button type="button" wire:click="openEditForm('{{ $corporate['id'] }}')" class="kc-button kc-button--ghost kc-button--sm">
                                    Edit
                                </button>
                                <button type="button" wire:click="askDelete('{{ $corporate['id'] }}')" class="kc-button kc-button--ghost kc-button--sm kc-button--danger-text">
                                    Hapus
                                </button>
                            @endif
                        </td>
                    ...
                    <tr class="kc-table__row kc-table__row--static">
                        <td colspan="6">
                            <div class="kc-empty">
                                ...
                                <p class="kc-empty__title">Belum ada Corporate</p>
                                <p class="kc-empty__subtitle">Klik "Tambah Corporate" untuk menambahkan data baru.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
                        ...
                        <div class="kc-form-section">
                            ...
                            <div class="kc-form-grid">
                                <div class="kc-form-field">
                                    <label for="corporate_code" class="kc-form-field__label">
                                        Kode Corporate <span class="kc-form-field__required">*</span>
                                    </label>
                                    <input
                                        type="text"
                                        id="corporate_code"
                                        wire:model="form.corporate_code"
                                        class="kc-form-field__input @error('form.corporate_code') kc-form-field__input--error @enderror"
                                        autofocus
                                    >
                                    @error('form.corporate_code')
                                        <p class="kc-form-field__error">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="kc-form-field">
                                    <label for="name" class="kc-form-field__label">
                                        Nama Corporate <span class="kc-form-field__required">*</span>
                                    </label>
                                    <input
                                        type="text"
                                        id="name"
    ...
    <style>
        /* Design tokens — uiux-spec: brand #249360. Inlined here (same
           approach as every other Livewire view in this codebase so far —
           resources/views/livewire/data/data-browser-weighbridge.blade.php,
           resources/views/livewire/settings/change-password-form.blade.php)
           since the backend scaffold has no frontend build pipeline yet —
           see implementation_notes. `kc-` prefix (Kelola Corporate) keeps
           these rules scoped/non-colliding with other screens' inlined
           styles, mirroring the `wb-` / `form-field__` precedent. */
        .kc-page {
            --kc-brand: #249360;
