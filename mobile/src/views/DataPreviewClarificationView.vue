...
/**
 * DataPreviewClarificationView —
 * screen-089--data-preview-clarification /
 * usecase-111--data-preview-clarification (mounted at
 * /stations/clarification/preview/:id? — `:id` is OPTIONAL; meta.public =
 ...
 *
 * Dual-mode, mirroring DataPreviewBoilerRoomView.vue exactly:
 *
 *  - LIST mode — route has NO `:id` param. Shows every local
 *    clarification_record row for the current user, ANY status, with a
 *    client-side date filter (defaults to today) + search filter (on
 *    clarification_id). Tapping a row navigates onward:
 *      - draft_ongoing/draft_paused rows -> `clarification-form`.
 ...
 *    (full header + Clarification Detail grid, however many rows the user
 *    added), reusing `getDraftWithDetails()` (already exported by
 *    clarificationRecordRepo.ts for screen-079). Renders Checked By and
 ...
 *    (an empty-state message, `clarification-detail-rows-empty`).
 *
 ...
 * DataPreviewBoilerRoomView.vue's header comment (Vue Router reuses this
 * component instance for param-only navigations on the same route
 * record).
 *
 * Header/breadcrumb/nav-menu: copied verbatim from
 * DataPreviewBoilerRoomView.vue. Breadcrumb is 4 segments deep (Home >
 * Production Process Activity > Clarification > Load Data).
 *
 * List-row status badge (StatusBadge.vue):
 *  - draft_ongoing / draft_paused -> status="paused" label="Pause" (matches
 *    Monitor Clarification's list exactly).
 ...
 * (same as DataPreviewBoilerRoomView.vue's DETAIL_STATUS_BADGE_MAP).
 *
 * Date filter: `clarification_record.date` is a full local datetime string
 * (`YYYY-MM-DDTHH:mm:ss`, set by FormClarificationView.vue's
...
const allRecords = ref<ClarificationRecord[]>([])
...
async function loadList(): Promise<void> {
  ...
  try {
    allRecords.value = await clarificationRecordRepo.getAllRecords(userId)
  } catch (err) {
    listError.value = err instanceof Error ? err.message : 'Gagal memuat daftar data clarification lokal.'
  } finally {
    listLoading.value = false
  }
}

function recordLabel(item: ClarificationRecord): string {
  return item.clarification_id?.trim() ? item.clarification_id : 'Clarification ID belum diisi'
}

const LIST_STATUS_BADGE_MAP: Record<ClarificationRecord['status'], { status: BadgeStatus; label: string }> = {
...
function listBadgeInfo(item: ClarificationRecord) {
...
const filteredRecords = computed(() => {
  ...
  return allRecords.value.filter((item) => {
    ...
    if (keyword) {
      const clarificationIdMatch = (item.clarification_id ?? '').toLowerCase().includes(keyword)
      if (!clarificationIdMatch) {
        return false
      }
    }
...
function onItemClick(item: ClarificationRecord): void {
  if (item.status === 'draft_ongoing' || item.status === 'draft_paused') {
    router.push({ name: 'clarification-form', params: { id: item.id } })
    return
  }

  router.push({ name: 'data-preview-clarification', params: { id: item.id } })
}
...
const detailRecord = ref<ClarificationRecord | null>(null)
const detailRows = ref<ClarificationDetailRow[]>([])
...
async function loadDetail(recordId: string): Promise<void> {
  ...
  try {
    ...
    detailLoadErrorMessage.value = err instanceof Error ? err.message : 'Gagal memuat data clarification.'
  } finally {
...
const DETAIL_STATUS_BADGE_MAP: Record<ClarificationRecord['status'], { status: BadgeStatus; label: string }> = {
...
const screenTitle = computed(() => {
  ...
  return detailRecord.value?.clarification_id
    ? `Clarification ${detailRecord.value.clarification_id}`
    : 'Detail Clarification'
})

function onBack(): void {
  if (isDetailMode.value) {
    router.push({ name: 'data-preview-clarification' })
    return
  }
...
<template>
  <main class="data-preview-clarification-view">
    ...
    <div class="preview-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <button
          type="button"
          class="breadcrumb-link"
          data-testid="breadcrumb-clarification"
          @click="goToMonitorClarification"
        >
          Clarification
        </button>
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
    ...
    <template v-if="!isDetailMode">
      ...
      <section v-else class="record-list-section" aria-label="Daftar Data Clarification">
    ...
    <!-- DETAIL mode -->
    <template v-else>
      <p v-if="detailLoading" class="status-text">Memuat data clarification…</p>
      <p
      ...
      >
        Data clarification tidak ditemukan.
      </p>
      ...
      <div v-else-if="detailRecord" class="preview-body">
        <FormField :model-value="detailRecord.clarification_id" label="Clarification ID" disabled />
        ...
        <section class="detail-rows-section" aria-label="Clarification Detail">
          <h2 class="detail-rows-title">Clarification Detail</h2>

          <p v-if="detailRows.length === 0" class="status-text" data-testid="clarification-detail-rows-empty">
            Belum ada baris clarification detail.
          </p>

          <ul v-else class="detail-rows-list" data-testid="clarification-detail-rows-list">
            <li
              v-for="row in detailRows"
              :key="row.id"
              class="detail-row-view"
              :data-testid="`clarification-detail-row-${row.id}`"
            >
              <div class="detail-row-view-header">
                <span class="detail-row-view-time-slot">{{ row.time_slot }}</span>
              </div>
              <div class="detail-row-view-grid">
                <span>Clarification Tank Temp (°C): {{ row.clarification_tank_temp_c ?? '-' }}</span>
                <span>Oil Tank Temperature (°C): {{ row.oil_tank_temperature_c ?? '-' }}</span>
                <span>Sludge Tank Temp (°C): {{ row.sludge_tank_temp_c ?? '-' }}</span>
                <span>Buffer Tank Level (%): {{ row.buffer_tank_level_percent ?? '-' }}</span>
                <span>Pure Oil Production Rate (Ton/Hour): {{ row.pure_oil_production_rate_ton_hour ?? '-' }}</span>
                <span>Downtime (Mins): {{ row.downtime_mins ?? '-' }}</span>
...
<style scoped>
.data-preview-clarification-view {
  min-height: 100vh;
  display: flex;
