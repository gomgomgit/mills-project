...
/**
 * FormDepricarpingView — screen-043--form-depricarping /
 * usecase-043--form-depricarping (mounted at
 * /stations/depricarping/form/:id, meta.public = false — requires an
 ...
 * local (offline) `depricarping_record` + `depricarping_detail` tables via
 * depricarpingRecordRepo.ts.
 *
 ...
 * verbatim from FormPressingView.vue (screen-042), Depricarping's exact
 * structural sibling, adapted for Depricarping's own header field and 8
 * reading columns:
 *
 *  - "Identitas Depricarping" section: `presser_id` (required, freely
 *    editable — labeled "Presser ID" per entity-catalog, the same field
 *    name/label as Pressing's header ID, not a typo), `date` (auto-set once,
 *    in memory, ONLY for a brand-new draft —
 *    depricarpingRecordRepo.ts's createDraft() already auto-fills it at
 *    draft-creation time; this screen's populateForm() re-applies the same
 *    "only if still empty" rule defensively). Always rendered disabled.
 *
 ...
 *    EXACT pattern FormPressingView.vue's Pressing Detail grid already uses
 *    (itself mirroring FormThreshingView.vue / FormCagesTrackView.vue):
 *    `detailRows` (in-memory, rows without a saved `id` are new),
 ...
 *    logic as FormPressingView.vue's `availableTimeSlotOptions()`). Each
 *    row's Time-Slot is now a SearchableSelect dropdown (was a read-only
 *    label); the 7 numeric reading fields, Downtime (Mins), and Findings
 *    stay freely editable at all times, unchanged. "Tambah baris" is
 *    disabled once all 24 canonical slots are already used
 *    (`detailRows.length >= 24`) or while an action is in progress.
 *    STRUCTURAL DIFFERENCE FROM THRESHING/PRESSING: those stations have one
 *    free-text "Downtime Reason" column; Depricarping has TWO separate
 *    columns instead — "Downtime (Mins)" (numeric) and "Findings" (free
 *    text) — matching its own source log sheet exactly, kept distinct
 *    end-to-end and unaffected by this dynamic-row revision (a row counts
 *    as filled when EITHER is set, same as any other reading column).
 *
 *  - "Target Operasional" section — read-only reference table (6 rows, from
 *    the static DEPRICARPING_OPERATIONAL_TARGETS constant — see that
 ...
 *    save/pause payload. UNTOUCHED by this revision. STRUCTURAL DIFFERENCE
 *    FROM THRESHING/PRESSING: this table has 4 columns (Parameter/Metric,
 *    Target Range, Critical Limit, Operational Consequence / Justification)
 *    — one more than Threshing/Pressing's 3-column shape.
 *
 *  - 'Simpan': validates presser_id (required) — inline error — AND at
 *    least one valid Depricarping Detail row (a selected Time-Slot AND at
 ...
 *    repo level (DepricarpingDetailRequiredError) — mirrors
 *    FormPressingView.vue's grid validation exactly. On success: calls
 *    depricarpingRecordRepo.saveDraft() (status='saved'), navigates to
 *    `monitor-depricarping`.
 *  - 'Pause': no validation at all, persists current field values as-is via
 *    depricarpingRecordRepo.pauseDraftWithFormData() (status='draft_paused'),
 *    navigates to `monitor-depricarping`.
 *  - 'Clear': ConfirmDialog.vue first; on confirm, calls
 *    depricarpingRecordRepo.deleteDraft() (cascades delete to every
 *    depricarping_detail row for the record, however many exist), then
 *    navigates to `monitor-depricarping`; on cancel, no change.
 ...
 *
 * Header/breadcrumb/nav-menu: copied verbatim from FormPressingView.vue.
 * Breadcrumb is 4 segments deep (Home > Production Process Activity >
 * Depricarping > Form), the first 3 tappable.
 */
...
import depricarpingRecordRepo, {
  DepricarpingDetailRequiredError,
  type DepricarpingDetailFormRow,
  type DepricarpingDraftWithDetails,
...
const REQUIRED_FIELDS: (keyof DepricarpingHeaderFormData)[] = ['presser_id']
...
// reasoning as FormPressingView.vue excluding date).
const DIRTY_CHECK_FIELDS: (keyof DepricarpingHeaderFormData)[] = [
  'presser_id',
  'note',
  'checked_by',
  'acknowledged_by',
...
function emptyFormData(): DepricarpingHeaderFormData {
  return {
    presser_id: '',
    date: '',
    note: '',
...
const form = reactive<DepricarpingHeaderFormData>(emptyFormData())
const errors = reactive<Partial<Record<keyof DepricarpingHeaderFormData, string>>>({})
const detailRows = ref<DepricarpingDetailFormRow[]>([])
...
// next Simpan/Pause call (mirrors FormPressingView.vue's
...
// surfaced on the Depricarping Detail section rather than mixed into the
...
// FormPressingView.vue's `isDirty`.
let loadedSnapshot = dirtySnapshot()

function dirtySnapshot(): string {
  const header: Partial<Record<keyof DepricarpingHeaderFormData, string>> = {}
 ...
 * device's local time. Mirrors FormPressingView.vue's
...
// untouched.
function populateForm(record: DepricarpingRecord): void {
...
function populateDetailRows(rows: DepricarpingDraftWithDetails['details']): void {
...
async function loadDraft(): Promise<void> {
  ...
  try {
    const draft = await depricarpingRecordRepo.getDraftWithDetails(recordId)
    ...
    loadErrorMessage.value = err instanceof Error ? err.message : 'Gagal memuat draft depricarping.'
  } finally {
    loading.value = false
  }
}
 ...
 * FormPressingView.vue's `availableTimeSlotOptions()`. Both exclusions
...
function availableTimeSlotOptions(index: number): string[] {
  ...
  const order = depricarpingRecordRepo.canonicalTimeSlots()
...
// FormPressingView.vue's timeSlotSelectOptions() uses for its own per-row
// dropdown.
...
// mirrors FormPressingView.vue's removeDetailRow() exactly. Either way, the
// row's time-slot is immediately freed for other rows' dropdowns (falls out
// of availableTimeSlotOptions() automatically once the row is gone from
// detailRows.value).
...
function validateHeader(): boolean {
  for (const key of Object.keys(errors) as (keyof DepricarpingHeaderFormData)[]) {
    delete errors[key]
  }
...
function isRowFilled(row: DepricarpingDetailFormRow): boolean {
  return (
...
// grid, not a per-field header error — mirrors FormPressingView.vue's
// validateDetailRows() exactly.
function isRowValid(row: DepricarpingDetailFormRow): boolean {
...
function validateDetailRows(): boolean {
  ...
  if (!hasValidRow) {
    detailRowsError.value =
      'Minimal 1 baris Depricarping Detail (Time-Slot terpilih + minimal 1 kolom bacaan terisi) harus diisi sebelum menyimpan.'
    return false
  }
...
async function onSimpan(): Promise<void> {
  ...
  try {
    await depricarpingRecordRepo.saveDraft(
      recordId,
      { ...form },
      [...detailRows.value],
      [...pendingDeletionIds.value],
    ...
    await syncAfterSave('depricarping_record', recordId)
    ...
    actionErrorMessage.value = err instanceof Error ? err.message : 'Gagal menyimpan data depricarping.'
  } finally {
    saving.value = false
  }
}
...
async function onPause(): Promise<void> {
  ...
  try {
    await depricarpingRecordRepo.pauseDraftWithFormData(
      recordId,
      { ...form },
      [...detailRows.value],
      [...pendingDeletionIds.value],
...
function goToMonitorDepricarping(): void {
  router.push({ name: 'monitor-depricarping' })
}
</script>

<template>
  <main class="form-depricarping-view">
    ...
    <div class="form-depricarping-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <button type="button" class="breadcrumb-link" data-testid="breadcrumb-depricarping" @click="goToMonitorDepricarping">
          Depricarping
        </button>
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
    ...
    <form v-else class="form-body" novalidate @submit.prevent="onSimpan">
      ...
      <section class="form-section" aria-label="Identitas Depricarping">
        <h2 class="form-section-title">Identitas Depricarping</h2>
      ...
      <section class="form-section" aria-label="Depricarping Detail">
        <h2 class="form-section-title">Depricarping Detail</h2>
        ...
        <div
          ...
          class="detail-row"
          data-testid="depricarping-detail-row"
        >
          <div class="form-field">
          ...
          <FormField
            v-model="row.fan_static_pressure_mmh2o"
            label="Fan Static Pressure (mmH2O)"
            type="number"
            :id="`fan-static-pressure-${index}`"
            :data-testid="`fan-static-pressure-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.polishing_drum_speed_rpm"
            label="Polishing Drum Speed (RPM)"
            type="number"
            :id="`polishing-drum-speed-${index}`"
            :data-testid="`polishing-drum-speed-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.air_velocity_ms"
            label="Air Velocity (m/s)"
            type="number"
            :id="`air-velocity-${index}`"
            :data-testid="`air-velocity-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.fibre_moisture_percent"
            label="Fibre Moisture (%)"
            type="number"
            :id="`fibre-moisture-${index}`"
            :data-testid="`fibre-moisture-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
          ...
          />
          <FormField
            v-model="row.nut_silo_1_temp_c"
            label="Nut Silo 1 Temp (°C)"
            type="number"
            :id="`nut-silo-1-temp-${index}`"
            :data-testid="`nut-silo-1-temp-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.nut_silo_2_temp_c"
            label="Nut Silo 2 Temp (°C)"
            type="number"
            :id="`nut-silo-2-temp-${index}`"
            :data-testid="`nut-silo-2-temp-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
      ...
      <CollapsibleSection title="Target Operasional">
        ...
        <table class="operational-target-table" data-testid="operational-target-table">
          ...
          <tbody>
            <tr v-for="target in DEPRICARPING_OPERATIONAL_TARGETS" :key="target.parameter_metric">
    ...
    <ConfirmDialog
      ...
      title="Hapus Draft"
      message="Draft depricarping ini akan dihapus secara permanen dan tidak dapat dikembalikan. Lanjutkan?"
