...
/**
 * DataPreviewThreshingView — screen-045--data-preview-threshing /
 * usecase-045--data-preview-threshing (mounted at
 * /stations/threshing/preview/:id? — `:id` is OPTIONAL; meta.public =
 ...
 *
 * Dual-mode, mirroring DataPreviewCagesTrackView.vue exactly:
 *
 *  - LIST mode — route has NO `:id` param. Shows every local
 *    threshing_record row for the current user, ANY status, with a
 *    client-side date filter (defaults to today) + search filter (on
 *    thresher_id). Tapping a row navigates onward:
 *      - draft_ongoing/draft_paused rows -> `threshing-form`.
 ...
 *    (full header + Threshing Detail grid, however many rows the user
 *    added — no longer always 24, see threshingRecordRepo.ts's header
 *    comment — + Target Operasional reference table), reusing
 *    `getDraftWithDetails()` (already exported by threshingRecordRepo.ts
 *    for screen-041). Renders Checked By and Acknowledged By read-only —
 ...
 * (same as DataPreviewCagesTrackView.vue's DETAIL_STATUS_BADGE_MAP).
 *
 * Date filter: `threshing_record.date` is a full local datetime string
 * (`YYYY-MM-DDTHH:mm:ss`, set by FormThreshingView.vue's
 * `nowLocalDateTimeString()`), so this filter compares against
 * `item.date.slice(0, 10)`.
 *
 * Search filter: case-insensitive substring match against `thresher_id`.
 */
...
const allRecords = ref<ThreshingRecord[]>([])
...
function recordLabel(item: ThreshingRecord): string {
  return item.thresher_id?.trim() ? item.thresher_id : 'Thresher ID belum diisi'
}

const LIST_STATUS_BADGE_MAP: Record<ThreshingRecord['status'], { status: BadgeStatus; label: string }> = {
...
function listBadgeInfo(item: ThreshingRecord) {
...
const filteredRecords = computed(() => {
  ...
  return allRecords.value.filter((item) => {
    ...
    if (keyword) {
      const thresherIdMatch = (item.thresher_id ?? '').toLowerCase().includes(keyword)
      if (!thresherIdMatch) {
        return false
      }
    }
...
function onItemClick(item: ThreshingRecord): void {
  if (item.status === 'draft_ongoing' || item.status === 'draft_paused') {
    router.push({ name: 'threshing-form', params: { id: item.id } })
    return
  }

  router.push({ name: 'data-preview-threshing', params: { id: item.id } })
}
...
const DETAIL_STATUS_BADGE_MAP: Record<ThreshingRecord['status'], { status: BadgeStatus; label: string }> = {
...
const screenTitle = computed(() => {
  ...
  return detailRecord.value?.thresher_id ? `Threshing ${detailRecord.value.thresher_id}` : 'Detail Threshing'
})
...
<template>
  <main class="data-preview-threshing-view">
    ...
    <div class="preview-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <button
          type="button"
          class="breadcrumb-link"
          data-testid="breadcrumb-threshing"
          @click="goToMonitorThreshing"
        >
          Threshing
        </button>
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
      ...
      <section v-else class="record-list-section" aria-label="Daftar Data Threshing">
        <div v-if="filteredRecords.length === 0" class="empty-state" data-testid="record-list-empty">
          <p class="empty-state-text">
            {{ hasActiveFilter ? 'Tidak ada data yang cocok dengan filter.' : 'Belum ada data threshing.' }}
          </p>
          <button
    ...
    <template v-else>
      ...
      >
        Data threshing tidak ditemukan.
      </p>
      ...
      <div v-else-if="detailRecord" class="preview-body">
        ...
        <section class="detail-rows-section" aria-label="Threshing Detail">
          <h2 class="detail-rows-title">Threshing Detail</h2>

          <p v-if="detailRows.length === 0" class="status-text" data-testid="threshing-detail-rows-empty">
            Belum ada baris threshing detail.
          </p>

          <ul v-else class="detail-rows-list" data-testid="threshing-detail-rows-list">
            <li v-for="row in detailRows" :key="row.id" class="detail-row-view" :data-testid="`threshing-detail-row-${row.id}`">
              <div class="detail-row-view-header">
                <span class="detail-row-view-time-slot">{{ row.time_slot }}</span>
              </div>
              <div class="detail-row-view-grid">
                <span>FFB Throughput: {{ row.ffb_throughput_mt_hour ?? '-' }}</span>
                <span>Drum Speed: {{ row.thresher_drum_speed_rpm ?? '-' }}</span>
                <span>Motor Current: {{ row.motor_current_amps ?? '-' }}</span>
                <span>Unstripped Bunch: {{ row.unstripped_bunch_count_percent ?? '-' }}</span>
                <span>Oil Loss: {{ row.empty_bunch_oil_loss_percent ?? '-' }}</span>
