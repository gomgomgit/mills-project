...
/**
 * DataPreviewCpoDispatchView — screen-084--data-preview-cpo-dispatch
 * / usecase-081--data-preview-cpo-dispatch (mounted at
 * /stations/cpo-dispatch/preview/:id? — `:id` OPTIONAL, meta.public =
 ...
 *
 * Mirrors DataPreviewKernelDispatchView.vue's dual route-driven mode
 * pattern:
 *  - LIST mode (no `:id`) — every local record for the current user, any
 *    status, client-side date + search (cpo_dispatch_id) filter.
...
const allRecords = ref<CpoDispatchRecord[]>([])
...
async function loadList(): Promise<void> {
  ...
  try {
    allRecords.value = await cpoDispatchRecordRepo.getAllRecords(userId)
  } catch (err) {
    listError.value = err instanceof Error ? err.message : 'Gagal memuat daftar data cpo dispatch lokal.'
  } finally {
    listLoading.value = false
  }
}

function recordLabel(item: CpoDispatchRecord): string {
  return item.cpo_dispatch_id?.trim() ? item.cpo_dispatch_id : 'CPO Dispatch ID belum diisi'
}

const LIST_STATUS_BADGE_MAP: Record<CpoDispatchRecord['status'], { status: BadgeStatus; label: string }> = {
...
function listBadgeInfo(item: CpoDispatchRecord) {
...
const filteredRecords = computed(() => {
  ...
  return allRecords.value.filter((item) => {
    ...
    if (keyword) {
      const idMatch = (item.cpo_dispatch_id ?? '').toLowerCase().includes(keyword)
      if (!idMatch) {
        return false
      }
    }
...
function onItemClick(item: CpoDispatchRecord): void {
  if (item.status === 'draft_ongoing' || item.status === 'draft_paused') {
    router.push({ name: 'cpo-dispatch-form', params: { id: item.id } })
    return
  }

  router.push({ name: 'data-preview-cpo-dispatch', params: { id: item.id } })
}
...
async function loadDetail(recordId: string): Promise<void> {
  ...
  try {
    ...
    detailLoadErrorMessage.value = err instanceof Error ? err.message : 'Gagal memuat data cpo dispatch.'
  } finally {
...
const DETAIL_STATUS_BADGE_MAP: Record<CpoDispatchRecord['status'], { status: BadgeStatus; label: string }> = {
...
<template>
  <main class="preview-cd-view">
    ...
    <div class="preview-cd-header">
      ...
      <h1 class="screen-title">Data Preview CPO Dispatch</h1>
    </div>
    ...
    <template v-if="!isDetailMode">
      <div class="filter-bar">
        ...
        <label class="filter-field">
          <span>Cari (CPO Dispatch ID)</span>
    ...
    <template v-else>
      ...
      <div v-else-if="detailRecord" class="detail-body">
        <section class="detail-section">
          ...
          <p data-testid="detail-cpo-dispatch-id"><strong>CPO Dispatch ID:</strong> {{ detailRecord.cpo_dispatch_id || '-' }}</p>
        ...
        <section class="detail-section">
          ...
          <div v-else class="detail-table-wrap" data-testid="cpo-dispatch-detail-log">
    ...
    <RecordVerificationActions
      station-type="cpo-dispatch"
      local-table="cpo_dispatch_record"
      :record="detailRecord"
...
<style scoped>
.preview-cd-view { min-height: 100vh; display: flex; flex-direction: column; gap: 16px; padding: 0 16px 20px; background: #ffffff; font-family: 'Inter', sans-serif; box-sizing: border-box; }
...
.preview-cd-header { display: flex; flex-direction: column; gap: 6px; }
