...
/**
 * DataPreviewDepricarpingView — screen-047--data-preview-depricarping /
 * usecase-047--data-preview-depricarping (mounted at
 * /stations/depricarping/preview/:id? — `:id` is OPTIONAL; meta.public =
 ...
 *
 * Dual-mode, mirroring DataPreviewPressingView.vue (screen-046) exactly:
 *
 *  - LIST mode — route has NO `:id` param. Shows every local
 *    depricarping_record row for the current user, ANY status, with a
 *    client-side date filter (defaults to today) + search filter (on
 *    presser_id). Tapping a row navigates onward:
 *      - draft_ongoing/draft_paused rows -> `depricarping-form`.
 ...
 *    (full header + Depricarping Detail grid, however many rows the user
 *    added — no longer always 24, see depricarpingRecordRepo.ts's header
 *    comment — + Target Operasional reference table), reusing
 *    `getDraftWithDetails()` (already exported by depricarpingRecordRepo.ts
 *    for screen-043). Renders Checked By and Acknowledged By read-only —
 ...
 * (same as DataPreviewPressingView.vue's DETAIL_STATUS_BADGE_MAP).
 *
 * Date filter: `depricarping_record.date` is a full local datetime string
 * (`YYYY-MM-DDTHH:mm:ss`, set by FormDepricarpingView.vue's
...
const allRecords = ref<DepricarpingRecord[]>([])
...
function recordLabel(item: DepricarpingRecord): string {
  return item.presser_id?.trim() ? item.presser_id : 'Presser ID belum diisi'
}

const LIST_STATUS_BADGE_MAP: Record<DepricarpingRecord['status'], { status: BadgeStatus; label: string }> = {
...
function listBadgeInfo(item: DepricarpingRecord) {
...
function onItemClick(item: DepricarpingRecord): void {
  if (item.status === 'draft_ongoing' || item.status === 'draft_paused') {
    router.push({ name: 'depricarping-form', params: { id: item.id } })
    return
  }

  router.push({ name: 'data-preview-depricarping', params: { id: item.id } })
}
...
const DETAIL_STATUS_BADGE_MAP: Record<DepricarpingRecord['status'], { status: BadgeStatus; label: string }> = {
...
const screenTitle = computed(() => {
  ...
  return detailRecord.value?.presser_id ? `Depricarping ${detailRecord.value.presser_id}` : 'Detail Depricarping'
})
...
<template>
  <main class="data-preview-depricarping-view">
    ...
    <div class="preview-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <button
          type="button"
          class="breadcrumb-link"
          data-testid="breadcrumb-depricarping"
          @click="goToMonitorDepricarping"
        >
          Depricarping
        </button>
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
      ...
      <section v-else class="record-list-section" aria-label="Daftar Data Depricarping">
        <div v-if="filteredRecords.length === 0" class="empty-state" data-testid="record-list-empty">
          <p class="empty-state-text">
            {{ hasActiveFilter ? 'Tidak ada data yang cocok dengan filter.' : 'Belum ada data depricarping.' }}
          </p>
          <button
    ...
    <template v-else>
      ...
      >
        Data depricarping tidak ditemukan.
      </p>
      ...
      <div v-else-if="detailRecord" class="preview-body">
        ...
        <section class="detail-rows-section" aria-label="Depricarping Detail">
          <h2 class="detail-rows-title">Depricarping Detail</h2>

          <p v-if="detailRows.length === 0" class="status-text" data-testid="depricarping-detail-rows-empty">
            Belum ada baris depricarping detail.
          </p>

          <ul v-else class="detail-rows-list" data-testid="depricarping-detail-rows-list">
            <li
              v-for="row in detailRows"
              :key="row.id"
              class="detail-row-view"
              :data-testid="`depricarping-detail-row-${row.id}`"
            >
              <div class="detail-row-view-header">
                <span class="detail-row-view-time-slot">{{ row.time_slot }}</span>
              </div>
              <div class="detail-row-view-grid">
                <span>Fan Static Pressure: {{ row.fan_static_pressure_mmh2o ?? '-' }}</span>
