...
/**
 * DataPreviewBoilerRoomView — screen-088--data-preview-boiler-room /
 * usecase-105--data-preview-boiler-room (mounted at
 * /stations/boiler-room/preview/:id? — `:id` is OPTIONAL; meta.public =
 ...
 *
 * Dual-mode, mirroring DataPreviewEngineRoomView.vue exactly:
 *
 *  - LIST mode — route has NO `:id` param. Shows every local
 *    boiler_room_record row for the current user, ANY status, with a
 *    client-side date filter (defaults to today) + search filter (on
 *    boiler_room_id). Tapping a row navigates onward:
 *      - draft_ongoing/draft_paused rows -> `boiler-room-form`.
 ...
 *    (full header + Boiler Room Detail grid, however many rows the user
 *    added), reusing `getDraftWithDetails()` (already exported by
 *    boilerRoomRecordRepo.ts for screen-078). Renders Checked By and
 ...
 *    (an empty-state message, `boiler-room-detail-rows-empty`).
 *
 ...
 * DataPreviewEngineRoomView.vue's header comment (Vue Router reuses this
 * component instance for param-only navigations on the same route
 * record).
 *
 * Header/breadcrumb/nav-menu: copied verbatim from
 * DataPreviewEngineRoomView.vue. Breadcrumb is 4 segments deep (Home >
 * Production Process Activity > Boiler Room > Load Data).
 *
 * List-row status badge (StatusBadge.vue):
 *  - draft_ongoing / draft_paused -> status="paused" label="Pause" (matches
 *    Monitor Boiler Room's list exactly).
 ...
 * (same as DataPreviewEngineRoomView.vue's DETAIL_STATUS_BADGE_MAP).
 *
 * Date filter: `boiler_room_record.date` is a full local datetime string
 * (`YYYY-MM-DDTHH:mm:ss`, set by FormBoilerRoomView.vue's
...
const allRecords = ref<BoilerRoomRecord[]>([])
...
function recordLabel(item: BoilerRoomRecord): string {
  return item.boiler_room_id?.trim() ? item.boiler_room_id : 'Boiler Room ID belum diisi'
}

const LIST_STATUS_BADGE_MAP: Record<BoilerRoomRecord['status'], { status: BadgeStatus; label: string }> = {
...
function listBadgeInfo(item: BoilerRoomRecord) {
...
const filteredRecords = computed(() => {
  ...
  return allRecords.value.filter((item) => {
    ...
    if (keyword) {
      const boilerRoomIdMatch = (item.boiler_room_id ?? '').toLowerCase().includes(keyword)
      if (!boilerRoomIdMatch) {
        return false
      }
    }
...
function onItemClick(item: BoilerRoomRecord): void {
  if (item.status === 'draft_ongoing' || item.status === 'draft_paused') {
    router.push({ name: 'boiler-room-form', params: { id: item.id } })
    return
  }

  router.push({ name: 'data-preview-boiler-room', params: { id: item.id } })
}
...
const DETAIL_STATUS_BADGE_MAP: Record<BoilerRoomRecord['status'], { status: BadgeStatus; label: string }> = {
...
const screenTitle = computed(() => {
  ...
  return detailRecord.value?.boiler_room_id ? `Boiler Room ${detailRecord.value.boiler_room_id}` : 'Detail Boiler Room'
})

function onBack(): void {
  if (isDetailMode.value) {
    router.push({ name: 'data-preview-boiler-room' })
    return
  }
...
<template>
  <main class="data-preview-boiler-room-view">
    ...
    <div class="preview-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <button
          type="button"
          class="breadcrumb-link"
          data-testid="breadcrumb-boiler-room"
          @click="goToMonitorBoilerRoom"
        >
          Boiler Room
        </button>
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
      ...
      <div class="filter-row">
        ...
        <label class="filter-field">
          ...
          <input
            ...
            type="text"
            class="filter-input"
            placeholder="Boiler Room ID"
            data-testid="search-filter-input"
          />
        </label>
      </div>

      <p v-if="listLoading" class="status-text">Memuat daftar data boiler room lokal…</p>
      <p v-else-if="listError" class="status-text status-text--error" role="alert">{{ listError }}</p>

      <section v-else class="record-list-section" aria-label="Daftar Data Boiler Room">
        <div v-if="filteredRecords.length === 0" class="empty-state" data-testid="record-list-empty">
          <p class="empty-state-text">
            {{ hasActiveFilter ? 'Tidak ada data yang cocok dengan filter.' : 'Belum ada data boiler room.' }}
          </p>
          <button
    ...
    <!-- DETAIL mode -->
    <template v-else>
      <p v-if="detailLoading" class="status-text">Memuat data boiler room…</p>
      <p
      ...
      >
        Data boiler room tidak ditemukan.
      </p>
      ...
      <div v-else-if="detailRecord" class="preview-body">
        ...
        <section class="detail-rows-section" aria-label="Boiler Room Detail">
          <h2 class="detail-rows-title">Boiler Room Detail</h2>

          <p v-if="detailRows.length === 0" class="status-text" data-testid="boiler-room-detail-rows-empty">
            Belum ada baris boiler room detail.
          </p>

          <ul v-else class="detail-rows-list" data-testid="boiler-room-detail-rows-list">
            <li v-for="row in detailRows" :key="row.id" class="detail-row-view" :data-testid="`boiler-room-detail-row-${row.id}`">
              <div class="detail-row-view-header">
                <span class="detail-row-view-time-slot">{{ row.time_slot }}</span>
              </div>
              <div class="detail-row-view-grid">
                <span>Steam Pressure (bar): {{ row.steam_pressure_bar ?? '-' }}</span>
                <span>Steam Temp (°C): {{ row.steam_temp_c ?? '-' }}</span>
                <span>Feed Water Temp (°C): {{ row.feed_water_temp_c ?? '-' }}</span>
                <span>Feed Water Tank Level (%): {{ row.feed_water_tank_level_percent ?? '-' }}</span>
                <span>Boiler Water Level (%): {{ row.boiler_water_level_percent ?? '-' }}</span>
                <span>Water TDS (ppm): {{ row.water_tds_ppm ?? '-' }}</span>
                <span>Water pH: {{ row.water_ph ?? '-' }}</span>
                <span>Fuel Feed Rate: {{ row.fuel_feed_rate ?? '-' }}</span>
                <span>ID Fan Load: {{ row.id_fan_load ?? '-' }}</span>
