...
/**
 * DataPreviewEffluentPlantView — screen-085--data-preview-effluent-plant /
 * usecase-087--data-preview-effluent-plant (mounted at
 * /stations/effluent-plant/preview/:id? — `:id` is OPTIONAL; meta.public =
 ...
 *    effluent_plant_record row for the current user, ANY status, with a
 *    client-side date filter (defaults to today) + search filter (on
 *    effluent_plant_id). Tapping a row navigates onward:
 *      - draft_ongoing/draft_paused rows -> `effluent-plant-form`.
 ...
 *    (full header + Effluent Plant Detail grid, however many rows the
 *    user added), reusing `getDraftWithDetails()` (already exported by
 *    effluentPlantRecordRepo.ts for screen-072). Renders Checked By and
 ...
 *    `effluent-plant-detail-rows-empty`).
 *
 ...
 *
 * Date filter: `effluent_plant_record.date` is a full local datetime
 * string (`YYYY-MM-DDTHH:mm:ss`, set by FormEffluentPlantView.vue's
...
const allRecords = ref<EffluentPlantRecord[]>([])
...
async function loadList(): Promise<void> {
  ...
  try {
    allRecords.value = await effluentPlantRecordRepo.getAllRecords(userId)
  } catch (err) {
    listError.value = err instanceof Error ? err.message : 'Gagal memuat daftar data effluent plant lokal.'
  } finally {
    listLoading.value = false
  }
}

function recordLabel(item: EffluentPlantRecord): string {
  return item.effluent_plant_id?.trim() ? item.effluent_plant_id : 'Effluent Plant ID belum diisi'
}

const LIST_STATUS_BADGE_MAP: Record<EffluentPlantRecord['status'], { status: BadgeStatus; label: string }> = {
...
function listBadgeInfo(item: EffluentPlantRecord) {
...
const filteredRecords = computed(() => {
  ...
  return allRecords.value.filter((item) => {
    ...
    if (keyword) {
      const effluentPlantIdMatch = (item.effluent_plant_id ?? '').toLowerCase().includes(keyword)
      if (!effluentPlantIdMatch) {
        return false
      }
    }
...
function onItemClick(item: EffluentPlantRecord): void {
  if (item.status === 'draft_ongoing' || item.status === 'draft_paused') {
    router.push({ name: 'effluent-plant-form', params: { id: item.id } })
    return
  }

  router.push({ name: 'data-preview-effluent-plant', params: { id: item.id } })
}
...
const detailRecord = ref<EffluentPlantRecord | null>(null)
const detailRows = ref<EffluentPlantDetailRow[]>([])
...
async function loadDetail(recordId: string): Promise<void> {
  ...
  try {
    ...
    detailLoadErrorMessage.value = err instanceof Error ? err.message : 'Gagal memuat data effluent plant.'
  } finally {
...
const DETAIL_STATUS_BADGE_MAP: Record<EffluentPlantRecord['status'], { status: BadgeStatus; label: string }> = {
...
const screenTitle = computed(() => {
  ...
  return detailRecord.value?.effluent_plant_id ? `Effluent Plant ${detailRecord.value.effluent_plant_id}` : 'Detail Effluent Plant'
})
...
<template>
  <main class="data-preview-effluent-plant-view">
    ...
    <div class="preview-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <button
          type="button"
          class="breadcrumb-link"
          data-testid="breadcrumb-effluent-plant"
          @click="goToMonitorEffluentPlant"
        >
          Effluent Plant
        </button>
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
      ...
      <div class="filter-row">
        ...
        <label class="filter-field">
          ...
          <input
            ...
            type="text"
            class="filter-input"
            placeholder="Effluent Plant ID"
            data-testid="search-filter-input"
          />
        </label>
      </div>

      <p v-if="listLoading" class="status-text">Memuat daftar data effluent plant lokal…</p>
      <p v-else-if="listError" class="status-text status-text--error" role="alert">{{ listError }}</p>

      <section v-else class="record-list-section" aria-label="Daftar Data Effluent Plant">
        <div v-if="filteredRecords.length === 0" class="empty-state" data-testid="record-list-empty">
          <p class="empty-state-text">
            {{ hasActiveFilter ? 'Tidak ada data yang cocok dengan filter.' : 'Belum ada data effluent plant.' }}
          </p>
          <button
    ...
    <!-- DETAIL mode -->
    <template v-else>
      <p v-if="detailLoading" class="status-text">Memuat data effluent plant…</p>
      <p
      ...
      >
        Data effluent plant tidak ditemukan.
      </p>
      ...
      <div v-else-if="detailRecord" class="preview-body">
        <FormField :model-value="detailRecord.effluent_plant_id" label="Effluent Plant ID" disabled />
        ...
        <section class="detail-rows-section" aria-label="Effluent Plant Detail">
          <h2 class="detail-rows-title">Effluent Plant Detail</h2>

          <p v-if="detailRows.length === 0" class="status-text" data-testid="effluent-plant-detail-rows-empty">
            Belum ada baris effluent plant detail.
          </p>

          <ul v-else class="detail-rows-list" data-testid="effluent-plant-detail-rows-list">
            <li v-for="row in detailRows" :key="row.id" class="detail-row-view" :data-testid="`effluent-plant-detail-row-${row.id}`">
              <div class="detail-row-view-header">
                <span class="detail-row-view-time-slot">{{ row.time_slot }}</span>
              </div>
              <div class="detail-row-view-grid">
                <span>Anaerobic Pond 1 pH: {{ row.anaerobic_pond_1_ph ?? '-' }}</span>
                <span>Anaerobic Pond 1 Temp: {{ row.anaerobic_pond_1_temp_c ?? '-' }}</span>
                <span>Anaerobic Pond 2 pH: {{ row.anaerobic_pond_2_ph ?? '-' }}</span>
                ...
                <span>Final Discharge pH: {{ row.final_discharge_ph ?? '-' }}</span>
...
<style scoped>
.data-preview-effluent-plant-view {
  min-height: 100vh;
  display: flex;
