...
/**
 * MonitorKernelDispatchView — screen-063--monitor-kernel-dispatch /
 * usecase-073--monitor-kernel-dispatch (mounted at
 * /stations/kernel-dispatch/monitor, meta.public = false). Actors:
 * operator, supervisor.
 *
 ...
 *
 * Mirrors MonitorSolidWasteDisposalView.vue's structural pattern
 ...
 * metric: "Hari Ini" shows Jumlah Kernel Dispatch (COUNT of today's
...
import {
  ...
  type KernelDispatchLastEvent,
  type KernelDispatchTodaySummary,
...
const drafts = ref<KernelDispatchDraftListItem[]>([])
const loading = ref(false)
const error = ref<string | null>(null)

const todaySummary = ref<KernelDispatchTodaySummary>({ countRecords: 0, countEvents: 0 })
const lastEvent = ref<KernelDispatchLastEvent | null>(null)
...
async function loadDrafts() {
  ...
  try {
    drafts.value = await kernelDispatchRecordRepo.getDrafts(userId)
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal memuat daftar draft kernel dispatch lokal.'
  } finally {
    loading.value = false
  }
}
...
async function loadTodaySummary() {
  ...
  try {
    todaySummary.value = await kernelDispatchRecordRepo.getTodaySummary(userId)
  } catch {
...
async function loadLastEvent() {
  ...
  try {
    lastEvent.value = await kernelDispatchRecordRepo.getLastEventSummary(userId)
  } catch {
...
function draftLabel(draft: KernelDispatchDraftListItem): string {
  return draft.kernel_dispatch_id?.trim() ? draft.kernel_dispatch_id : 'Kernel Dispatch ID belum diisi'
}
...
async function onNewData() {
  ...
  try {
    const id = await kernelDispatchRecordRepo.createDraft(userId)
    router.push({ name: 'kernel-dispatch-form', params: { id } })
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal membuat draft kernel dispatch baru.'
  }
}

function onDraftClick(draft: KernelDispatchDraftListItem) {
  router.push({ name: 'kernel-dispatch-form', params: { id: draft.id } })
}
  ...
  <main class="monitor-kd-view">
    ...
    <div class="monitor-kd-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span aria-hidden="true">/</span>
        <span aria-current="page">Kernel Dispatch</span>
      </nav>
      <h1 class="screen-title">Monitor Kernel Dispatch</h1>
    </div>
    ...
    <section class="today-summary-section" aria-label="Hari Ini">
      ...
      <div class="today-summary-grid">
        <div class="today-summary-card">
          <span class="today-summary-label">Jumlah Kernel Dispatch</span>
    ...
    <section class="last-event-section" aria-label="Kejadian Terakhir" data-testid="last-event-card">
      ...
      <div v-else class="last-event-body">
        ...
        <span>{{ lastEvent.vehiclePlateNo ?? '-' }}</span>
      </div>
    </section>

    <p v-if="loading" class="status-text">Memuat daftar draft kernel dispatch lokal…</p>
    <p v-else-if="error" class="status-text status-text--error" role="alert">{{ error }}</p>

    <section v-else class="draft-list-section" aria-label="Daftar Draft Kernel Dispatch">
      <p v-if="drafts.length === 0" class="empty-state" data-testid="draft-list-empty">
        Belum ada draft kernel dispatch tersimpan.
      </p>
...
<style scoped>
.monitor-kd-view { min-height: 100vh; display: flex; flex-direction: column; gap: 20px; padding: 0 20px 20px; background: #ffffff; font-family: 'Inter', sans-serif; box-sizing: border-box; }
...
.monitor-kd-header { display: flex; flex-direction: column; gap: 6px; }
