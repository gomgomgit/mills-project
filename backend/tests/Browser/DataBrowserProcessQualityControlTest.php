...
/**
 * DataBrowserProcessQualityControlTest (Browser/Playwright) —
 * screen-100--data-browser-process-quality-control-web /
 * usecase-118--data-browser-process-quality-control-web.
 *
 * Playwright spec, mirrors tests/Browser/DataBrowserClarificationTest.php's
 * conventions exactly.
 *
 ...
 * (pqctest-admin01 / Passw0rd!), a business unit with at least one
 * Process Quality Control record dated within 2026-08-01..2026-08-15.
 */
...
test.describe('Data Browser Process Quality Control (Web)', () => {
  // Scenario: "success"
  test('apply date range and business unit filters, click export and choose format', async ({ page }) => {
    await login(page, 'pqctest-admin01', PASSWORD);
    await page.goto(`${BASE_URL}/data/process-quality-control`);
    ...
    expect(download.suggestedFilename()).toContain('process-quality-control-records');
  });
  ...
  test('apply filters with no matching data, empty state shown', async ({ page }) => {
    ...
    await expect(page.locator('.pqc-empty__title')).toContainText('Tidak ada data');
  });
  ...
  test('start date later than end date, validation message displayed', async ({ page }) => {
    ...
    await expect(page.locator('.pqc-alert')).toContainText('Rentang tanggal tidak valid');
  });

  // Scenario: "Klik Baris Membuka Detail"
  test('click a table row, browser navigates to Detail Process Quality Control', async ({ page }) => {
    await login(page, 'pqctest-admin01', PASSWORD);
    await page.goto(`${BASE_URL}/data/process-quality-control`);
