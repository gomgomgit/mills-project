...
/**
 * DataBrowserCagesTrackTest (Feature/Livewire) —
 * screen-018--data-browser-cages-track-web / usecase-018--data-browser-
 * cages-track-web.
 *
 * Component tests for App\Livewire\Data\DataBrowserCagesTrack, one per
 * test_scenarios' component_test step (scenarios 1-5). Mirrors tests/
 * Feature/Livewire/DataBrowserGradingTest.php's structure/conventions
 * exactly, adapted to cages-track fields.
 *
 ...
 * event — there is nothing to ->call() for it, and screen-021's
 * `data.cages-track.detail` route does not exist yet); it instead asserts
 * the guarded rendering behavior: since Route::has('data.cages-track.
 * detail') is false in this test env, rows render with the
 * `ct-table__row--static` class and no `onclick=` attribute — see the
 * Blade view's `$hasDetailRoute` check.
 *
 ...
 * Livewire/DataBrowserGradingTest.php.
 *
 * Known implementation gaps documented (not asserted as failures — see
 * this screen's known_issues, mirroring screen-017's documented gaps):
 ...
 *     DataBrowserCagesTrack::exportUrl()'s docblock) — a real export
...
it('success: shows filtered rows and export links after setting filter properties', function () {
    $recordWithTippedTimes = CagesTrackRecord::factory()
        ...
        ->onDate('2026-08-05')
        ->create();
    CagesTippedTime::factory()->forRecord($recordWithTippedTimes)->count(3)->create();
    ...
    Livewire::actingAs($this->user)
        ->test(DataBrowserCagesTrack::class)
        ->set('date_from', '2026-08-01')
        ->set('date_to', '2026-08-15')
        ->set('business_unit_id', $this->businessUnit->id)
        ->assertSet('errorMessage', null)
        ->assertViewHas('records', function ($records) use ($recordWithTippedTimes) {
            $byId = collect($records)->keyBy('id');

            return count($records) === 2
                && $byId[$recordWithTippedTimes->id]['tipped_time_count'] === 3;
        })
...
it('Ekspor Gagal: export links are always built from the current filters (no client-side size guard)', function () {
    Livewire::actingAs($this->user)
        ...
        ->assertViewHas('exportCsvUrl', fn ($url) => str_contains($url, '/api/cages-track-records/export')
    ...
    // DataBrowserCagesTrackTest.php's "Ekspor Gagal" scenario) — the
...
// (row-click navigation itself is client-side and screen-021's
// `data.cages-track.detail` route does not exist yet, so navigation is
// NOT asserted here — only the guarded rendering behavior is: since
// Route::has('data.cages-track.detail') is false in this test env, rows
// must render WITHOUT the clickable-row affordance, i.e. with the
// `ct-table__row--static` class and no `onclick=` attribute.)
it('Klik Baris Membuka Detail: rows render without navigation since the detail route does not exist yet', function () {
    ...
    Livewire::actingAs($this->user)
        ->test(DataBrowserCagesTrack::class)
        ->assertSeeHtml('ct-table__row--static')
