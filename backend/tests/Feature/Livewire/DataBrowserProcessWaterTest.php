...
/**
 * DataBrowserProcessWaterTest (Feature/Livewire) —
 * screen-092--data-browser-process-water-web /
 * usecase-070--data-browser-process-water-web.
 *
 * Component tests for App\Livewire\Data\DataBrowserProcessWater, mirroring
...
it('success: shows filtered rows and export links after setting filter properties', function () {
    $recordWithFilledRows = ProcessWaterRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();
    ProcessWaterDetail::factory()->forRecord($recordWithFilledRows)->timeSlot('07:00')->filled()->create();
    ProcessWaterDetail::factory()->forRecord($recordWithFilledRows)->timeSlot('08:00')->filled()->create();

    ProcessWaterRecord::factory()->forStation($this->station)->onDate('2026-08-06')->create();
    ProcessWaterRecord::factory()->forStation($this->station)->onDate('2026-09-01')->create();

    Livewire::actingAs($this->user)
        ->test(DataBrowserProcessWater::class)
...
it('Ekspor Gagal: export links are always built from the current filters', function () {
    Livewire::actingAs($this->user)
        ...
        ->assertViewHas('exportCsvUrl', fn ($url) => str_contains($url, '/api/process-water-records/export')
...
it('Klik Baris Membuka Detail: rows render with a clickable-row link to the real detail route', function () {
    $record = ProcessWaterRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();

    Livewire::actingAs($this->user)
        ->test(DataBrowserProcessWater::class)
        ->assertSeeHtml("onclick=\"window.location.href='".route('data.process-water.detail', ['id' => $record->id])."'\"");
});
