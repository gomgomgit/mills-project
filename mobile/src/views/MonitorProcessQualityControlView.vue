...
/**
 * MonitorProcessQualityControlView — screen-070--monitor-process-quality-control /
 * usecase-115--monitor-process-quality-control (mounted at
 * /stations/process-quality-control/monitor, meta.public = false — requires an
 ...
 * local (offline) `process_quality_control_record` table (and its child
 * `process_quality_control_detail` table, for the "filled slot" counter) via
 * processQualityControlRecordRepo.ts. api_contracts.endpoints is empty for
 * this screen.
 *
 * Structural pattern copied from MonitorClarificationView.vue exactly: a
 * "Hari Ini" 2-card counter row plus a full scrollable LIST of the current
 ...
 * deleteDraft() calls) — those actions live in Form Process Quality
 * Control (screen-080).
 *
 * Counter card labels per this screen's tech spec: "Jumlah Process Quality
 * Control Record" (COUNT of today's process_quality_control_record rows,
 * any status) / "Jumlah Baris Time-Slot Tercatat" (COUNT of
 * process_quality_control_detail rows belonging to those records — row
 * EXISTENCE itself is the meaningful signal, a plain count of rows the
 * user has added, with no fixed-24 denominator).
 *
 * Header/breadcrumb/nav-menu: copied verbatim from
 * MonitorClarificationView.vue's isNavMenuOpen/toggleNavMenu/closeNavMenu/
 ...
 * (Home > Production Process Activity > Process Quality Control, the first
 * two tappable).
 *
 * Navigation:
 *  - 'New Data' -> processQualityControlRecordRepo.createDraft(userId)
 *    (INSERTs a new status='draft_ongoing' header row ONLY — no
 *    process_quality_control_detail rows are pre-created, see that
 *    function's doc comment), then -> route name
 *    `monitor-process-quality-control`'s sibling
 *    `process-quality-control-form` with the new draft's id.
 *  - Tapping a list item -> route name `process-quality-control-form` with
 *    that item's EXISTING id, directly — no status-transition call.
 *  - 'Load Data' -> route name `data-preview-process-quality-control` with
 *    NO id param (list mode).
...
import {
  ...
  type ProcessQualityControlDraftListItem,
  type ProcessQualityControlTodaySummary,
...
const drafts = ref<ProcessQualityControlDraftListItem[]>([])
...
// today.
const todaySummary = ref<ProcessQualityControlTodaySummary>({
  countProcessQualityControlRecord: 0,
  detailRowCount: 0,
})
...
async function loadDrafts() {
  ...
  try {
    drafts.value = await processQualityControlRecordRepo.getDrafts(userId)
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal memuat daftar draft process quality control lokal.'
  } finally {
    loading.value = false
  }
}
...
async function loadTodaySummary() {
  ...
  try {
    todaySummary.value = await processQualityControlRecordRepo.getTodaySummary(userId)
  } catch {
...
/**
 * Process QC ID fallback text for a draft whose `process_qc_id` has not
 * been filled in yet.
 */
function draftLabel(draft: ProcessQualityControlDraftListItem): string {
  return draft.process_qc_id?.trim() ? draft.process_qc_id : 'Process QC ID belum diisi'
}
...
async function onNewData() {
  ...
  try {
    const id = await processQualityControlRecordRepo.createDraft(userId)
    router.push({ name: 'process-quality-control-form', params: { id } })
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal membuat draft process quality control baru.'
  }
}
...
function onDraftClick(draft: ProcessQualityControlDraftListItem) {
  router.push({ name: 'process-quality-control-form', params: { id: draft.id } })
}
...
// business_logic step 8 — breadcrumb taps. 'Process Quality Control' is
// the current page (not a link — rendered as plain text with
// aria-current="page" in the template).
function goToHome() {
...
<template>
  <main class="monitor-process-quality-control-view">
    ...
    <div class="monitor-process-quality-control-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <span class="breadcrumb-current" aria-current="page">Process Quality Control</span>
      </nav>
      <h1 class="screen-title">Monitor Process Quality Control</h1>
    </div>
    ...
    <section class="today-summary-section" aria-label="Hari Ini">
      ...
      <div class="today-summary-grid">
        <div class="today-summary-card">
          <span class="today-summary-label">Jumlah Process Quality Control Record</span>
          <span class="today-summary-value" data-testid="counter-count-process-quality-control-record">{{ todaySummary.countProcessQualityControlRecord }}</span>
        </div>
        <div class="today-summary-card">
    ...
    <p v-if="loading" class="status-text">Memuat daftar draft process quality control lokal…</p>
    <p v-else-if="error" class="status-text status-text--error" role="alert">{{ error }}</p>

    <section v-else class="draft-list-section" aria-label="Daftar Draft Process Quality Control">
      <p v-if="drafts.length === 0" class="empty-state" data-testid="draft-list-empty">
        Belum ada draft process quality control tersimpan.
      </p>
...
<style scoped>
.monitor-process-quality-control-view {
  min-height: 100vh;
  display: flex;
...
.monitor-process-quality-control-header {
  display: flex;
  flex-direction: column;
  gap: 6px;
}
