/**
 * DataBrowserGradingTest (Browser/Playwright) — screen-017--data-browser-
 * grading-web / usecase-017--data-browser-grading-web.
 *
 * Playwright spec, one test per test_scenarios' browser_test step. Mirrors
 * tests/Browser/DataBrowserWeighbridgeTest.php's convention exactly (same
 ...
 * screen-016's precedent).
 *
 * Covers scenario 1 ("Telusuri & Ekspor Data Grading — berhasil") end-to-
 * end through the UI, per this screen's browser_test spec: apply a filter,
 * see the table update, click export, verify the download starts.
 *
 * Test data assumption (mirrors DataBrowserWeighbridgeTest.php's
 * approach): this screen requires an authenticated session with role
 * supervisor, mill_management, or admin, so the scenario logs in first via
 * /login, then navigates to /data/grading.
 *   - business unit: "Mill A" (any business_unit whose name is selectable
 *     in the login form's Business Area <select>)
 *   - grtest-browse01 / Passw0rd! (role: supervisor)
 ...
 * one GradingRecord for that business unit, dated within the filter range
 * used below (2026-01-01..2026-12-31), must exist in the seeded
...
test.describe('Data Browser Grading', () => {
  ...
  test('menerapkan filter, tabel diperbarui, dan ekspor CSV memicu unduhan', async ({ page }) => {
    ...
    await expect(page.locator('.gr-browser')).not.toHaveClass(/gr-browser--busy/);
    ...
    await expect(page.locator('.gr-table__row').first()).toBeVisible();
    ...
    // AJAX/blob call — see App\Livewire\Data\DataBrowserGrading's
    // exportUrl() docblock), so Playwright's download event fires on the
    // resulting navigation.
    const [download] = await Promise.all([
      page.waitForEvent('download'),
      page.locator('.gr-browser__export a', { hasText: 'Ekspor CSV' }).click(),
    ]);

    expect(download.suggestedFilename()).toMatch(/^grading-records_.*\.csv$/);
  });
});
