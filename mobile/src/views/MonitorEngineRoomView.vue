...
/**
 * MonitorEngineRoomView — screen-067--monitor-engine-room /
 * usecase-097--monitor-engine-room (mounted at
 * /stations/engine-room/monitor, meta.public = false — requires an
 ...
 * local (offline) `engine_room_record` table (and its child
 * `engine_room_detail` table, for the "filled slot" counter) via
 * engineRoomRecordRepo.ts. api_contracts.endpoints is empty for this
 * screen.
 *
 ...
 * deleteDraft() calls) — those actions live in Form Engine Room
 * (screen-077).
 *
 * Counter card labels per this screen's tech spec: "Jumlah Engine Room
 * Record" (COUNT of today's engine_room_record rows, any status) /
 * "Jumlah Baris Time-Slot Tercatat" (COUNT of engine_room_detail rows
 ...
 * (Home > Production Process Activity > Engine Room, the first two
 * tappable).
 *
 * Navigation:
 *  - 'New Data' -> engineRoomRecordRepo.createDraft(userId) (INSERTs a new
 *    status='draft_ongoing' header row ONLY — no engine_room_detail rows
 *    are pre-created, see that function's doc comment), then -> route name
 *    `monitor-engine-room`'s sibling `engine-room-form` with the new
 *    draft's id.
 *  - Tapping a list item -> route name `engine-room-form` with that item's
 *    EXISTING id, directly — no status-transition call.
 *  - 'Load Data' -> route name `data-preview-engine-room` with NO id param
 *    (list mode).
...
import {
  ...
  type EngineRoomDraftListItem,
  type EngineRoomTodaySummary,
...
const drafts = ref<EngineRoomDraftListItem[]>([])
...
// today.
const todaySummary = ref<EngineRoomTodaySummary>({ countEngineRoomRecord: 0, detailRowCount: 0 })
...
async function loadDrafts() {
  ...
  try {
    drafts.value = await engineRoomRecordRepo.getDrafts(userId)
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal memuat daftar draft engine room lokal.'
  } finally {
    loading.value = false
  }
}
...
async function loadTodaySummary() {
  ...
  try {
    todaySummary.value = await engineRoomRecordRepo.getTodaySummary(userId)
  } catch {
...
/**
 * Engine Room ID fallback text for a draft whose `engine_room_id` has not
 * been filled in yet.
 */
function draftLabel(draft: EngineRoomDraftListItem): string {
  return draft.engine_room_id?.trim() ? draft.engine_room_id : 'Engine Room ID belum diisi'
}
...
async function onNewData() {
  ...
  try {
    const id = await engineRoomRecordRepo.createDraft(userId)
    router.push({ name: 'engine-room-form', params: { id } })
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal membuat draft engine room baru.'
  }
}
...
function onDraftClick(draft: EngineRoomDraftListItem) {
  router.push({ name: 'engine-room-form', params: { id: draft.id } })
}
...
// business_logic step 8 — breadcrumb taps. 'Engine Room' is the current
...
<template>
  <main class="monitor-engine-room-view">
    ...
    <div class="monitor-engine-room-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <span class="breadcrumb-current" aria-current="page">Engine Room</span>
      </nav>
      <h1 class="screen-title">Monitor Engine Room</h1>
    </div>
    ...
    <section class="today-summary-section" aria-label="Hari Ini">
      ...
      <div class="today-summary-grid">
        <div class="today-summary-card">
          <span class="today-summary-label">Jumlah Engine Room Record</span>
          <span class="today-summary-value" data-testid="counter-count-engine-room-record">{{ todaySummary.countEngineRoomRecord }}</span>
        </div>
        <div class="today-summary-card">
    ...
    <p v-if="loading" class="status-text">Memuat daftar draft engine room lokal…</p>
    <p v-else-if="error" class="status-text status-text--error" role="alert">{{ error }}</p>

    <section v-else class="draft-list-section" aria-label="Daftar Draft Engine Room">
      <p v-if="drafts.length === 0" class="empty-state" data-testid="draft-list-empty">
        Belum ada draft engine room tersimpan.
      </p>
