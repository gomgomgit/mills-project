...
/**
 * DataPreviewKernelPlantView — screen-048--data-preview-kernel-plant /
 * usecase-048--data-preview-kernel-plant (mounted at
 * /stations/kernel-plant/preview/:id? — `:id` is OPTIONAL; meta.public =
 ...
 *
 * Dual-mode, mirroring DataPreviewDepricarpingView.vue (screen-047)
 * exactly:
 *
 *  - LIST mode — route has NO `:id` param. Shows every local
 *    kernel_plant_record row for the current user, ANY status, with a
 *    client-side date filter (defaults to today) + search filter (on
 *    kernel_plant_id). Tapping a row navigates onward:
 *      - draft_ongoing/draft_paused rows -> `kernel-plant-form`.
 ...
 *    (full header + Kernel Plant Detail grid, however many rows the user
 *    added — no longer always 24, see kernelPlantRecordRepo.ts's header
 *    comment — + Target Operasional reference table), reusing
 *    `getDraftWithDetails()` (already exported by kernelPlantRecordRepo.ts
 *    for screen-044). Renders Checked By and Acknowledged By read-only — no
 *    role restriction on VIEWING them here. Detail rows are rendered
 *    straight from their stored columns — no recomputation, this is
 *    historical data. The already-present empty-state guard (`v-if
 *    detailRows.length === 0` -> status-text message,
 *    `kernel-plant-detail-rows-empty`) — no functional change needed here
 *    for the dynamic add-row revision (REVISED 2026-08-24, entity-catalog
 *    v12), mirrors DataPreviewDepricarpingView.vue's own doc-only fix.
 *
 * Route-driven mode (not a one-time onMounted read) via a `watch()` on the
 * id param (immediate: true) — same reasoning as
 * DataPreviewDepricarpingView.vue's header comment (Vue Router reuses this
 * component instance for param-only navigations on the same route record).
 *
 * Header/breadcrumb/nav-menu: copied verbatim from
 * DataPreviewDepricarpingView.vue. Breadcrumb is 4 segments deep (Home >
 * Production Process Activity > Kernel Plant > Load Data).
 *
 * List-row status badge (StatusBadge.vue):
 *  - draft_ongoing / draft_paused -> status="paused" label="Pause" (matches
 *    Monitor Kernel Plant's list exactly).
 ...
 * (same as DataPreviewDepricarpingView.vue's DETAIL_STATUS_BADGE_MAP).
 *
 * Date filter: `kernel_plant_record.date` is a full local datetime string
 * (`YYYY-MM-DDTHH:mm:ss`, set by FormKernelPlantView.vue's
 * `nowLocalDateTimeString()`), so this filter compares against
 * `item.date.slice(0, 10)`.
 *
 * Search filter: case-insensitive substring match against `kernel_plant_id`.
 */
...
const allRecords = ref<KernelPlantRecord[]>([])
...
async function loadList(): Promise<void> {
  ...
  try {
    allRecords.value = await kernelPlantRecordRepo.getAllRecords(userId)
  } catch (err) {
    listError.value = err instanceof Error ? err.message : 'Gagal memuat daftar data kernel plant lokal.'
  } finally {
    listLoading.value = false
  }
}

function recordLabel(item: KernelPlantRecord): string {
  return item.kernel_plant_id?.trim() ? item.kernel_plant_id : 'Kernel Plant ID belum diisi'
}

const LIST_STATUS_BADGE_MAP: Record<KernelPlantRecord['status'], { status: BadgeStatus; label: string }> = {
...
function listBadgeInfo(item: KernelPlantRecord) {
...
const filteredRecords = computed(() => {
  ...
  return allRecords.value.filter((item) => {
    ...
    if (keyword) {
      const kernelPlantIdMatch = (item.kernel_plant_id ?? '').toLowerCase().includes(keyword)
      if (!kernelPlantIdMatch) {
        return false
      }
    }
...
function onItemClick(item: KernelPlantRecord): void {
  if (item.status === 'draft_ongoing' || item.status === 'draft_paused') {
    router.push({ name: 'kernel-plant-form', params: { id: item.id } })
    return
  }

  router.push({ name: 'data-preview-kernel-plant', params: { id: item.id } })
}
...
const detailRecord = ref<KernelPlantRecord | null>(null)
const detailRows = ref<KernelPlantDetailRow[]>([])
...
async function loadDetail(recordId: string): Promise<void> {
  ...
  try {
    ...
    detailLoadErrorMessage.value = err instanceof Error ? err.message : 'Gagal memuat data kernel plant.'
  } finally {
...
const DETAIL_STATUS_BADGE_MAP: Record<KernelPlantRecord['status'], { status: BadgeStatus; label: string }> = {
...
const screenTitle = computed(() => {
  ...
  return detailRecord.value?.kernel_plant_id ? `Kernel Plant ${detailRecord.value.kernel_plant_id}` : 'Detail Kernel Plant'
})
...
/* ---------------------------------------------------------------------- *
 * Header / breadcrumb / nav-menu — copied verbatim from
 * DataPreviewDepricarpingView.vue.
 * ---------------------------------------------------------------------- */
...
<template>
  <main class="data-preview-kernel-plant-view">
    ...
    <div class="preview-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <button
          type="button"
          class="breadcrumb-link"
          data-testid="breadcrumb-kernel-plant"
          @click="goToMonitorKernelPlant"
        >
          Kernel Plant
        </button>
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
      ...
      <div class="filter-row">
        ...
        <label class="filter-field">
          ...
          <input
            ...
            type="text"
            class="filter-input"
            placeholder="Kernel Plant ID"
            data-testid="search-filter-input"
          />
        </label>
      </div>

      <p v-if="listLoading" class="status-text">Memuat daftar data kernel plant lokal…</p>
      <p v-else-if="listError" class="status-text status-text--error" role="alert">{{ listError }}</p>

      <section v-else class="record-list-section" aria-label="Daftar Data Kernel Plant">
        <div v-if="filteredRecords.length === 0" class="empty-state" data-testid="record-list-empty">
          <p class="empty-state-text">
            {{ hasActiveFilter ? 'Tidak ada data yang cocok dengan filter.' : 'Belum ada data kernel plant.' }}
          </p>
          <button
    ...
    <!-- DETAIL mode -->
    <template v-else>
      <p v-if="detailLoading" class="status-text">Memuat data kernel plant…</p>
      <p
      ...
      >
        Data kernel plant tidak ditemukan.
      </p>
      ...
      <div v-else-if="detailRecord" class="preview-body">
        ...
        <section class="detail-rows-section" aria-label="Kernel Plant Detail">
          <h2 class="detail-rows-title">Kernel Plant Detail</h2>

          <p v-if="detailRows.length === 0" class="status-text" data-testid="kernel-plant-detail-rows-empty">
            Belum ada baris kernel plant detail.
          </p>

          <ul v-else class="detail-rows-list" data-testid="kernel-plant-detail-rows-list">
            <li
              v-for="row in detailRows"
              :key="row.id"
              class="detail-row-view"
              :data-testid="`kernel-plant-detail-row-${row.id}`"
            >
              <div class="detail-row-view-header">
                <span class="detail-row-view-time-slot">{{ row.time_slot }}</span>
              </div>
              <div class="detail-row-view-grid">
                <span>Ripple Mill 1: {{ row.ripple_mill_1_amps ?? '-' }}</span>
                <span>Ripple Mill 2: {{ row.ripple_mill_2_amps ?? '-' }}</span>
                <span>Claybath/Hydro SG: {{ row.claybath_hydro_sg ?? '-' }}</span>
                <span>Kernel Silo 1 Temp: {{ row.kernel_silo_1_temp_c ?? '-' }}</span>
                <span>Kernel Silo 2 Temp: {{ row.kernel_silo_2_temp_c ?? '-' }}</span>
                <span>Kernel Moisture: {{ row.kernel_moisture_percent ?? '-' }}</span>
                <span>Shell Loss: {{ row.shell_loss_percent ?? '-' }}</span>
...
<style scoped>
.data-preview-kernel-plant-view {
  min-height: 100vh;
  display: flex;
