...
/**
 * DataBrowserThreshingTest (Feature/Livewire) —
 * screen-049--data-browser-threshing-web /
 * usecase-049--data-browser-threshing-web.
 *
 * Component tests for App\Livewire\Data\DataBrowserThreshing, mirroring
 * tests/Feature/Livewire/DataBrowserCagesTrackTest.php's structure exactly.
 */
...
it('success: shows filtered rows and export links after setting filter properties', function () {
    $recordWithFilledRows = ThreshingRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();
    ThreshingDetail::factory()->forRecord($recordWithFilledRows)->timeSlot('07:00')->filled()->create();
    ThreshingDetail::factory()->forRecord($recordWithFilledRows)->timeSlot('08:00')->filled()->create();

    ThreshingRecord::factory()->forStation($this->station)->onDate('2026-08-06')->create();
    ThreshingRecord::factory()->forStation($this->station)->onDate('2026-09-01')->create();

    Livewire::actingAs($this->user)
        ->test(DataBrowserThreshing::class)
...
it('Ekspor Gagal: export links are always built from the current filters', function () {
    Livewire::actingAs($this->user)
        ...
        ->assertViewHas('exportCsvUrl', fn ($url) => str_contains($url, '/api/threshing-records/export')
...
it('Klik Baris Membuka Detail: rows render with a clickable-row link to the real detail route', function () {
    $record = ThreshingRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();

    Livewire::actingAs($this->user)
        ->test(DataBrowserThreshing::class)
        ->assertSeeHtml("onclick=\"window.location.href='".route('data.threshing.detail', ['id' => $record->id])."'\"");
});
