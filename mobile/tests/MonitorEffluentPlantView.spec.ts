/**
 * MonitorEffluentPlantView.spec.ts — screen-065--monitor-effluent-plant /
 * usecase-085--monitor-effluent-plant.
 *
 ...
 *   4. 'New Data' creates a new draft and navigates to Form Effluent Plant
 *   5. tapping a list item navigates with the EXISTING draft id
 *   6. 'Load Data' navigates to Data Preview Effluent Plant (list mode, no id)
...
import MonitorEffluentPlantView from '@/views/MonitorEffluentPlantView.vue'
import type { EffluentPlantDraftListItem } from '@/services/effluentPlantRecordRepo'
...
function makeDraft(overrides: Partial<EffluentPlantDraftListItem> & { id: string }): EffluentPlantDraftListItem {
  return {
...
beforeEach(() => {
  ...
  getTodaySummaryMock.mockResolvedValue({ countEffluentPlantRecord: 0, detailRowCount: 0 })
})

describe('MonitorEffluentPlantView', () => {
  it('loads and renders a populated draft/pause list', async () => {
    getDraftsMock.mockResolvedValue([
      makeDraft({ id: 'draft-1', effluent_plant_id: 'EP-001' }),
      makeDraft({ id: 'draft-2', effluent_plant_id: 'EP-002' }),
    ])

    const wrapper = mount(MonitorEffluentPlantView)
    await flushPromises()
  ...
  it('creates a new draft and navigates to Form Effluent Plant when New Data is pressed', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'effluent-plant-form', params: { id: 'new-draft-id' } })
  })

  it('navigates to Form Effluent Plant with the existing draft id when a list item is tapped, without any status-update call', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'effluent-plant-form', params: { id: 'draft-existing' } })
    expect(createDraftMock).not.toHaveBeenCalled()
  })

  it('navigates to Data Preview Effluent Plant in list mode with no id when Load Data is pressed', async () => {
    const wrapper = mount(MonitorEffluentPlantView)
    await flushPromises()
  ...
  it("renders the Hari Ini counter cards from getTodaySummary()'s resolved values", async () => {
    getTodaySummaryMock.mockResolvedValue({ countEffluentPlantRecord: 3, detailRowCount: 7 })

    const wrapper = mount(MonitorEffluentPlantView)
    await flushPromises()

    expect(wrapper.find('[data-testid="counter-count-effluent-plant-record"]').text()).toBe('3')
  ...
  it('renders zero counters when there is no data today', async () => {
    ...
    expect(wrapper.find('[data-testid="counter-count-effluent-plant-record"]').text()).toBe('0')
