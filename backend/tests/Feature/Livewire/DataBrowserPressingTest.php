...
/**
 * DataBrowserPressingTest (Feature/Livewire) —
 * screen-050--data-browser-pressing-web /
 * usecase-050--data-browser-pressing-web.
 *
 * Component tests for App\Livewire\Data\DataBrowserPressing, mirroring
...
it('success: shows filtered rows and export links after setting filter properties', function () {
    $recordWithFilledRows = PressingRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();
    PressingDetail::factory()->forRecord($recordWithFilledRows)->timeSlot('07:00')->filled()->create();
    PressingDetail::factory()->forRecord($recordWithFilledRows)->timeSlot('08:00')->filled()->create();

    PressingRecord::factory()->forStation($this->station)->onDate('2026-08-06')->create();
    PressingRecord::factory()->forStation($this->station)->onDate('2026-09-01')->create();

    Livewire::actingAs($this->user)
        ->test(DataBrowserPressing::class)
...
it('Ekspor Gagal: export links are always built from the current filters', function () {
    Livewire::actingAs($this->user)
        ...
        ->assertViewHas('exportCsvUrl', fn ($url) => str_contains($url, '/api/pressing-records/export')
...
it('Klik Baris Membuka Detail: rows render with a clickable-row link to the real detail route', function () {
    $record = PressingRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();

    Livewire::actingAs($this->user)
        ->test(DataBrowserPressing::class)
        ->assertSeeHtml("onclick=\"window.location.href='".route('data.pressing.detail', ['id' => $record->id])."'\"");
});
