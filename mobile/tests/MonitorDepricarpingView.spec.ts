/**
 * MonitorDepricarpingView.spec.ts — screen-039--monitor-depricarping /
 * usecase-039--monitor-depricarping.
 *
 * Mirrors MonitorPressingView.spec.ts's structure/pattern.
 *
 ...
 *   4. 'New Data' creates a new draft and navigates to Form Depricarping
 *   5. tapping a list item navigates with the EXISTING draft id
 *   6. 'Load Data' navigates to Data Preview Depricarping (list mode, no id)
...
import MonitorDepricarpingView from '@/views/MonitorDepricarpingView.vue'
import type { DepricarpingDraftListItem } from '@/services/depricarpingRecordRepo'
...
function makeDraft(overrides: Partial<DepricarpingDraftListItem> & { id: string }): DepricarpingDraftListItem {
  return {
...
beforeEach(() => {
  ...
  getTodaySummaryMock.mockResolvedValue({ countDepricarpingRecord: 0, detailRowCount: 0 })
})

describe('MonitorDepricarpingView', () => {
  it('loads and renders a populated draft/pause list', async () => {
    getDraftsMock.mockResolvedValue([
      makeDraft({ id: 'draft-1', presser_id: 'DP-001' }),
      makeDraft({ id: 'draft-2', presser_id: 'DP-002' }),
    ])

    const wrapper = mount(MonitorDepricarpingView)
    await flushPromises()
  ...
  it('creates a new draft and navigates to Form Depricarping when New Data is pressed', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'depricarping-form', params: { id: 'new-draft-id' } })
  })

  it('navigates to Form Depricarping with the existing draft id when a list item is tapped, without any status-update call', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'depricarping-form', params: { id: 'draft-existing' } })
    expect(createDraftMock).not.toHaveBeenCalled()
  })

  it('navigates to Data Preview Depricarping in list mode with no id param when Load Data is pressed', async () => {
    const wrapper = mount(MonitorDepricarpingView)
    await flushPromises()
  ...
  it("renders the Hari Ini counter cards from getTodaySummary()'s resolved values", async () => {
    getTodaySummaryMock.mockResolvedValue({ countDepricarpingRecord: 3, detailRowCount: 7 })

    const wrapper = mount(MonitorDepricarpingView)
    await flushPromises()

    expect(wrapper.find('[data-testid="counter-count-depricarping-record"]').text()).toBe('3')
  ...
  it('renders zero counters when there is no data today', async () => {
    ...
    expect(wrapper.find('[data-testid="counter-count-depricarping-record"]').text()).toBe('0')
