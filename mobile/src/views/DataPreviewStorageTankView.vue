...
/**
 * DataPreviewStorageTankView — screen-086--data-preview-storage-tank /
 * usecase-093--data-preview-storage-tank (mounted at
 * /stations/storage-tank/preview/:id? — `:id` is OPTIONAL; meta.public =
 ...
 *    storage_tank_record row for the current user, ANY status, with a
 *    client-side date filter (defaults to today) + search filter (on
 *    storage_tank_id). Tapping a row navigates onward:
 *      - draft_ongoing/draft_paused rows -> `storage-tank-form`.
 ...
 *    (full header + Storage Tank Detail grid, however many rows the user
 *    added), reusing `getDraftWithDetails()` (already exported by
 *    storageTankRecordRepo.ts for screen-076). Renders Checked By and
 ...
 *    (an empty-state message, `storage-tank-detail-rows-empty`).
 *
 ...
 *
 * Date filter: `storage_tank_record.date` is a full local datetime string
 * (`YYYY-MM-DDTHH:mm:ss`, set by FormStorageTankView.vue's
...
const allRecords = ref<StorageTankRecord[]>([])
...
async function loadList(): Promise<void> {
  ...
  try {
    allRecords.value = await storageTankRecordRepo.getAllRecords(userId)
  } catch (err) {
    listError.value = err instanceof Error ? err.message : 'Gagal memuat daftar data storage tank lokal.'
  } finally {
    listLoading.value = false
  }
}

function recordLabel(item: StorageTankRecord): string {
  return item.storage_tank_id?.trim() ? item.storage_tank_id : 'Storage Tank ID belum diisi'
}

const LIST_STATUS_BADGE_MAP: Record<StorageTankRecord['status'], { status: BadgeStatus; label: string }> = {
...
function listBadgeInfo(item: StorageTankRecord) {
...
const filteredRecords = computed(() => {
  ...
  return allRecords.value.filter((item) => {
    ...
    if (keyword) {
      const storageTankIdMatch = (item.storage_tank_id ?? '').toLowerCase().includes(keyword)
      if (!storageTankIdMatch) {
        return false
      }
    }
...
function onItemClick(item: StorageTankRecord): void {
  if (item.status === 'draft_ongoing' || item.status === 'draft_paused') {
    router.push({ name: 'storage-tank-form', params: { id: item.id } })
    return
  }

  router.push({ name: 'data-preview-storage-tank', params: { id: item.id } })
}
...
const detailRecord = ref<StorageTankRecord | null>(null)
const detailRows = ref<StorageTankDetailRow[]>([])
...
async function loadDetail(recordId: string): Promise<void> {
  ...
  try {
    ...
    detailLoadErrorMessage.value = err instanceof Error ? err.message : 'Gagal memuat data storage tank.'
  } finally {
...
const DETAIL_STATUS_BADGE_MAP: Record<StorageTankRecord['status'], { status: BadgeStatus; label: string }> = {
...
const screenTitle = computed(() => {
  ...
  return detailRecord.value?.storage_tank_id ? `Storage Tank ${detailRecord.value.storage_tank_id}` : 'Detail Storage Tank'
})

function onBack(): void {
  if (isDetailMode.value) {
    router.push({ name: 'data-preview-storage-tank' })
    return
  }
...
<template>
  <main class="data-preview-storage-tank-view">
    ...
    <div class="preview-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <button
          type="button"
          class="breadcrumb-link"
          data-testid="breadcrumb-storage-tank"
          @click="goToMonitorStorageTank"
        >
          Storage Tank
        </button>
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
      ...
      <div class="filter-row">
        ...
        <label class="filter-field">
          ...
          <input
            ...
            type="text"
            class="filter-input"
            placeholder="Storage Tank ID"
            data-testid="search-filter-input"
          />
        </label>
      </div>

      <p v-if="listLoading" class="status-text">Memuat daftar data storage tank lokal…</p>
      <p v-else-if="listError" class="status-text status-text--error" role="alert">{{ listError }}</p>

      <section v-else class="record-list-section" aria-label="Daftar Data Storage Tank">
        <div v-if="filteredRecords.length === 0" class="empty-state" data-testid="record-list-empty">
          <p class="empty-state-text">
            {{ hasActiveFilter ? 'Tidak ada data yang cocok dengan filter.' : 'Belum ada data storage tank.' }}
          </p>
          <button
    ...
    <!-- DETAIL mode -->
    <template v-else>
      <p v-if="detailLoading" class="status-text">Memuat data storage tank…</p>
      <p
      ...
      >
        Data storage tank tidak ditemukan.
      </p>
      ...
      <div v-else-if="detailRecord" class="preview-body">
        <FormField :model-value="detailRecord.storage_tank_id" label="Storage Tank ID" disabled />
        ...
        <section class="detail-rows-section" aria-label="Storage Tank Detail">
          <h2 class="detail-rows-title">Storage Tank Detail</h2>

          <p v-if="detailRows.length === 0" class="status-text" data-testid="storage-tank-detail-rows-empty">
            Belum ada baris storage tank detail.
          </p>

          <ul v-else class="detail-rows-list" data-testid="storage-tank-detail-rows-list">
            <li v-for="row in detailRows" :key="row.id" class="detail-row-view" :data-testid="`storage-tank-detail-row-${row.id}`">
              <div class="detail-row-view-header">
                <span class="detail-row-view-time-slot">{{ row.time_slot }}</span>
              </div>
              <div class="detail-row-view-grid">
                <span>CPO Sounding Depth: {{ row.cpo_sounding_depth_mm ?? '-' }}</span>
                <span>Water Dip/Bottom Depth: {{ row.water_dip_bottom_depth_mm ?? '-' }}</span>
                <span>Net Oil Depth: {{ row.net_oil_depth_mm ?? '-' }}</span>
                <span>Oil Temperature - Top: {{ row.oil_temperature_top_c ?? '-' }}</span>
                <span>Oil Temperature - Middle: {{ row.oil_temperature_middle_c ?? '-' }}</span>
                <span>Oil Temperature - Bottom: {{ row.oil_temperature_bottom_c ?? '-' }}</span>
                <span>Average Temperature: {{ row.average_temperature_c ?? '-' }}</span>
                <span>Calculated Volume: {{ row.calculated_volume_m3 ?? '-' }}</span>
                <span>Calculated Weight: {{ row.calculated_weight_mt ?? '-' }}</span>
                <span>FFA: {{ row.ffa_percent ?? '-' }}</span>
                <span>Moisture Content: {{ row.moisture_content_percent ?? '-' }}</span>
                <span>Impurities/Dirt: {{ row.impurities_dirt_percent ?? '-' }}</span>
                <span>DOBI Index: {{ row.dobi_index ?? '-' }}</span>
                <span>Steam Heating Valve Status: {{ row.steam_heating_valve_status ?? '-' }}</span>
                <span>Tank Structural Condition: {{ row.tank_structural_condition ?? '-' }}</span>
                <span>Inspector Name: {{ row.inspector_name ?? '-' }}</span>
...
<style scoped>
.data-preview-storage-tank-view {
  min-height: 100vh;
  display: flex;
