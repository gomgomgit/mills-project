...
/**
 * MonitorClarificationView — screen-069--monitor-clarification /
 * usecase-109--monitor-clarification (mounted at
 * /stations/clarification/monitor, meta.public = false — requires an
 ...
 * local (offline) `clarification_record` table (and its child
 * `clarification_detail` table, for the "filled slot" counter) via
 * clarificationRecordRepo.ts. api_contracts.endpoints is empty for this
 * screen.
 *
 * Structural pattern copied from MonitorBoilerRoomView.vue exactly: a
 ...
 * deleteDraft() calls) — those actions live in Form Clarification
 * (screen-079).
 *
 * Counter card labels per this screen's tech spec: "Jumlah Clarification
 * Record" (COUNT of today's clarification_record rows, any status) /
 * "Jumlah Baris Time-Slot Tercatat" (COUNT of clarification_detail rows
 ...
 * MonitorBoilerRoomView.vue's isNavMenuOpen/toggleNavMenu/closeNavMenu/
 ...
 * (Home > Production Process Activity > Clarification, the first two
 * tappable).
 *
 * Navigation:
 *  - 'New Data' -> clarificationRecordRepo.createDraft(userId) (INSERTs a
 *    new status='draft_ongoing' header row ONLY — no clarification_detail
 *    rows are pre-created, see that function's doc comment), then -> route
 *    name `monitor-clarification`'s sibling `clarification-form` with the
 *    new draft's id.
 *  - Tapping a list item -> route name `clarification-form` with that
 *    item's EXISTING id, directly — no status-transition call.
 *  - 'Load Data' -> route name `data-preview-clarification` with NO id
 *    param (list mode).
...
import {
  clarificationRecordRepo,
  type ClarificationDraftListItem,
  type ClarificationTodaySummary,
...
const drafts = ref<ClarificationDraftListItem[]>([])
...
// today.
const todaySummary = ref<ClarificationTodaySummary>({ countClarificationRecord: 0, detailRowCount: 0 })
...
async function loadDrafts() {
  ...
  try {
    drafts.value = await clarificationRecordRepo.getDrafts(userId)
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal memuat daftar draft clarification lokal.'
  } finally {
    loading.value = false
  }
}
...
async function loadTodaySummary() {
  ...
  try {
    todaySummary.value = await clarificationRecordRepo.getTodaySummary(userId)
  } catch {
...
/**
 * Clarification ID fallback text for a draft whose `clarification_id` has
 * not been filled in yet.
 */
function draftLabel(draft: ClarificationDraftListItem): string {
  return draft.clarification_id?.trim() ? draft.clarification_id : 'Clarification ID belum diisi'
}
...
async function onNewData() {
  ...
  try {
    const id = await clarificationRecordRepo.createDraft(userId)
    router.push({ name: 'clarification-form', params: { id } })
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal membuat draft clarification baru.'
  }
}
...
function onDraftClick(draft: ClarificationDraftListItem) {
  router.push({ name: 'clarification-form', params: { id: draft.id } })
}
...
// business_logic step 8 — breadcrumb taps. 'Clarification' is the current
...
<template>
  <main class="monitor-clarification-view">
    ...
    <div class="monitor-clarification-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <span class="breadcrumb-current" aria-current="page">Clarification</span>
      </nav>
      <h1 class="screen-title">Monitor Clarification</h1>
    </div>
    ...
    <section class="today-summary-section" aria-label="Hari Ini">
      ...
      <div class="today-summary-grid">
        <div class="today-summary-card">
          <span class="today-summary-label">Jumlah Clarification Record</span>
          <span class="today-summary-value" data-testid="counter-count-clarification-record">{{ todaySummary.countClarificationRecord }}</span>
        </div>
        <div class="today-summary-card">
    ...
    <p v-if="loading" class="status-text">Memuat daftar draft clarification lokal…</p>
    <p v-else-if="error" class="status-text status-text--error" role="alert">{{ error }}</p>

    <section v-else class="draft-list-section" aria-label="Daftar Draft Clarification">
      <p v-if="drafts.length === 0" class="empty-state" data-testid="draft-list-empty">
        Belum ada draft clarification tersimpan.
      </p>
