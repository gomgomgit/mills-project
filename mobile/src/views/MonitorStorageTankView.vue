...
/**
 * MonitorStorageTankView — screen-066--monitor-storage-tank /
 * usecase-091--monitor-storage-tank (mounted at
 * /stations/storage-tank/monitor, meta.public = false — requires an
 ...
 * local (offline) `storage_tank_record` table (and its child
 * `storage_tank_detail` table, for the "filled slot" counter) via
 * storageTankRecordRepo.ts. api_contracts.endpoints is empty for this
 * screen.
 *
 * Structural pattern copied from MonitorEffluentPlantView.vue exactly: a
 ...
 * deleteDraft() calls) — those actions live in Form Storage Tank
 * (screen-076).
 *
 * Counter card labels per this screen's tech spec: "Jumlah Storage Tank
 * Record" (COUNT of today's storage_tank_record rows, any status) /
 * "Jumlah Baris Time-Slot Tercatat" (COUNT of storage_tank_detail rows
 ...
 * MonitorEffluentPlantView.vue's isNavMenuOpen/toggleNavMenu/closeNavMenu/
 ...
 * (Home > Production Process Activity > Storage Tank, the first two
 * tappable).
 *
 * Navigation:
 *  - 'New Data' -> storageTankRecordRepo.createDraft(userId) (INSERTs a new
 *    status='draft_ongoing' header row ONLY — no storage_tank_detail rows
 *    are pre-created, see that function's doc comment), then -> route name
 *    `monitor-storage-tank`'s sibling `storage-tank-form` with the new
 *    draft's id.
 *  - Tapping a list item -> route name `storage-tank-form` with that item's
 *    EXISTING id, directly — no status-transition call.
 *  - 'Load Data' -> route name `data-preview-storage-tank` with NO id param
 *    (list mode).
...
import {
  storageTankRecordRepo,
  type StorageTankDraftListItem,
  type StorageTankTodaySummary,
...
const drafts = ref<StorageTankDraftListItem[]>([])
...
// today.
const todaySummary = ref<StorageTankTodaySummary>({ countStorageTankRecord: 0, detailRowCount: 0 })
...
async function loadDrafts() {
  ...
  try {
    drafts.value = await storageTankRecordRepo.getDrafts(userId)
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal memuat daftar draft storage tank lokal.'
  } finally {
    loading.value = false
  }
}
...
async function loadTodaySummary() {
  ...
  try {
    todaySummary.value = await storageTankRecordRepo.getTodaySummary(userId)
  } catch {
...
/**
 * Storage Tank ID fallback text for a draft whose `storage_tank_id` has not
 * been filled in yet.
 */
function draftLabel(draft: StorageTankDraftListItem): string {
  return draft.storage_tank_id?.trim() ? draft.storage_tank_id : 'Storage Tank ID belum diisi'
}
...
async function onNewData() {
  ...
  try {
    const id = await storageTankRecordRepo.createDraft(userId)
    router.push({ name: 'storage-tank-form', params: { id } })
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal membuat draft storage tank baru.'
  }
}
...
function onDraftClick(draft: StorageTankDraftListItem) {
  router.push({ name: 'storage-tank-form', params: { id: draft.id } })
}
...
// business_logic step 8 — breadcrumb taps. 'Storage Tank' is the current
...
<template>
  <main class="monitor-storage-tank-view">
    ...
    <div class="monitor-storage-tank-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <span class="breadcrumb-current" aria-current="page">Storage Tank</span>
      </nav>
      <h1 class="screen-title">Monitor Storage Tank</h1>
    </div>
    ...
    <section class="today-summary-section" aria-label="Hari Ini">
      ...
      <div class="today-summary-grid">
        <div class="today-summary-card">
          <span class="today-summary-label">Jumlah Storage Tank Record</span>
          <span class="today-summary-value" data-testid="counter-count-storage-tank-record">{{ todaySummary.countStorageTankRecord }}</span>
        </div>
        <div class="today-summary-card">
    ...
    <p v-if="loading" class="status-text">Memuat daftar draft storage tank lokal…</p>
    <p v-else-if="error" class="status-text status-text--error" role="alert">{{ error }}</p>

    <section v-else class="draft-list-section" aria-label="Daftar Draft Storage Tank">
      <p v-if="drafts.length === 0" class="empty-state" data-testid="draft-list-empty">
        Belum ada draft storage tank tersimpan.
      </p>
