...
/**
 * DetailClarificationTest (Feature/Api) —
 * screen-109--detail-clarification-web / usecase-113--detail-clarification-web.
 *
 * Integration tests for GET /api/clarification-records/{id}, mirroring
 * tests/Feature/Api/DetailBoilerRoomTest.php's structure exactly.
 */
...
it('berhasil: returns the full record with resolved names and its details grid', function () {
    ...
    ClarificationDetail::factory()->forRecord($record)->timeSlot('09:00')->filled()->create();

    $response = $this->actingAs($this->supervisor, 'web')->getJson("/api/clarification-records/{$record->id}");
...
it('returns details ordered by canonical time-slot, not creation order', function () {
    $record = ClarificationRecord::factory()->forStation($this->station)->create();
    ClarificationDetail::factory()->forRecord($record)->timeSlot('14:00')->create();
    ClarificationDetail::factory()->forRecord($record)->timeSlot('08:00')->create();
    ClarificationDetail::factory()->forRecord($record)->timeSlot('00:00')->create();

    $response = $this->actingAs($this->admin, 'web')->getJson("/api/clarification-records/{$record->id}");
...
it('Mill Management and Admin can also access the detail endpoint', function () {
    ...
    $this->actingAs($this->millManagement, 'web')->getJson("/api/clarification-records/{$record->id}")->assertOk();
    $this->actingAs($this->admin, 'web')->getJson("/api/clarification-records/{$record->id}")->assertOk();
});

it('returns 403 for the Operator role (route-level role gate)', function () {
    $record = ClarificationRecord::factory()->forStation($this->station)->create();

    $this->actingAs($this->operator, 'web')->getJson("/api/clarification-records/{$record->id}")->assertStatus(403);
});

it('returns 404 when the id does not exist', function () {
    $this->actingAs($this->admin, 'web')->getJson('/api/clarification-records/00000000-0000-0000-0000-000000000000')->assertStatus(404);
});
