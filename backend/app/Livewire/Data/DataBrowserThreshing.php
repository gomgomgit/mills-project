...
/**
 * DataBrowserThreshing — screen-049--data-browser-threshing-web /
 * "Data Browser Threshing" (Livewire web page, route name `data.threshing`,
 * /data/threshing).
 *
 * Reuses ThreshingRecordService — the exact same service the API
 * controller (App\Http\Controllers\Api\ThreshingRecordController) uses —
 * so filtering/pagination rules stay identical between the web and API
 * entry points (mirrors screen-018's DataBrowserCagesTrack pattern).
 *
 * Filter state mirrors the tech-spec exactly: date_from, date_to,
 * business_unit_id.
 *
 * Pagination: calls ThreshingRecordService::listRecords() directly and
 * keeps its own $page/$perPage state, rather than Livewire's
 * WithPagination trait — same approach as DataBrowserCagesTrack.
 *
 * Export: dispatched as a plain browser-navigable link (GET
 * /api/threshing-records/export?...), NOT an AJAX/blob download — session
 * cookie carries auth on a same-origin navigation.
 */
#[Layout('data.threshing')]
class DataBrowserThreshing extends Component
{
    ...
    protected function exportUrl(string $format): string
    {
        ...
        return url('/api/threshing-records/export').'?'.http_build_query($query);
    }
    ...
    public function render()
    {
        ...
        return view('livewire.data.data-browser-threshing', [
            'records' => $result['data'],
            'meta' => $result['meta'],
