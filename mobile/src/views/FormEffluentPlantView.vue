...
/**
 * FormEffluentPlantView — screen-075--form-effluent-plant /
 * usecase-086--form-effluent-plant (mounted at
 * /stations/effluent-plant/form/:id, meta.public = false — requires an
 ...
 * local (offline) `effluent_plant_record` + `effluent_plant_detail` tables
 * via effluentPlantRecordRepo.ts.
 *
 ...
 * detail grid pattern:
 *
 *  - "Identitas Effluent Plant" section: `effluent_plant_id` (required,
 *    freely editable), `date` (auto-set once, in memory, ONLY for a
 *    brand-new draft — effluentPlantRecordRepo.ts's createDraft() already
 ...
 *    SearchableSelect dropdown; the 16 numeric/text reading fields plus 3
 *    enum status dropdowns (Biogas Flare Status, Dosing Pump 1 Status,
 *    Sludge Dewatering Status) stay freely editable at all times. "Tambah
 *    baris" is disabled
 ...
 *
 *  - 'Simpan': validates effluent_plant_id (required) — inline error — AND
 *    at least one valid Effluent Plant Detail row (a selected Time-Slot AND
 ...
 *    the repo level (EffluentPlantDetailRequiredError) — mirrors
 *    FormThreshingView.vue's grid validation exactly. On success: calls
 *    effluentPlantRecordRepo.saveDraft() (status='saved'), navigates to
 *    `monitor-effluent-plant`.
 *  - 'Pause': no validation at all, persists current field values as-is
 *    via effluentPlantRecordRepo.pauseDraftWithFormData()
 *    (status='draft_paused'), navigates to `monitor-effluent-plant`.
 *  - 'Clear': ConfirmDialog.vue first; on confirm, calls
 *    effluentPlantRecordRepo.deleteDraft() (cascades delete to every
 *    effluent_plant_detail row for the record, however many exist), then
 *    navigates to `monitor-effluent-plant`; on cancel, no change.
 ...
 * Effluent Plant > Form), the first 3 tappable.
 */
...
import effluentPlantRecordRepo, {
  EffluentPlantDetailRequiredError,
  type EffluentPlantDetailFormRow,
  type EffluentPlantDraftWithDetails,
  type EffluentPlantHeaderFormData,
...
const REQUIRED_FIELDS: (keyof EffluentPlantHeaderFormData)[] = ['effluent_plant_id']

const REQUIRED_FIELD_LABELS: Record<string, string> = {
  effluent_plant_id: 'Effluent Plant ID',
}
...
const DIRTY_CHECK_FIELDS: (keyof EffluentPlantHeaderFormData)[] = [
  'effluent_plant_id',
  'note',
  'checked_by',
  'acknowledged_by',
...
function emptyFormData(): EffluentPlantHeaderFormData {
  return {
    effluent_plant_id: '',
    date: '',
    note: '',
...
function emptyDetailRow(): EffluentPlantDetailFormRow {
  return {
    time_slot: null,
...
const form = reactive<EffluentPlantHeaderFormData>(emptyFormData())
const errors = reactive<Partial<Record<keyof EffluentPlantHeaderFormData, string>>>({})
const detailRows = ref<EffluentPlantDetailFormRow[]>([])
...
// surfaced on the Effluent Plant Detail section rather than mixed into the
...
function dirtySnapshot(): string {
  const header: Partial<Record<keyof EffluentPlantHeaderFormData, string>> = {}
...
// untouched.
function populateForm(record: EffluentPlantRecord): void {
  form.effluent_plant_id = record.effluent_plant_id ?? ''
...
function populateDetailRows(rows: EffluentPlantDraftWithDetails['details']): void {
  detailRows.value = rows.map((row) => ({
    id: row.id,
    time_slot: row.time_slot ?? null,
    anaerobic_pond_1_ph: row.anaerobic_pond_1_ph ?? null,
    anaerobic_pond_1_temp_c: row.anaerobic_pond_1_temp_c ?? null,
    anaerobic_pond_2_ph: row.anaerobic_pond_2_ph ?? null,
    anaerobic_pond_2_temp_c: row.anaerobic_pond_2_temp_c ?? null,
    cooling_pond_ph: row.cooling_pond_ph ?? null,
    cooling_pond_temp_c: row.cooling_pond_temp_c ?? null,
    biogas_flare_status: row.biogas_flare_status ?? '',
    biogas_flow_rate_m3h: row.biogas_flow_rate_m3h ?? null,
    raw_pome_feed_rate_m3h: row.raw_pome_feed_rate_m3h ?? null,
    effluent_discharge_flow_rate_m3h: row.effluent_discharge_flow_rate_m3h ?? null,
    final_discharge_ph: row.final_discharge_ph ?? null,
    final_discharge_bod_mgl_lab: row.final_discharge_bod_mgl_lab ?? null,
    final_discharge_cod_mgl_lab: row.final_discharge_cod_mgl_lab ?? null,
    final_discharge_tss_mgl_lab: row.final_discharge_tss_mgl_lab ?? null,
    dosing_pump_1_status: row.dosing_pump_1_status ?? '',
    chemical_consumed_kgl: row.chemical_consumed_kgl ?? null,
    sludge_dewatering_status: row.sludge_dewatering_status ?? '',
    remarks_maintenance_actions: row.remarks_maintenance_actions ?? '',
...
async function loadDraft(): Promise<void> {
  ...
  try {
    const draft = await effluentPlantRecordRepo.getDraftWithDetails(recordId)
    ...
    loadErrorMessage.value = err instanceof Error ? err.message : 'Gagal memuat draft effluent plant.'
  } finally {
    loading.value = false
  }
}
...
function availableTimeSlotOptions(index: number): string[] {
  ...
  const order = effluentPlantRecordRepo.canonicalTimeSlots()
...
function validateHeader(): boolean {
  for (const key of Object.keys(errors) as (keyof EffluentPlantHeaderFormData)[]) {
    delete errors[key]
  }
...
function isRowFilled(row: EffluentPlantDetailFormRow): boolean {
  return (
...
function isRowValid(row: EffluentPlantDetailFormRow): boolean {
...
async function onSimpan(): Promise<void> {
  ...
  try {
    await effluentPlantRecordRepo.saveDraft(
      recordId,
      { ...form },
      [...detailRows.value],
      [...pendingDeletionIds.value],
    ...
    await syncAfterSave('effluent_plant_record', recordId)
    ...
    actionErrorMessage.value = err instanceof Error ? err.message : 'Gagal menyimpan data effluent plant.'
  } finally {
    saving.value = false
  }
}
...
async function onPause(): Promise<void> {
  ...
  try {
    await effluentPlantRecordRepo.pauseDraftWithFormData(
      recordId,
      { ...form },
      [...detailRows.value],
      [...pendingDeletionIds.value],
...
function goToMonitorEffluentPlant(): void {
  router.push({ name: 'monitor-effluent-plant' })
}
</script>

<template>
  <main class="form-effluent-plant-view">
    ...
    <div class="form-effluent-plant-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <button type="button" class="breadcrumb-link" data-testid="breadcrumb-effluent-plant" @click="goToMonitorEffluentPlant">
          Effluent Plant
        </button>
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
    ...
    <form v-else class="form-body" novalidate @submit.prevent="onSimpan">
      ...
      <section class="form-section" aria-label="Identitas Effluent Plant">
        <h2 class="form-section-title">Identitas Effluent Plant</h2>

        <FormField
          v-model="form.effluent_plant_id"
          label="Effluent Plant ID"
          required
          :error="errors.effluent_plant_id"
      ...
      <section class="form-section" aria-label="Effluent Plant Detail">
        <h2 class="form-section-title">Effluent Plant Detail</h2>
        ...
        <div
          ...
          class="detail-row"
          data-testid="effluent-plant-detail-row"
        >
          <div class="form-field">
          ...
          <FormField
            v-model="row.anaerobic_pond_1_ph"
            label="Anaerobic Pond 1 pH"
            type="number"
            :id="`anaerobic-pond-1-ph-${index}`"
            :data-testid="`anaerobic-pond-1-ph-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.anaerobic_pond_1_temp_c"
            label="Anaerobic Pond 1 Temp (°C)"
            type="number"
            :id="`anaerobic-pond-1-temp-${index}`"
            :data-testid="`anaerobic-pond-1-temp-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.anaerobic_pond_2_ph"
            label="Anaerobic Pond 2 pH"
            type="number"
            :id="`anaerobic-pond-2-ph-${index}`"
            :data-testid="`anaerobic-pond-2-ph-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.anaerobic_pond_2_temp_c"
            label="Anaerobic Pond 2 Temp (°C)"
            type="number"
            :id="`anaerobic-pond-2-temp-${index}`"
            :data-testid="`anaerobic-pond-2-temp-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.cooling_pond_ph"
            label="Cooling Pond pH"
            type="number"
            :id="`cooling-pond-ph-${index}`"
            :data-testid="`cooling-pond-ph-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.cooling_pond_temp_c"
            label="Cooling Pond Temp (°C)"
            type="number"
            :id="`cooling-pond-temp-${index}`"
            :data-testid="`cooling-pond-temp-${index}`"
            :disabled="actionInProgress"
          />
          <div class="form-field">
            <label :for="`biogas-flare-status-${index}`" class="form-field-label">Biogas Flare Status</label>
            <select
              :id="`biogas-flare-status-${index}`"
              :data-testid="`biogas-flare-status-${index}`"
              v-model="row.biogas_flare_status"
              class="native-select"
              :disabled="actionInProgress"
            >
          ...
          <FormField
            v-model="row.biogas_flow_rate_m3h"
            label="Biogas Flow Rate (m3/h)"
            type="number"
            :id="`biogas-flow-rate-${index}`"
            :data-testid="`biogas-flow-rate-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.raw_pome_feed_rate_m3h"
            label="Raw POME Feed Rate (m3/h)"
            type="number"
            :id="`raw-pome-feed-rate-${index}`"
            :data-testid="`raw-pome-feed-rate-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.effluent_discharge_flow_rate_m3h"
            label="Effluent Discharge Flow Rate (m3/h)"
            type="number"
            :id="`effluent-discharge-flow-rate-${index}`"
            :data-testid="`effluent-discharge-flow-rate-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.final_discharge_ph"
            label="Final Discharge pH"
            type="number"
            :id="`final-discharge-ph-${index}`"
            :data-testid="`final-discharge-ph-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.final_discharge_bod_mgl_lab"
            label="Final Discharge BOD (mg/L - Lab)"
            type="number"
            :id="`final-discharge-bod-${index}`"
            :data-testid="`final-discharge-bod-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.final_discharge_cod_mgl_lab"
            label="Final Discharge COD (mg/L - Lab)"
            type="number"
            :id="`final-discharge-cod-${index}`"
            :data-testid="`final-discharge-cod-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.final_discharge_tss_mgl_lab"
            label="Final Discharge TSS (mg/L - Lab)"
            type="number"
            :id="`final-discharge-tss-${index}`"
            :data-testid="`final-discharge-tss-${index}`"
            :disabled="actionInProgress"
          />
          <div class="form-field">
            <label :for="`dosing-pump-1-status-${index}`" class="form-field-label">Dosing Pump 1 Status</label>
            <select
              :id="`dosing-pump-1-status-${index}`"
              :data-testid="`dosing-pump-1-status-${index}`"
              v-model="row.dosing_pump_1_status"
              class="native-select"
              :disabled="actionInProgress"
            >
              <option value="">-- Pilih --</option>
          ...
          <FormField
            v-model="row.chemical_consumed_kgl"
            label="Chemical Consumed (kg/L)"
            type="number"
            :id="`chemical-consumed-${index}`"
            :data-testid="`chemical-consumed-${index}`"
            :disabled="actionInProgress"
          />
          <div class="form-field">
            <label :for="`sludge-dewatering-status-${index}`" class="form-field-label">Sludge Dewatering Status</label>
            <select
              :id="`sludge-dewatering-status-${index}`"
              :data-testid="`sludge-dewatering-status-${index}`"
              v-model="row.sludge_dewatering_status"
              class="native-select"
              :disabled="actionInProgress"
            >
              <option value="">-- Pilih --</option>
          ...
          <FormField
            v-model="row.remarks_maintenance_actions"
            label="Remarks/Maintenance Actions"
            :id="`remarks-maintenance-actions-${index}`"
            :data-testid="`remarks-maintenance-actions-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
    ...
    <ConfirmDialog
      ...
      title="Hapus Draft"
      message="Draft effluent plant ini akan dihapus secara permanen dan tidak dapat dikembalikan. Lanjutkan?"
