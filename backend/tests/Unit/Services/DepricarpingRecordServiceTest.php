...
/**
 * DepricarpingRecordServiceTest — screen-051--data-browser-depricarping-web
 * / screen-055--detail-depricarping-web / screen-059--form-depricarping-web.
 *
 * Unit tests for App\Services\DepricarpingRecordService, mirroring
 * PressingRecordServiceTest.php's structure. REVISED 2026-08-24
 * (entity-catalog v12): depricarping_detail is now a DYNAMIC add-row/
 ...
 * PressingRecordService's pressing_detail upsert pattern, adapted for
 * Depricarping's own header field (presser_id) and 8 reading columns (7
 * numeric + downtime_minutes + findings — a row is filled when EITHER
 * downtime_minutes OR findings, or any other reading column, is set).
 */
...
function depricarpingFormPayload(array $overrides = []): array
{
    return array_merge([
...
beforeEach(function () {
    $this->service = new DepricarpingRecordService();
...
it('returns an empty data list and meta.total = 0 when no records match the filter', function () {
    DepricarpingRecord::factory()->forStation($this->station)->onDate('2026-01-01')->create();
...
it('returns a paginated, filtered list with the shared pagination meta shape', function () {
    ...
    DepricarpingRecord::factory()->forStation($this->station)->onDate('2026-02-05')->count(3)->create();
    DepricarpingRecord::factory()->forStation($this->station)->onDate('2026-03-01')->create();
    DepricarpingRecord::factory()->forStation($otherStation)->onDate('2026-02-05')->create();
...
it('computes filled_slot_count as the number of detail rows with at least one non-null reading column', function () {
    $withTwoFilled = DepricarpingRecord::factory()->forStation($this->station)->onDate('2026-02-05')->create();
    DepricarpingDetail::factory()->forRecord($withTwoFilled)->timeSlot('07:00')->filled()->create();
    DepricarpingDetail::factory()->forRecord($withTwoFilled)->timeSlot('08:00')->create(['findings' => 'Maintenance']);
    DepricarpingDetail::factory()->forRecord($withTwoFilled)->timeSlot('09:00')->create(); // unfilled

    $withNone = DepricarpingRecord::factory()->forStation($this->station)->onDate('2026-02-05')->create();
    DepricarpingDetail::factory()->forRecord($withNone)->timeSlot('07:00')->create();
...
it('treats a row with only downtime_minutes filled as satisfying filled_slot_count', function () {
    $record = DepricarpingRecord::factory()->forStation($this->station)->onDate('2026-02-05')->create();
    DepricarpingDetail::factory()->forRecord($record)->timeSlot('07:00')->create(['downtime_minutes' => 15]);
...
it('throws ExportFailedException when the filtered dataset exceeds the export row limit', function () {
    $limit = DepricarpingRecordService::EXPORT_ROW_LIMIT;
    $total = $limit + 1;
    ...
    expect(DepricarpingRecord::count())->toBe($total);
...
it('returns a StreamedResponse with the correct content-type for csv and excel formats', function (string $format, string $expectedContentType) {
    DepricarpingRecord::factory()->forStation($this->station)->onDate('2026-02-05')->create();
    DepricarpingRecord::factory()->forStation($this->station)->onDate('2026-02-05')->create();
...
it('returns details array sorted into canonical time-slot order, not alphabetical', function () {
    $record = DepricarpingRecord::factory()->forStation($this->station)->create();
    DepricarpingDetail::factory()->forRecord($record)->timeSlot('00:00')->create();
    DepricarpingDetail::factory()->forRecord($record)->timeSlot('09:00')->create();
    DepricarpingDetail::factory()->forRecord($record)->timeSlot('07:00')->create();
...
it('returns null checked_by_name and acknowledged_by_name when not set', function () {
    $record = DepricarpingRecord::factory()->forStation($this->station)->create(['checked_by' => null, 'acknowledged_by' => null]);
...
// screen-059--form-depricarping-web: create()/update() tests below.

it('creates record with resolved station_id and only the given detail rows inserted (no forced 24)', function () {
    $result = $this->service->create(
        depricarpingFormPayload(['production_line_id' => $this->depricarpingStation->production_line_id]),
        $this->creator
    );

    expect($result['station_id'])->toBe($this->depricarpingStation->id);
    expect($result['status'])->toBe('saved');
    expect($result['details'])->toHaveCount(1);
    expect(DepricarpingDetail::where('depricarping_record_id', $result['id'])->count())->toBe(1);
});

it('creates a record with multiple detail rows when given several valid, ascending time slots', function () {
    $result = $this->service->create(
        depricarpingFormPayload([
            'production_line_id' => $this->depricarpingStation->production_line_id,
            'details' => [
                ['time_slot' => '07:00', 'fan_static_pressure_mmh2o' => 45],
                ['time_slot' => '09:00', 'fan_static_pressure_mmh2o' => 46],
...
it('throws ValidationException when a required field is empty', function () {
    expect(fn () => $this->service->create(
        depricarpingFormPayload(['production_line_id' => $this->depricarpingStation->production_line_id, 'presser_id' => '']),
        $this->creator
...
it('throws ValidationException when details is empty', function () {
    expect(fn () => $this->service->create(
        depricarpingFormPayload(['production_line_id' => $this->depricarpingStation->production_line_id, 'details' => []]),
        $this->creator
    ))->toThrow(ValidationException::class);
});
...
it('throws ValidationException when a detail row has a time_slot not in the 24 canonical slots', function () {
    expect(fn () => $this->service->create(
        depricarpingFormPayload([
            'production_line_id' => $this->depricarpingStation->production_line_id,
            'details' => [['time_slot' => '07:30', 'fan_static_pressure_mmh2o' => 1]],
        ]),
        $this->creator
    ))->toThrow(ValidationException::class);
});
...
it('throws ValidationException when two detail rows share the same time_slot', function () {
    expect(fn () => $this->service->create(
        depricarpingFormPayload([
            'production_line_id' => $this->depricarpingStation->production_line_id,
            'details' => [
                ['time_slot' => '07:00', 'fan_static_pressure_mmh2o' => 1],
                ['time_slot' => '07:00', 'fan_static_pressure_mmh2o' => 2],
            ],
        ]),
        $this->creator
...
it('treats a row with only findings filled as satisfying the minimum-one-row rule', function () {
    $result = $this->service->create(
        depricarpingFormPayload([
            'production_line_id' => $this->depricarpingStation->production_line_id,
            'details' => [['time_slot' => '10:00', 'findings' => 'Fan belt loose']],
        ]),
        $this->creator
    );

    expect($result['status'])->toBe('saved');
    expect($result['details'][0]['findings'])->toBe('Fan belt loose');
});
...
it('throws NoActiveDepricarpingStationException when production_line_id has no active depricarping station', function () {
    $otherProductionLine = ProductionLine::factory()->create();

    expect(fn () => $this->service->create(
        depricarpingFormPayload(['production_line_id' => $otherProductionLine->id]),
        $this->creator
    ))->toThrow(NoActiveDepricarpingStationException::class);
});
...
it('sets checked_by to requester id when checked=true and requester role=supervisor', function () {
    ...
    $result = $this->service->create(
        depricarpingFormPayload(['production_line_id' => $this->depricarpingStation->production_line_id, 'checked' => true]),
        $supervisor
    );
...
it('sets acknowledged_by to requester id when acknowledged=true and requester role=mill_management', function () {
    ...
    $result = $this->service->create(
        depricarpingFormPayload(['production_line_id' => $this->depricarpingStation->production_line_id, 'acknowledged' => true]),
        $millManagement
    );
...
it('updates record and upserts details: inserts new row, updates existing row, deletes removed row', function () {
    ...
    $keptDetail = DepricarpingDetail::factory()->forRecord($record)->timeSlot('07:00')->create(['fan_static_pressure_mmh2o' => 45]);
    DepricarpingDetail::factory()->forRecord($record)->timeSlot('12:00')->filled()->create();

    $result = $this->service->update(
        $record->id,
        depricarpingFormPayload([
            'presser_id' => 'DP-EDITED',
            'details' => [
                ['id' => $keptDetail->id, 'time_slot' => '07:00', 'fan_static_pressure_mmh2o' => 47.2],
    ...
    expect($result['presser_id'])->toBe('DP-EDITED');
    expect($result['details'])->toHaveCount(2);
    expect(DepricarpingDetail::where('depricarping_record_id', $record->id)->count())->toBe(2);
    expect(DepricarpingDetail::find($keptDetail->id)->fan_static_pressure_mmh2o)->toBe(47.2);
    expect(DepricarpingDetail::where('time_slot', '12:00')->exists())->toBeFalse();
});
