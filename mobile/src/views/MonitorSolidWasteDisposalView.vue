...
/**
 * MonitorSolidWasteDisposalView — screen-061--monitor-solid-waste-disposal /
 * usecase-061--monitor-solid-waste-disposal (mounted at
 * /stations/solid-waste-disposal/monitor, meta.public = false). Actors:
 * operator, supervisor.
 *
 ...
 *
 * Mirrors MonitorCagesTrackView.vue's structural pattern (breadcrumb +
 * hamburger header, draft/pause list, New Data/Load Data/Back footer) but
 * with EVENT-LOG summary semantics instead of a tipping-progress metric:
 * "Hari Ini" shows Jumlah Solid Waste Disposal (COUNT of today's records,
 * any status) and Jumlah Kejadian Tercatat (COUNT of detail/event rows
 * belonging to those records), plus a "Kejadian Terakhir" card summarizing
 * the single most-recently-logged event across all of the user's records
 * (any date) — there is no meaningful "progress toward N" metric for a
 * free event log, so this replaces Cages Track's sumTotalCages-style card.
 */
...
import {
  ...
  type SolidWasteDisposalLastEvent,
  type SolidWasteDisposalTodaySummary,
...
const drafts = ref<SolidWasteDisposalDraftListItem[]>([])
const loading = ref(false)
const error = ref<string | null>(null)

const todaySummary = ref<SolidWasteDisposalTodaySummary>({ countRecords: 0, countEvents: 0 })
const lastEvent = ref<SolidWasteDisposalLastEvent | null>(null)
...
async function loadDrafts() {
  ...
  try {
    drafts.value = await solidWasteDisposalRecordRepo.getDrafts(userId)
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal memuat daftar draft solid waste disposal lokal.'
  } finally {
    loading.value = false
  }
}
...
async function loadTodaySummary() {
  ...
  try {
    todaySummary.value = await solidWasteDisposalRecordRepo.getTodaySummary(userId)
  } catch {
...
async function loadLastEvent() {
  ...
  try {
    lastEvent.value = await solidWasteDisposalRecordRepo.getLastEventSummary(userId)
  } catch {
...
function draftLabel(draft: SolidWasteDisposalDraftListItem): string {
  return draft.solid_waste_disposal_id?.trim() ? draft.solid_waste_disposal_id : 'Solid Waste Disp. ID belum diisi'
}
...
async function onNewData() {
  ...
  try {
    const id = await solidWasteDisposalRecordRepo.createDraft(userId)
    router.push({ name: 'solid-waste-disposal-form', params: { id } })
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal membuat draft solid waste disposal baru.'
  }
}

function onDraftClick(draft: SolidWasteDisposalDraftListItem) {
  router.push({ name: 'solid-waste-disposal-form', params: { id: draft.id } })
}

function onLoadData() {
  router.push({ name: 'data-preview-solid-waste-disposal' })
}
  ...
  <main class="monitor-swd-view">
    ...
    <div class="monitor-swd-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span aria-hidden="true">/</span>
        <span aria-current="page">Solid Waste Disposal</span>
      </nav>
      <h1 class="screen-title">Monitor Solid Waste Disposal</h1>
    </div>
    ...
    <section class="today-summary-section" aria-label="Hari Ini">
      ...
      <div class="today-summary-grid">
        <div class="today-summary-card">
          <span class="today-summary-label">Jumlah Solid Waste Disposal</span>
    ...
    <section class="last-event-section" aria-label="Kejadian Terakhir" data-testid="last-event-card">
      ...
      <div v-else class="last-event-body">
        <span>{{ lastEvent.solidWasteType ?? 'Tipe tidak tercatat' }}</span>
        <span>{{ lastEvent.eventDate ?? '-' }}</span>
        <span>{{ lastEvent.vehicleNo ?? '-' }}</span>
      </div>
    </section>

    <p v-if="loading" class="status-text">Memuat daftar draft solid waste disposal lokal…</p>
    <p v-else-if="error" class="status-text status-text--error" role="alert">{{ error }}</p>

    <section v-else class="draft-list-section" aria-label="Daftar Draft Solid Waste Disposal">
      <p v-if="drafts.length === 0" class="empty-state" data-testid="draft-list-empty">
        Belum ada draft solid waste disposal tersimpan.
      </p>
...
<style scoped>
.monitor-swd-view { min-height: 100vh; display: flex; flex-direction: column; gap: 20px; padding: 0 20px 20px; background: #ffffff; font-family: 'Inter', sans-serif; box-sizing: border-box; }
...
.monitor-swd-header { display: flex; flex-direction: column; gap: 6px; }
