...
/**
 * FormProcessQualityControlView — screen-080--form-process-quality-control /
 * usecase-116--form-process-quality-control (mounted at
 * /stations/process-quality-control/form/:id, meta.public = false —
 * requires an authenticated session, enforced by the router's global auth
 * guard; see router/index.ts). Actors: operator, supervisor (Checked By
 * toggle interactive only for supervisor, Acknowledged By toggle
 * interactive only for mill_management).
 *
 * Mobile-only screen — no backend API endpoint; all data comes from the
 * local (offline) `process_quality_control_record` + `process_quality_control_detail`
 * tables via processQualityControlRecordRepo.ts.
 *
 ...
 * FormClarificationView.vue exactly, including its dynamic add-row/
 * remove-row detail grid pattern:
 *
 *  - "Identitas Process Quality Control" section: `process_qc_id`
 *    (required, freely editable), `date` (auto-set once, in memory, ONLY
 *    for a brand-new draft — processQualityControlRecordRepo.ts's
 *    createDraft() already auto-fills it at draft-creation time; this
 *    screen's populateForm() re-applies the same "only if still empty"
 *    rule defensively). Always rendered disabled.
 *
 ...
 *
 *  - "Process Quality Control Detail" section — the dynamic grid, 16
 *    non-time_slot columns per row (the MOST of any station in the
 *    project): Shift (text), Fruit Press Oil Loss in Sludge/Fibre (%),
 *    Purifier & Clarification Balance Inlet Temp (°C)/Backpressure (Bar),
 *    Vacuum Drying Station Drier Temp (°C)/Vacuum Pressure (Bar),
 *    Decanter/Centrifuge Feed Rate (MT/h)/Oil Loss in Cake (%), Final
 *    Storage FFA/Moisture Content/Impurities-Dirt (%), Final Storage DOBI
 *    Index, QC Inspector ID (text), QC Engineering Corrective
 *    Actions/Remarks (text), Findings (text). `detailRows` (in-memory,
 *    rows without a saved `id` are new), `addDetailRow()` (pushes a fresh
 *    empty row), `removeDetailRow(index)` (splices the array; if the row
 *    had an `id`, its id is queued into `pendingDeletionIds` for the next
 *    save/pause — not deleted immediately), `availableTimeSlotOptions(index)`
 *    (dropdown options = all 24 canonical slots MINUS ones used by OTHER
 *    rows MINUS ones at-or-before the highest slot already picked by
 *    another row, always including the row's own current value). "Tambah
 *    baris" is disabled once all 24 canonical slots are already used
 *    (`detailRows.length >= 24`) or while an action is in progress.
 *
 *  - THIS IS THE STATION WITH THE MOST NON-TIME_SLOT COLUMNS IN THE
 *    PROJECT — NO operational-target reference table, NO enum `<select>`
 *    dropdowns anywhere on this screen (all 12 numeric readings are plain
 *    type=number FormFields; Shift/QC Inspector ID/QC Engineering
 *    Corrective Actions/Findings are plain text FormFields).
 *
 *  - 'Simpan': validates process_qc_id (required) — inline error — AND at
 *    least one valid Process Quality Control Detail row (a selected
 *    Time-Slot AND at least one of the 14 reading/keterangan columns
 *    filled, excluding Shift/QC Inspector ID), a dedicated error message
 *    shown on the grid (not mixed into per-field header errors),
 *    re-enforced at the repo level
 *    (ProcessQualityControlDetailRequiredError) — mirrors
 *    FormClarificationView.vue's grid validation exactly. On success:
 *    calls processQualityControlRecordRepo.saveDraft() (status='saved'),
 *    navigates to `monitor-process-quality-control`.
 *  - 'Pause': no validation at all, persists current field values as-is via
 *    processQualityControlRecordRepo.pauseDraftWithFormData()
 *    (status='draft_paused'), navigates to `monitor-process-quality-control`.
 *  - 'Clear': ConfirmDialog.vue first; on confirm, calls
 *    processQualityControlRecordRepo.deleteDraft() (cascades delete to
 *    every process_quality_control_detail row for the record, however
 *    many exist), then navigates to `monitor-process-quality-control`; on
 *    cancel, no change.
 ...
 * FormClarificationView.vue. Breadcrumb is 4 segments deep (Home >
 * Production Process Activity > Process Quality Control > Form), the
 * first 3 tappable.
 */
...
import processQualityControlRecordRepo, {
  ProcessQualityControlDetailRequiredError,
  type ProcessQualityControlDetailFormRow,
  type ProcessQualityControlDraftWithDetails,
  type ProcessQualityControlHeaderFormData,
...
// business_logic step 6 — required header fields. Note/Checked By/
// Acknowledged By are deliberately NOT required.
const REQUIRED_FIELDS: (keyof ProcessQualityControlHeaderFormData)[] = ['process_qc_id']

const REQUIRED_FIELD_LABELS: Record<string, string> = {
  process_qc_id: 'Process QC ID',
}
...
// reasoning as FormClarificationView.vue excluding date).
const DIRTY_CHECK_FIELDS: (keyof ProcessQualityControlHeaderFormData)[] = [
  'process_qc_id',
  'note',
  'checked_by',
  'acknowledged_by',
...
function emptyFormData(): ProcessQualityControlHeaderFormData {
  return {
    process_qc_id: '',
    date: '',
    note: '',
...
function emptyDetailRow(): ProcessQualityControlDetailFormRow {
  return {
    time_slot: null,
    shift: null,
...
const form = reactive<ProcessQualityControlHeaderFormData>(emptyFormData())
const errors = reactive<Partial<Record<keyof ProcessQualityControlHeaderFormData, string>>>({})
const detailRows = ref<ProcessQualityControlDetailFormRow[]>([])
...
// next Simpan/Pause call (mirrors FormClarificationView.vue's
...
// surfaced on the Process Quality Control Detail section rather than
...
// FormClarificationView.vue's `isDirty`.
let loadedSnapshot = dirtySnapshot()

function dirtySnapshot(): string {
  const header: Partial<Record<keyof ProcessQualityControlHeaderFormData, string>> = {}
 ...
 * device's local time. Mirrors FormClarificationView.vue's
...
// untouched.
function populateForm(record: ProcessQualityControlRecord): void {
  form.process_qc_id = record.process_qc_id ?? ''
...
function populateDetailRows(rows: ProcessQualityControlDraftWithDetails['details']): void {
  detailRows.value = rows.map((row) => ({
    id: row.id,
    time_slot: row.time_slot ?? null,
    shift: row.shift ?? '',
    fruit_press_oil_loss_in_sludge_percent: row.fruit_press_oil_loss_in_sludge_percent ?? null,
    fruit_press_oil_loss_in_fibre_percent: row.fruit_press_oil_loss_in_fibre_percent ?? null,
    purifier_clarification_balance_inlet_temp_c: row.purifier_clarification_balance_inlet_temp_c ?? null,
    purifier_clarification_balance_backpressure_bar: row.purifier_clarification_balance_backpressure_bar ?? null,
    vacuum_drying_station_drier_temp_c: row.vacuum_drying_station_drier_temp_c ?? null,
    vacuum_drying_station_vacuum_pressure_bar: row.vacuum_drying_station_vacuum_pressure_bar ?? null,
    decanter_centrifuge_feed_rate_mth: row.decanter_centrifuge_feed_rate_mth ?? null,
    decanter_centrifuge_oil_loss_in_cake_percent: row.decanter_centrifuge_oil_loss_in_cake_percent ?? null,
    final_storage_ffa_percent: row.final_storage_ffa_percent ?? null,
    final_storage_moisture_content_percent: row.final_storage_moisture_content_percent ?? null,
    final_storage_impurities_dirt_percent: row.final_storage_impurities_dirt_percent ?? null,
    final_storage_dobi_index: row.final_storage_dobi_index ?? null,
    qc_inspector_id: row.qc_inspector_id ?? '',
    qc_engineering_corrective_actions: row.qc_engineering_corrective_actions ?? '',
...
async function loadDraft(): Promise<void> {
  ...
  try {
    ...
    loadErrorMessage.value = err instanceof Error ? err.message : 'Gagal memuat draft process quality control.'
  } finally {
    loading.value = false
  }
}
 ...
 * mirrors FormClarificationView.vue's `availableTimeSlotOptions()`
 * exactly. Both exclusions exempt the row's OWN current selection.
 * Reactive over the entire `detailRows.value` array.
 */
function availableTimeSlotOptions(index: number): string[] {
  ...
  const order = processQualityControlRecordRepo.canonicalTimeSlots()
...
// FormClarificationView.vue's timeSlotSelectOptions() uses for its own
// per-row dropdown.
...
// business_logic — "Tambah baris": disabled once all 24 canonical slots
// are already used by existing rows, or while an action is in progress
// (see the template's :disabled binding).
const canAddDetailRow = computed(() => detailRows.value.length < 24)

// business_logic — "Tambah baris".
...
// mirrors FormClarificationView.vue's removeDetailRow() exactly. Either
...
function validateHeader(): boolean {
  for (const key of Object.keys(errors) as (keyof ProcessQualityControlHeaderFormData)[]) {
    delete errors[key]
  }
...
function isRowFilled(row: ProcessQualityControlDetailFormRow): boolean {
  return (
...
// business_logic — at-least-one-valid-row validation (a selected Time-Slot
// AND at least one filled reading/keterangan column, excluding Shift/QC
// Inspector ID), kept separate from validateHeader() since it's a
// distinct/special error surfaced on the grid, not a per-field header
// error — mirrors FormClarificationView.vue's validateDetailRows()
// exactly.
function isRowValid(row: ProcessQualityControlDetailFormRow): boolean {
...
async function onSimpan(): Promise<void> {
  ...
  try {
    await processQualityControlRecordRepo.saveDraft(
      recordId,
      { ...form },
      [...detailRows.value],
      [...pendingDeletionIds.value],
    ...
    actionErrorMessage.value = err instanceof Error ? err.message : 'Gagal menyimpan data process quality control.'
  } finally {
    saving.value = false
  }
}

// business_logic — 'Pause'. Checkpoint save, no required-field validation.
async function onPause(): Promise<void> {
  actionErrorMessage.value = null
  pausing.value = true

  try {
    await processQualityControlRecordRepo.pauseDraftWithFormData(
      recordId,
      { ...form },
      [...detailRows.value],
      [...pendingDeletionIds.value],
...
async function onClearConfirm(): Promise<void> {
  ...
  try {
    await processQualityControlRecordRepo.deleteDraft(recordId)
    router.push({ name: 'monitor-process-quality-control' })
  } catch (err) {
    actionErrorMessage.value = err instanceof Error ? err.message : 'Gagal menghapus draft process quality control.'
  } finally {
...
// business_logic — Checked By / Acknowledged By "Saya verifikasi..."
// toggles.
...
<template>
  <main class="form-process-quality-control-view">
    ...
    <div class="form-process-quality-control-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <button type="button" class="breadcrumb-link" data-testid="breadcrumb-process-quality-control" @click="goToMonitorProcessQualityControl">
          Process Quality Control
        </button>
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <span class="breadcrumb-current" aria-current="page">Form</span>
      </nav>
      <h1 class="screen-title">Form Process Quality Control</h1>
    </div>

    <p v-if="loading" class="status-text">Memuat draft process quality control…</p>
    <p v-else-if="notFound" class="status-text status-text--error" role="alert">
      Draft process quality control tidak ditemukan.
    </p>
    ...
    <form v-else class="form-body" novalidate @submit.prevent="onSimpan">
      ...
      <section class="form-section" aria-label="Identitas Process Quality Control">
        <h2 class="form-section-title">Identitas Process Quality Control</h2>

        <FormField
          v-model="form.process_qc_id"
          label="Process QC ID"
          required
          :error="errors.process_qc_id"
      ...
      <section class="form-section" aria-label="Process Quality Control Detail">
        <h2 class="form-section-title">Process Quality Control Detail</h2>
        ...
        <div
          ...
          class="detail-row"
          data-testid="process-quality-control-detail-row"
        >
          <div class="form-field">
          ...
          />
          <FormField
            v-model="row.fruit_press_oil_loss_in_sludge_percent"
            label="Fruit Press Oil Loss in Sludge (%)"
            type="number"
            :id="`fruit-press-oil-loss-in-sludge-${index}`"
            :data-testid="`fruit-press-oil-loss-in-sludge-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.fruit_press_oil_loss_in_fibre_percent"
            label="Fruit Press Oil Loss in Fibre (%)"
            type="number"
            :id="`fruit-press-oil-loss-in-fibre-${index}`"
            :data-testid="`fruit-press-oil-loss-in-fibre-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.purifier_clarification_balance_inlet_temp_c"
            label="Purifier & Clarification Balance Inlet Temp (°C)"
            type="number"
            :id="`purifier-clarification-balance-inlet-temp-${index}`"
            :data-testid="`purifier-clarification-balance-inlet-temp-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.purifier_clarification_balance_backpressure_bar"
            label="Purifier & Clarification Balance Backpressure (Bar)"
            type="number"
            :id="`purifier-clarification-balance-backpressure-${index}`"
            :data-testid="`purifier-clarification-balance-backpressure-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.vacuum_drying_station_drier_temp_c"
            label="Vacuum Drying Station Drier Temp (°C)"
            type="number"
            :id="`vacuum-drying-station-drier-temp-${index}`"
            :data-testid="`vacuum-drying-station-drier-temp-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.vacuum_drying_station_vacuum_pressure_bar"
            label="Vacuum Drying Station Vacuum Pressure (Bar)"
            type="number"
            :id="`vacuum-drying-station-vacuum-pressure-${index}`"
            :data-testid="`vacuum-drying-station-vacuum-pressure-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.decanter_centrifuge_feed_rate_mth"
            label="Decanter/Centrifuge Feed Rate (MT/h)"
            type="number"
            :id="`decanter-centrifuge-feed-rate-${index}`"
            :data-testid="`decanter-centrifuge-feed-rate-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.decanter_centrifuge_oil_loss_in_cake_percent"
            label="Decanter/Centrifuge Oil Loss in Cake (%)"
            type="number"
            :id="`decanter-centrifuge-oil-loss-in-cake-${index}`"
            :data-testid="`decanter-centrifuge-oil-loss-in-cake-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.final_storage_ffa_percent"
            label="Final Storage FFA (%)"
            type="number"
            :id="`final-storage-ffa-${index}`"
            :data-testid="`final-storage-ffa-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.final_storage_moisture_content_percent"
            label="Final Storage Moisture Content (%)"
            type="number"
            :id="`final-storage-moisture-content-${index}`"
            :data-testid="`final-storage-moisture-content-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.final_storage_impurities_dirt_percent"
            label="Final Storage Impurities/Dirt (%)"
            type="number"
            :id="`final-storage-impurities-dirt-${index}`"
            :data-testid="`final-storage-impurities-dirt-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.final_storage_dobi_index"
            label="Final Storage DOBI Index"
            type="number"
            :id="`final-storage-dobi-index-${index}`"
            :data-testid="`final-storage-dobi-index-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.qc_inspector_id"
            label="QC Inspector ID"
            :id="`qc-inspector-id-${index}`"
            :data-testid="`qc-inspector-id-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
            v-model="row.qc_engineering_corrective_actions"
            label="QC Engineering Corrective Actions/Remarks"
            :id="`qc-engineering-corrective-actions-${index}`"
            :data-testid="`qc-engineering-corrective-actions-${index}`"
            :disabled="actionInProgress"
          />
          <FormField
    ...
    <ConfirmDialog
      ...
      title="Hapus Draft"
      message="Draft process quality control ini akan dihapus secara permanen dan tidak dapat dikembalikan. Lanjutkan?"
...
<style scoped>
.form-process-quality-control-view {
  min-height: 100vh;
  display: flex;
...
.form-process-quality-control-header {
  display: flex;
  flex-direction: column;
  gap: 6px;
}
