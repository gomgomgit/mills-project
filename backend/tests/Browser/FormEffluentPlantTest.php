...
/**
 * FormEffluentPlantTest (Browser/Playwright) —
 ...
 * (eptest-supervisor01 / Passw0rd!), a Production Line named
 * "PL Mill A" with an active Effluent Plant station, a second Production
 * Line "PL Tanpa Effluent Plant" with no active Effluent Plant station, and
 * a pre-seeded Effluent Plant record with Effluent Plant ID
...
test.describe('Form Effluent Plant (Web)', () => {
  ...
  test('klik Tambah Data, isi form lengkap termasuk 1 kolom bacaan, klik Simpan, halaman Detail menampilkan record baru', async ({ page }) => {
    ...
    await page.goto(`${BASE_URL}/data/effluent-plant`);
    ...
    await page.locator('[data-testid="effluent-plant-id-input"]').fill(`EP-BROWSER-${uniqueSuffix}`);
    await page.locator('[data-testid="add-row-button"]').click();
    await page.locator('[data-testid="time-slot-select-0"]').selectOption('07:00');
    await page.locator('[data-testid="anaerobic-pond-1-ph-0"]').fill('45.5');
    await page.locator('[data-testid="save-button"]').click();

    await page.waitForURL((url) => url.pathname.startsWith('/data/effluent-plant/') && !url.pathname.endsWith('/create'));
    await expect(page.locator('[data-testid="detail-effluent-plant-id"]')).toContainText(`EP-BROWSER-${uniqueSuffix}`);
  });
  ...
  test('kosongkan field wajib, klik Simpan, error inline muncul', async ({ page }) => {
    ...
    await expect(page.locator('.ef-field__error')).toContainText('Effluent Plant ID');
  });
  ...
  test('isi header lengkap tanpa menambah baris apapun, klik Simpan, pesan error khusus muncul', async ({ page }) => {
    ...
    await page.locator('[data-testid="production-line-select"]').selectOption({ label: PRODUCTION_LINE_NAME });
    await page.locator('[data-testid="effluent-plant-id-input"]').fill('EP-NO-DETAIL');
    await page.locator('[data-testid="save-button"]').click();

    await expect(page.locator('[data-testid="detail-error"]')).toBeVisible();
  });

  // Scenario: "Production Line Tanpa Station Effluent Plant Aktif"
  test('pilih Production Line tanpa station effluent-plant, klik Simpan, error ditampilkan', async ({ page }) => {
    await login(page, 'eptest-supervisor01', PASSWORD);
    await page.goto(`${BASE_URL}${CREATE_PATH}`);

    await page.locator('[data-testid="production-line-select"]').selectOption({ label: 'PL Tanpa Effluent Plant' });
    await page.locator('[data-testid="effluent-plant-id-input"]').fill('EP-NO-STATION');
    await page.locator('[data-testid="add-row-button"]').click();
    await page.locator('[data-testid="time-slot-select-0"]').selectOption('07:00');
    await page.locator('[data-testid="anaerobic-pond-1-ph-0"]').fill('10');
    await page.locator('[data-testid="save-button"]').click();
  ...
  test('grid Effluent Plant Detail dimulai kosong, bertambah satu baris tiap klik Tambah Baris, dan bisa dihapus', async ({ page }) => {
    await login(page, 'eptest-supervisor01', PASSWORD);
    await page.goto(`${BASE_URL}${CREATE_PATH}`);

    await expect(page.locator('[data-testid="effluent-plant-detail-grid"] tbody tr')).toHaveCount(0);

    await page.locator('[data-testid="add-row-button"]').click();
    await expect(page.locator('[data-testid="effluent-plant-detail-grid"] tbody tr')).toHaveCount(1);

    await page.locator('[data-testid="add-row-button"]').click();
    await expect(page.locator('[data-testid="effluent-plant-detail-grid"] tbody tr')).toHaveCount(2);

    await page.locator('[data-testid="remove-row-button-0"]').click();
    await expect(page.locator('[data-testid="effluent-plant-detail-grid"] tbody tr')).toHaveCount(1);
  });

  // Scenario: "Edit Record Effluent Plant — berhasil"
  test('klik Edit dari Detail, ubah field, klik Simpan, Detail menampilkan nilai baru', async ({ page }) => {
    await login(page, 'eptest-supervisor01', PASSWORD);
    await page.goto(`${BASE_URL}/data/effluent-plant`);
    await page.locator('.ep-table__row', { hasText: 'EP-BROWSER-EDIT' }).click();
    await page.locator('[data-testid="edit-button"]').click();

    await page.locator('[data-testid="effluent-plant-id-input"]').fill('EP-BROWSER-EDIT-DONE');
    await page.locator('[data-testid="save-button"]').click();

    await page.waitForURL((url) => !url.pathname.endsWith('/edit'));
    await expect(page.locator('body')).toContainText('EP-BROWSER-EDIT-DONE');
  });

  // Scenario: "Record Tidak Ditemukan (mode edit)"
  test('navigasi ke id yang tidak valid, halaman menampilkan error', async ({ page }) => {
    await login(page, 'eptest-supervisor01', PASSWORD);
    await page.goto(`${BASE_URL}/data/effluent-plant/00000000-0000-0000-0000-000000000000/edit`);
