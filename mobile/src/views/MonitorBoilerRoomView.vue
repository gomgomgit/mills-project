...
/**
 * MonitorBoilerRoomView — screen-068--monitor-boiler-room /
 * usecase-103--monitor-boiler-room (mounted at
 * /stations/boiler-room/monitor, meta.public = false — requires an
 ...
 * local (offline) `boiler_room_record` table (and its child
 * `boiler_room_detail` table, for the "filled slot" counter) via
 * boilerRoomRecordRepo.ts. api_contracts.endpoints is empty for this
 * screen.
 *
 * Structural pattern copied from MonitorEngineRoomView.vue exactly: a
 ...
 * deleteDraft() calls) — those actions live in Form Boiler Room
 * (screen-078).
 *
 * Counter card labels per this screen's tech spec: "Jumlah Boiler Room
 * Record" (COUNT of today's boiler_room_record rows, any status) /
 * "Jumlah Baris Time-Slot Tercatat" (COUNT of boiler_room_detail rows
 ...
 * MonitorEngineRoomView.vue's isNavMenuOpen/toggleNavMenu/closeNavMenu/
 ...
 * (Home > Production Process Activity > Boiler Room, the first two
 * tappable).
 *
 * Navigation:
 *  - 'New Data' -> boilerRoomRecordRepo.createDraft(userId) (INSERTs a new
 *    status='draft_ongoing' header row ONLY — no boiler_room_detail rows
 *    are pre-created, see that function's doc comment), then -> route name
 *    `monitor-boiler-room`'s sibling `boiler-room-form` with the new
 *    draft's id.
 *  - Tapping a list item -> route name `boiler-room-form` with that item's
 *    EXISTING id, directly — no status-transition call.
 *  - 'Load Data' -> route name `data-preview-boiler-room` with NO id param
 *    (list mode).
...
const drafts = ref<BoilerRoomDraftListItem[]>([])
...
// today.
const todaySummary = ref<BoilerRoomTodaySummary>({ countBoilerRoomRecord: 0, detailRowCount: 0 })
...
async function loadDrafts() {
  ...
  try {
    drafts.value = await boilerRoomRecordRepo.getDrafts(userId)
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal memuat daftar draft boiler room lokal.'
  } finally {
    loading.value = false
  }
}
...
async function loadTodaySummary() {
  ...
  try {
    todaySummary.value = await boilerRoomRecordRepo.getTodaySummary(userId)
  } catch {
...
/**
 * Boiler Room ID fallback text for a draft whose `boiler_room_id` has not
 * been filled in yet.
 */
function draftLabel(draft: BoilerRoomDraftListItem): string {
  return draft.boiler_room_id?.trim() ? draft.boiler_room_id : 'Boiler Room ID belum diisi'
}
...
function onDraftClick(draft: BoilerRoomDraftListItem) {
  router.push({ name: 'boiler-room-form', params: { id: draft.id } })
}
...
// business_logic step 8 — breadcrumb taps. 'Boiler Room' is the current
...
<template>
  <main class="monitor-boiler-room-view">
    ...
    <div class="monitor-boiler-room-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <span class="breadcrumb-current" aria-current="page">Boiler Room</span>
      </nav>
      <h1 class="screen-title">Monitor Boiler Room</h1>
    </div>
    ...
    <section class="today-summary-section" aria-label="Hari Ini">
      ...
      <div class="today-summary-grid">
        <div class="today-summary-card">
          <span class="today-summary-label">Jumlah Boiler Room Record</span>
          <span class="today-summary-value" data-testid="counter-count-boiler-room-record">{{ todaySummary.countBoilerRoomRecord }}</span>
        </div>
        <div class="today-summary-card">
    ...
    <section v-else class="draft-list-section" aria-label="Daftar Draft Boiler Room">
      <p v-if="drafts.length === 0" class="empty-state" data-testid="draft-list-empty">
        Belum ada draft boiler room tersimpan.
      </p>
