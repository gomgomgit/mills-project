...
/**
 * DataBrowserClarification — screen-099--data-browser-clarification-web /
 * "Data Browser Clarification" (Livewire web page, route name
 * `data.clarification`, /data/clarification).
 *
 * Reuses ClarificationRecordService — the exact same service the API
 * controller (App\Http\Controllers\Api\ClarificationRecordController) uses
 * — so filtering/pagination rules stay identical between the web and API
 * entry points (mirrors DataBrowserBoilerRoom's pattern).
 *
 * Filter state mirrors the tech-spec exactly: date_from, date_to,
 * business_unit_id.
 *
 * Pagination: calls ClarificationRecordService::listRecords() directly and
 * keeps its own $page/$perPage state, rather than Livewire's
 * WithPagination trait — same approach as DataBrowserBoilerRoom.
 *
 * Export: dispatched as a plain browser-navigable link (GET
 * /api/clarification-records/export?...), NOT an AJAX/blob download —
 * session cookie carries auth on a same-origin navigation.
 */
#[Layout('data.clarification')]
class DataBrowserClarification extends Component
{
    ...
    protected function exportUrl(string $format): string
    {
        ...
        return url('/api/clarification-records/export').'?'.http_build_query($query);
    }
    ...
    public function render()
    {
        ...
        return view('livewire.data.data-browser-clarification', [
            'records' => $result['data'],
            'meta' => $result['meta'],
