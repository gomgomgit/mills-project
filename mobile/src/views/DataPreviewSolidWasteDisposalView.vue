...
/**
 * DataPreviewSolidWasteDisposalView — screen-081--data-preview-solid-waste-disposal
 * / usecase-063--data-preview-solid-waste-disposal (mounted at
 * /stations/solid-waste-disposal/preview/:id? — `:id` OPTIONAL, meta.public
 ...
 *
 * Mirrors DataPreviewCagesTrackView.vue's dual route-driven mode pattern:
 *  - LIST mode (no `:id`) — every local record for the current user, any
 *    status, client-side date + search (solid_waste_disposal_id) filter.
...
const allRecords = ref<SolidWasteDisposalRecord[]>([])
...
function recordLabel(item: SolidWasteDisposalRecord): string {
  return item.solid_waste_disposal_id?.trim() ? item.solid_waste_disposal_id : 'Solid Waste Disp. ID belum diisi'
}

const LIST_STATUS_BADGE_MAP: Record<SolidWasteDisposalRecord['status'], { status: BadgeStatus; label: string }> = {
...
function listBadgeInfo(item: SolidWasteDisposalRecord) {
...
const filteredRecords = computed(() => {
  ...
  return allRecords.value.filter((item) => {
    ...
    if (keyword) {
      const idMatch = (item.solid_waste_disposal_id ?? '').toLowerCase().includes(keyword)
      if (!idMatch) {
        return false
      }
    }
...
function onItemClick(item: SolidWasteDisposalRecord): void {
  if (item.status === 'draft_ongoing' || item.status === 'draft_paused') {
    router.push({ name: 'solid-waste-disposal-form', params: { id: item.id } })
    return
  }

  router.push({ name: 'data-preview-solid-waste-disposal', params: { id: item.id } })
}
...
const DETAIL_STATUS_BADGE_MAP: Record<SolidWasteDisposalRecord['status'], { status: BadgeStatus; label: string }> = {
...
<template>
  <main class="preview-swd-view">
    ...
    <div class="preview-swd-header">
      ...
      <h1 class="screen-title">Data Preview Solid Waste Disposal</h1>
    </div>
    ...
    <template v-else>
      ...
      <div v-else-if="detailRecord" class="detail-body">
        ...
        <section class="detail-section">
          ...
          <div v-else class="detail-table-wrap" data-testid="solid-waste-disposal-detail-log">
...
.preview-swd-header { display: flex; flex-direction: column; gap: 6px; }
