...
/**
 * DataBrowserProcessQualityControlTest (Feature/Livewire) —
 * screen-100--data-browser-process-quality-control-web /
 * usecase-118--data-browser-process-quality-control-web.
 *
 * Component tests for App\Livewire\Data\DataBrowserProcessQualityControl,
 * mirroring tests/Feature/Livewire/DataBrowserClarificationTest.php's
 * structure exactly.
 */
...
it('success: shows filtered rows and export links after setting filter properties', function () {
    $recordWithFilledRows = ProcessQualityControlRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();
    ProcessQualityControlDetail::factory()->forRecord($recordWithFilledRows)->timeSlot('07:00')->filled()->create();
    ProcessQualityControlDetail::factory()->forRecord($recordWithFilledRows)->timeSlot('08:00')->filled()->create();

    ProcessQualityControlRecord::factory()->forStation($this->station)->onDate('2026-08-06')->create();
    ProcessQualityControlRecord::factory()->forStation($this->station)->onDate('2026-09-01')->create();

    Livewire::actingAs($this->user)
        ->test(DataBrowserProcessQualityControl::class)
...
it('Ekspor Gagal: export links are always built from the current filters', function () {
    Livewire::actingAs($this->user)
        ...
        ->assertViewHas('exportCsvUrl', fn ($url) => str_contains($url, '/api/process-quality-control-records/export')
...
it('Klik Baris Membuka Detail: rows render with a clickable-row link to the real detail route', function () {
    $record = ProcessQualityControlRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();

    Livewire::actingAs($this->user)
        ->test(DataBrowserProcessQualityControl::class)
        ->assertSeeHtml("onclick=\"window.location.href='".route('data.process-quality-control.detail', ['id' => $record->id])."'\"");
});
