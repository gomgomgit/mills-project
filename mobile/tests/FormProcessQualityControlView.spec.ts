/**
 * FormProcessQualityControlView.spec.ts — screen-080--form-process-quality-control /
 * usecase-116--form-process-quality-control.
 *
 * Mirrors FormClarificationView.spec.ts's structure/pattern exactly —
 * Process Quality Control follows the same hourly-grid, dynamic
 * add-row/remove-row pattern as Clarification, but this is the station
 * with the MOST non-time_slot columns in the whole project (16: Shift, 12
 * numeric readings, QC Inspector ID, QC Engineering Corrective Actions,
 * Findings), still NO enum status dropdowns, NO operational-target
 * reference table.
 *
 ...
 *   - required-field validation (Process QC ID)
 *   - "at least one valid row" validation (Time-Slot + reading filled,
 *     excluding Shift/QC Inspector ID)
 *   - Simpan success (status=saved), navigates to Monitor Process Quality
 *     Control
...
import FormProcessQualityControlView from '@/views/FormProcessQualityControlView.vue'
import {
...
vi.mock('@/services/processQualityControlRecordRepo', async () => {
  const actual = await vi.importActual<typeof import('@/services/processQualityControlRecordRepo')>(
    '@/services/processQualityControlRecordRepo',
  )
  return {
    ...actual,
    default: {
...
// Mirrors FormClarificationView.spec.ts's helpers exactly.
...
describe('FormProcessQualityControlView', () => {
  it('loads existing detail rows (however many) ordered by canonical time-slot on mount', async () => {
    ...
    const wrapper = mount(FormProcessQualityControlView)
    await flushPromises()

    const rows = wrapper.findAll('[data-testid="process-quality-control-detail-row"]')
  ...
  it('renders zero detail rows for a brand-new draft (no pre-created rows)', async () => {
    ...
    expect(wrapper.findAll('[data-testid="process-quality-control-detail-row"]')).toHaveLength(0)
  })
  ...
  it('adds one row at a time via "Tambah baris"', async () => {
    ...
    expect(wrapper.findAll('[data-testid="process-quality-control-detail-row"]')).toHaveLength(1)

    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    expect(wrapper.findAll('[data-testid="process-quality-control-detail-row"]')).toHaveLength(2)
  })
  ...
  it('shows inline error and does not save when Process QC ID is empty on Simpan', async () => {
    const wrapper = mount(FormProcessQualityControlView)
    await flushPromises()
    ...
    expect(saveDraftMock).not.toHaveBeenCalled()
    expect(wrapper.text()).toContain('Process QC ID wajib diisi')
  })
  ...
  it('shows a grid-specific error and does not save when there are zero valid detail rows', async () => {
    ...
    await wrapper.find('#field-process-qc-id').setValue('PQC-01')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()
  ...
  it('shows the grid error when a row has a selected time-slot but only Shift/QC Inspector ID filled', async () => {
    const wrapper = mount(FormProcessQualityControlView)
    await flushPromises()
    ...
    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    await chooseSearchableOption(wrapper, 'time-slot-select-0', '07:00')
    await wrapper.find('#shift-0').setValue('Shift 1')
    await wrapper.find('#qc-inspector-id-0').setValue('INS-01')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()
  ...
  it('saves successfully and navigates to Monitor Process Quality Control when Process QC ID and a valid detail row exist', async () => {
    ...
    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    await chooseSearchableOption(wrapper, 'time-slot-select-0', '07:00')
    await wrapper.find('#fruit-press-oil-loss-in-sludge-0').setValue('0.85')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()
    ...
    expect(headerData.process_qc_id).toBe('PQC-01')
    expect(details).toHaveLength(1)
    expect(details[0].time_slot).toBe('07:00')
    expect(idsToDelete).toEqual([])
    expect(pushMock).toHaveBeenCalledWith({ name: 'monitor-process-quality-control' })
  })

  it('falls back to the grid error (defense-in-depth) when the repo throws ProcessQualityControlDetailRequiredError', async () => {
    saveDraftMock.mockRejectedValue(new ProcessQualityControlDetailRequiredError())
    ...
    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    await chooseSearchableOption(wrapper, 'time-slot-select-0', '07:00')
    await wrapper.find('#fruit-press-oil-loss-in-sludge-0').setValue('1')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()

    expect(wrapper.find('[data-testid="detail-rows-error"]').exists()).toBe(true)
    expect(pushMock).not.toHaveBeenCalledWith({ name: 'monitor-process-quality-control' })
  })
  ...
  it('shows a confirm dialog on Back when the form is dirty', async () => {
    ...
    await wrapper.find('#field-process-qc-id').setValue('PQC-99')
