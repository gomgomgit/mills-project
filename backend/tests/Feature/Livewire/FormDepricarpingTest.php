...
/**
 * FormDepricarpingTest (Feature/Livewire) — screen-059--form-depricarping-web
 * / usecase-059--form-depricarping-web.
 *
 * Component tests for App\Livewire\Data\FormDepricarping, mirroring
 * tests/Feature/Livewire/FormPressingTest.php's structure. REVISED
 * 2026-08-24 (entity-catalog v12): the Depricarping Detail grid is now a
 ...
 * the original fixed 24-row design — adapted for Depricarping's own 8
...
beforeEach(function () {
    ...
    // FormDepricarping's render() queries DepricarpingOperationalTarget
...
it('mounts with zero detail rows for a brand-new draft (no pre-population)', function () {
    $component = Livewire::actingAs($this->supervisor)->test(FormDepricarping::class);
...
it('berhasil: creates a new record and redirects to Detail Depricarping', function () {
    $component = Livewire::actingAs($this->supervisor)->test(FormDepricarping::class);

    $component->set('form.production_line_id', $this->depricarpingStation->production_line_id);
    $component->set('form.presser_id', 'DP-LW-001');
    $component->call('addDetailRow');
    $component->set('detailRows.0.time_slot', '07:00');
    $component->set('detailRows.0.fan_static_pressure_mmh2o', '45');
    $component->call('save');

    expect(DepricarpingRecord::where('presser_id', 'DP-LW-001')->exists())->toBeTrue();
});

it('default: mode create renders empty form with Production Line dropdown', function () {
    Livewire::actingAs($this->supervisor)
        ->test(FormDepricarping::class)
...
it('date defaults to today but a manually-set value is preserved and saved', function () {
    ...
    $component->call('addDetailRow');
    $component->set('detailRows.0.time_slot', '07:00');
    $component->set('detailRows.0.fan_static_pressure_mmh2o', '10');
    $component->call('save');

    $record = DepricarpingRecord::where('presser_id', 'DP-LW-001')->first();
...
it('shows detail-specific error when there are zero valid detail rows on save', function () {
    ...
    $component->set('form.production_line_id', $this->depricarpingStation->production_line_id);
    $component->set('form.presser_id', 'DP-LW-002');
    $component->call('save');
...
it('accepts a row filled only via Downtime (Mins) or Findings as satisfying the minimum-one-row rule', function () {
    ...
    $component->set('form.production_line_id', $this->depricarpingStation->production_line_id);
    $component->set('form.presser_id', 'DP-LW-003');
    $component->call('addDetailRow');
    $component->set('detailRows.0.time_slot', '07:00');
    $component->set('detailRows.0.findings', 'Fan belt loose');
    $component->call('save');

    expect(DepricarpingRecord::where('presser_id', 'DP-LW-003')->exists())->toBeTrue();
});
...
it('accepts a row filled only via Downtime (Mins) as satisfying the minimum-one-row rule', function () {
    ...
    $component->set('form.production_line_id', $this->depricarpingStation->production_line_id);
    $component->set('form.presser_id', 'DP-LW-005');
    $component->call('addDetailRow');
    $component->set('detailRows.0.time_slot', '07:00');
    $component->set('detailRows.0.downtime_minutes', '15');
    $component->call('save');

    expect(DepricarpingRecord::where('presser_id', 'DP-LW-005')->exists())->toBeTrue();
});

it('shows an error when the selected Production Line has no active depricarping station', function () {
    $otherProductionLine = ProductionLine::factory()->create();
    $component = Livewire::actingAs($this->supervisor)->test(FormDepricarping::class);

    $component->set('form.production_line_id', $otherProductionLine->id);
    $component->set('form.presser_id', 'DP-LW-004');
    $component->call('addDetailRow');
    $component->set('detailRows.0.time_slot', '07:00');
    $component->set('detailRows.0.fan_static_pressure_mmh2o', '10');
    $component->call('save');
...
it('edit mode: prefills the form from the existing record and shows Station read-only', function () {
    $record = DepricarpingRecord::factory()->forStation($this->depricarpingStation)->create(['presser_id' => 'DP-EXISTING']);

    Livewire::actingAs($this->supervisor)
        ->test(FormDepricarping::class, ['id' => $record->id])
        ->assertSet('isEdit', true)
        ->assertSet('form.presser_id', 'DP-EXISTING')
...
it('edit mode: prefills existing detail rows with their ids (however many exist)', function () {
    ...
    $component = Livewire::actingAs($this->supervisor)->test(FormDepricarping::class, ['id' => $record->id]);
...
it('edit mode: saving updates the existing record', function () {
    ...
    $component = Livewire::actingAs($this->supervisor)->test(FormDepricarping::class, ['id' => $record->id]);
    $component->set('form.presser_id', 'DP-UPDATED');
    $component->set('detailRows.0.fan_static_pressure_mmh2o', '46');
    $component->call('save');

    expect($record->fresh()->presser_id)->toBe('DP-UPDATED');
});

it('edit mode: removing a row via Hapus deletes it from the persisted record on save', function () {
    $record = DepricarpingRecord::factory()->forStation($this->depricarpingStation)->create();
    DepricarpingDetail::factory()->forRecord($record)->timeSlot('07:00')->filled()->create();
    $toRemove = DepricarpingDetail::factory()->forRecord($record)->timeSlot('09:00')->filled()->create();

    $component = Livewire::actingAs($this->supervisor)->test(FormDepricarping::class, ['id' => $record->id]);
    $component->call('removeDetailRow', 1);
    $component->call('save');

    expect(DepricarpingDetail::find($toRemove->id))->toBeNull();
    expect(DepricarpingDetail::where('depricarping_record_id', $record->id)->count())->toBe(1);
});

it('edit mode: shows record-not-found error for an invalid id', function () {
    Livewire::actingAs($this->supervisor)
        ->test(FormDepricarping::class, ['id' => '00000000-0000-0000-0000-000000000000'])
        ->assertSet('notFound', true)
        ->assertSee('Record tidak ditemukan');
});

it('Checked checkbox only renders for Supervisor', function () {
    Livewire::actingAs($this->supervisor)->test(FormDepricarping::class)->assertSeeHtml('data-testid="checked-checkbox"');
    Livewire::actingAs($this->millManagement)->test(FormDepricarping::class)->assertDontSeeHtml('data-testid="checked-checkbox"');
});

it('Acknowledged checkbox only renders for Mill Management', function () {
    Livewire::actingAs($this->supervisor)->test(FormDepricarping::class)->assertDontSeeHtml('data-testid="acknowledged-checkbox"');
    Livewire::actingAs($this->millManagement)->test(FormDepricarping::class)->assertSeeHtml('data-testid="acknowledged-checkbox"');
});

it('renders the Target Operasional reference table with 6 rows and 4 columns', function () {
    Livewire::actingAs($this->supervisor)
        ->test(FormDepricarping::class)
        ->assertSeeHtml('data-testid="operational-target-table"')
        ->assertSee('Fan Static Pressure')
        ->assertSee('40 - 50 mmH2O')
        ->assertSee('Operational Consequence / Justification');
});
