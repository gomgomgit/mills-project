/**
 * threshingRecordRepo.spec.ts — screen-037--monitor-threshing /
 * screen-041--form-threshing / screen-045--data-preview-threshing.
 *
 * Unit tests for src/services/threshingRecordRepo.ts — covers the
 * authoritative unit_test_cases for these 3 screens' repo layer (mobile-only
 * / local-SQLite-only, no REST api_contract). Mirrors
 * cagesTrackRecordRepo.spec.ts's structure/pattern, adapted for
 * Threshing's DYNAMIC add-row/remove-row detail grid (REVISED 2026-08-24,
 * entity-catalog v12 — replaces the original FIXED 24-row design):
 * createDraft() inserts ONLY the header, saveDraft()/
 * pauseDraftWithFormData() upsert/delete threshing_detail rows via
 * applyDetailRowChanges(), mirroring cagesTrackRecordRepo.ts's
...
function makeHeaderData(overrides: Partial<ThreshingHeaderFormData> = {}): ThreshingHeaderFormData {
  return {
    thresher_id: 'TH-01',
...
function makeDetailFormRow(overrides: Partial<ThreshingDetailFormRow> = {}): ThreshingDetailFormRow {
  return {
    time_slot: '07:00',
    ffb_throughput_mt_hour: null,
    thresher_drum_speed_rpm: null,
...
function makeValidDetailRows(overrides: Partial<ThreshingDetailFormRow>[] = []): ThreshingDetailFormRow[] {
  if (overrides.length === 0) {
    return [makeDetailFormRow({ time_slot: '07:00', ffb_throughput_mt_hour: 45.5 })]
  }

  return overrides.map((o) => makeDetailFormRow(o))
}

describe('threshingRecordRepo', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  describe('createDraft()', () => {
    it('inserts a new draft_ongoing threshing_record with date auto-filled, and inserts NO threshing_detail rows', async () => {
      ...
      expect(headerSql).toContain('INSERT INTO threshing_record')
  ...
  describe('getDrafts()', () => {
    it('returns the draft/pause list, most-recently-updated first', async () => {
      const rows = [
        { id: 'draft-newest', status: 'draft_paused' as const, thresher_id: 'TH-002', updated_at: '2026-08-24T10:00:00.000Z' },
        { id: 'draft-oldest', status: 'draft_ongoing' as const, thresher_id: 'TH-001', updated_at: '2026-08-24T08:00:00.000Z' },
      ]
  ...
  describe('getTodaySummary()', () => {
    it("computes today's record count and a plain COUNT of threshing_detail rows belonging to those records (not filtered by filled-ness)", async () => {
      vi.mocked(query)
      ...
      expect(result).toEqual({ countThreshingRecord: 2, detailRowCount: 5 })
      expect(query).toHaveBeenNthCalledWith(1, expect.stringContaining('FROM threshing_record'), [USER_ID])
      ...
      expect(countSql).toContain('FROM threshing_detail')
      expect(countSql).toContain('WHERE threshing_record_id IN (?, ?)')
    ...
    it('returns zero counters and skips the detail query when no records match today', async () => {
      ...
      expect(result).toEqual({ countThreshingRecord: 0, detailRowCount: 0 })
  ...
  describe('getAllRecords()', () => {
    it('returns every record for the user regardless of status', async () => {
      ...
      expect(result).toEqual(rows)
      expect(query).toHaveBeenCalledWith(expect.stringContaining('FROM threshing_record WHERE created_by = ?'), [
        USER_ID,
      ])
      expect(query).toHaveBeenCalledWith(expect.stringContaining('ORDER BY updated_at DESC'), [USER_ID])
    })
  ...
  describe('saveDraft()', () => {
    it('throws ThreshingDetailRequiredError and calls neither run() nor query() when details is empty', async () => {
    ...
    it('throws ThreshingDetailRequiredError when every row lacks a selected time_slot or has zero reading columns filled', async () => {
      const headerData = makeHeaderData()
      const details: ThreshingDetailFormRow[] = [
    ...
    it('treats a row with only downtime_reason filled as satisfying the minimum-one-row rule', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      vi.mocked(query)
      ...
      const headerData = makeHeaderData()
      const details = [makeDetailFormRow({ time_slot: '07:00', downtime_reason: 'Breakdown' })]
    ...
    it('updates the header with status=saved, INSERTs a new (id-less) detail row, and returns the freshly saved draft', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      const saved = makeRecord({ id: 'draft-4', thresher_id: 'TH-01', status: 'saved' })
      const savedDetails = [makeDetailRow({ id: 'detail-new-1', threshing_record_id: 'draft-4' })]
      vi.mocked(query).mockResolvedValueOnce([saved]).mockResolvedValueOnce(savedDetails)

      const headerData = makeHeaderData()
      const details = makeValidDetailRows([{ time_slot: '09:00', ffb_throughput_mt_hour: 45.5 }])
      ...
      const [headerSql, headerParams] = vi.mocked(run).mock.calls[0]
      expect(headerSql).toContain('UPDATE threshing_record')
      expect(headerSql).toContain("status = 'saved'")
      expect(headerParams).toEqual(
        expect.arrayContaining(['TH-01', expect.any(String), null, null, expect.any(String), 'draft-4']),
      )

      const [detailSql, detailParams] = vi.mocked(run).mock.calls[1]
      expect(detailSql).toContain('INSERT INTO threshing_detail')
    ...
    it('UPDATEs an existing detail row (has an id) instead of inserting it', async () => {
      ...
      const headerData = makeHeaderData()
      const details: ThreshingDetailFormRow[] = [
        { id: 'detail-existing-1', time_slot: '11:00', ffb_throughput_mt_hour: 50, thresher_drum_speed_rpm: null, motor_current_amps: null, unstripped_bunch_count_percent: null, empty_bunch_oil_loss_percent: null, downtime_reason: null },
      ]
      ...
      expect(run).toHaveBeenCalledTimes(2)
      const [detailSql, detailParams] = vi.mocked(run).mock.calls[1]
      expect(detailSql).toContain('UPDATE threshing_detail')
      expect(detailParams).toEqual(['11:00', 50, null, null, null, null, null, expect.any(String), 'detail-existing-1'])
    })
    ...
    it('upserts multiple detail rows sequentially, in details order (mixed insert + update)', async () => {
      ...
      const headerData = makeHeaderData()
      const details: ThreshingDetailFormRow[] = [
        { id: 'detail-existing-2', time_slot: '07:00', ffb_throughput_mt_hour: 1, thresher_drum_speed_rpm: null, motor_current_amps: null, unstripped_bunch_count_percent: null, empty_bunch_oil_loss_percent: null, downtime_reason: null },
        makeDetailFormRow({ time_slot: '13:00', ffb_throughput_mt_hour: 2 }),
      ]
      ...
      expect(firstRowSql).toContain('UPDATE threshing_detail')
      expect(secondRowSql).toContain('INSERT INTO threshing_detail')
    })
    ...
    it('DELETEs every id in detailIdsToDelete, after the upserts', async () => {
      ...
      expect(deleteSql1).toContain('DELETE FROM threshing_detail WHERE id = ?')
      expect(deleteParams1).toEqual(['detail-removed-1'])
      expect(deleteSql2).toContain('DELETE FROM threshing_detail WHERE id = ?')
    ...
    it('nulls/strips checked_by for a non-supervisor role', async () => {
      ...
      // checked_by is the 4th positional SET param (index 3): thresher_id,
    ...
    it('throws when getDraftWithDetails() re-read after save unexpectedly finds nothing', async () => {
      ...
      await expect(saveDraft('draft-8', headerData, details, [], 'operator')).rejects.toThrow(
        'Gagal memuat ulang data threshing setelah disimpan.',
      )
    })
  })
  ...
  describe('pauseDraftWithFormData()', () => {
    it('saves with status=draft_paused even when details is empty (no validation gate)', async () => {
      ...
      const headerData = makeHeaderData({ thresher_id: '' })
    ...
    it('upserts detail rows and applies pending deletions, same contract as saveDraft()', async () => {
      ...
      expect(deleteSql).toContain('DELETE FROM threshing_detail WHERE id = ?')
    ...
    it('throws when getDraftWithDetails() re-read after pause unexpectedly finds nothing', async () => {
      ...
      await expect(pauseDraftWithFormData('draft-1', headerData, [], [], 'operator')).rejects.toThrow(
        'Gagal menyimpan progres threshing setelah pause.',
      )
    })
  })

  describe('deleteDraft()', () => {
    it('deletes threshing_detail rows before deleting the threshing_record row', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      ...
      expect(run).toHaveBeenNthCalledWith(
        1,
        expect.stringContaining('DELETE FROM threshing_detail WHERE threshing_record_id = ?'),
        ['draft-1'],
      )
      expect(run).toHaveBeenNthCalledWith(2, expect.stringContaining('DELETE FROM threshing_record WHERE id = ?'), [
        'draft-1',
      ])
    })
  })
