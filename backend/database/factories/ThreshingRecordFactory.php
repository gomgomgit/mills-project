...
/**
 * @extends Factory<ThreshingRecord>
 *
 ...
 * Mirrors CagesTrackRecordFactory.php's exact structure/conventions —
 * ThreshingRecord shares the same "minimal satu related row before
 * status=saved" constraint shape (ThreshingRecord::booted()).
 *
 * Default status is RecordStatus::Synced, deliberately NOT ::Saved: a
 * record created fresh via this factory (no related ThreshingDetail rows
 * exist yet) would fail the `saving` guard on every ->create() call if
 * ::Saved were the default.
 */
class ThreshingRecordFactory extends Factory
{
    protected $model = ThreshingRecord::class;
    ...
    public function definition(): array
    {
        ...
        return [
            ...
            'thresher_id' => 'TH-'.$this->faker->unique()->numerify('####'),
