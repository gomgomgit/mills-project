/**
 * processQualityControlRecordRepo.spec.ts — screen-070--monitor-process-quality-control /
 * screen-080--form-process-quality-control / screen-090--data-preview-process-quality-control.
 *
 * Unit tests for src/services/processQualityControlRecordRepo.ts — covers
 * the authoritative unit_test_cases for these 3 screens' repo layer
 * (mobile-only / local-SQLite-only, no REST api_contract). Mirrors
 * clarificationRecordRepo.spec.ts's structure/pattern exactly, since
 * Process Quality Control follows the same hourly-grid (dynamic
 * add-row/remove-row, 1..24 rows) pattern as Clarification/Boiler Room —
 * except this station has the MOST non-time_slot columns in the whole
 * project: 16 total (Shift, 12 numeric readings, QC Inspector ID, QC
 * Engineering Corrective Actions, Findings), NO enum columns. `shift` and
...
function makeRecord(overrides: Partial<ProcessQualityControlRecord> = {}): ProcessQualityControlRecord {
  return {
    id: 'draft-1',
...
function makeDetailRow(overrides: Partial<ProcessQualityControlDetailRow> = {}): ProcessQualityControlDetailRow {
  return {
    id: 'detail-1',
    process_quality_control_record_id: 'draft-1',
    time_slot: '07:00',
    shift: null,
...
function makeHeaderData(
  overrides: Partial<ProcessQualityControlHeaderFormData> = {},
): ProcessQualityControlHeaderFormData {
  return {
    process_qc_id: 'PQC-01',
...
function makeDetailFormRow(
  overrides: Partial<ProcessQualityControlDetailFormRow> = {},
): ProcessQualityControlDetailFormRow {
  return {
    time_slot: '07:00',
    shift: null,
    fruit_press_oil_loss_in_sludge_percent: null,
...
function makeValidDetailRows(
  overrides: Partial<ProcessQualityControlDetailFormRow>[] = [],
): ProcessQualityControlDetailFormRow[] {
  if (overrides.length === 0) {
    return [makeDetailFormRow({ time_slot: '07:00', fruit_press_oil_loss_in_sludge_percent: 0.85 })]
  }

  return overrides.map((o) => makeDetailFormRow(o))
}

// Positional order of the 16 non-time_slot detail columns as written by
// applyDetailRowChanges() — used to build expected INSERT/UPDATE param
// arrays without repeating all 16 `null`s by hand in every test.
function readingParams(overrides: Partial<ProcessQualityControlDetailFormRow> = {}): (string | number | null)[] {
...
describe('processQualityControlRecordRepo', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  describe('createDraft()', () => {
    it('inserts a new draft_ongoing process_quality_control_record with date auto-filled, and inserts NO process_quality_control_detail rows', async () => {
      ...
      expect(headerSql).toContain('INSERT INTO process_quality_control_record')
  ...
  describe('getDrafts()', () => {
    it('returns the draft/pause list, most-recently-updated first', async () => {
      const rows = [
        { id: 'draft-newest', status: 'draft_paused' as const, process_qc_id: 'PQC-002', updated_at: '2026-08-31T10:00:00.000Z' },
        { id: 'draft-oldest', status: 'draft_ongoing' as const, process_qc_id: 'PQC-001', updated_at: '2026-08-31T08:00:00.000Z' },
      ]
  ...
  describe('getTodaySummary()', () => {
    it("computes today's record count and a plain COUNT of process_quality_control_detail rows belonging to those records (not filtered by filled-ness)", async () => {
      vi.mocked(query)
      ...
      expect(result).toEqual({ countProcessQualityControlRecord: 2, detailRowCount: 5 })
      expect(query).toHaveBeenNthCalledWith(1, expect.stringContaining('FROM process_quality_control_record'), [
        USER_ID,
      ])
      ...
      expect(countSql).toContain('FROM process_quality_control_detail')
      expect(countSql).toContain('WHERE process_quality_control_record_id IN (?, ?)')
    ...
    it('returns zero counters and skips the detail query when no records match today', async () => {
      ...
      expect(result).toEqual({ countProcessQualityControlRecord: 0, detailRowCount: 0 })
  ...
  describe('getAllRecords()', () => {
    it('returns every record for the user regardless of status', async () => {
      ...
      expect(query).toHaveBeenCalledWith(
        expect.stringContaining('FROM process_quality_control_record WHERE created_by = ?'),
        [USER_ID],
      )
      expect(query).toHaveBeenCalledWith(expect.stringContaining('ORDER BY updated_at DESC'), [USER_ID])
    })
  ...
  describe('saveDraft()', () => {
    it('throws ProcessQualityControlDetailRequiredError and calls neither run() nor query() when details is empty', async () => {
    ...
    it('throws ProcessQualityControlDetailRequiredError when every row lacks a selected time_slot or has zero of the 14 reading/keterangan columns filled', async () => {
      const headerData = makeHeaderData()
      const details: ProcessQualityControlDetailFormRow[] = [
        makeDetailFormRow({ time_slot: null }),
        makeDetailFormRow({ time_slot: '08:00', shift: 'Shift 1', qc_inspector_id: 'INS-01' }), // identity columns only
      ]
    ...
    it('treats a row with only findings filled as satisfying the minimum-one-row rule, even with shift/qc_inspector_id empty', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      vi.mocked(query)
    ...
    it('updates the header with status=saved, INSERTs a new (id-less) detail row, and returns the freshly saved draft', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      const saved = makeRecord({ id: 'draft-4', process_qc_id: 'PQC-01', status: 'saved' })
      const savedDetails = [makeDetailRow({ id: 'detail-new-1', process_quality_control_record_id: 'draft-4' })]
      vi.mocked(query).mockResolvedValueOnce([saved]).mockResolvedValueOnce(savedDetails)

      const headerData = makeHeaderData()
      const details = makeValidDetailRows([{ time_slot: '09:00', fruit_press_oil_loss_in_sludge_percent: 0.6 }])
      ...
      const [headerSql, headerParams] = vi.mocked(run).mock.calls[0]
      expect(headerSql).toContain('UPDATE process_quality_control_record')
      expect(headerSql).toContain("status = 'saved'")
      expect(headerParams).toEqual(
        expect.arrayContaining(['PQC-01', expect.any(String), null, null, expect.any(String), 'draft-4']),
      )

      const [detailSql, detailParams] = vi.mocked(run).mock.calls[1]
      expect(detailSql).toContain('INSERT INTO process_quality_control_detail')
      expect(detailParams).toEqual([
        expect.any(String), // freshly generated detail id
        'draft-4',
        '09:00',
        ...readingParams({ time_slot: '09:00', fruit_press_oil_loss_in_sludge_percent: 0.6 }),
        expect.any(String),
        expect.any(String),
      ])
    ...
    it('UPDATEs an existing detail row (has an id) instead of inserting it', async () => {
      ...
      const headerData = makeHeaderData()
      const details: ProcessQualityControlDetailFormRow[] = [
        { ...makeDetailFormRow(), id: 'detail-existing-1', time_slot: '11:00', fruit_press_oil_loss_in_sludge_percent: 0.7 },
      ]
      ...
      expect(run).toHaveBeenCalledTimes(2)
      const [detailSql, detailParams] = vi.mocked(run).mock.calls[1]
      expect(detailSql).toContain('UPDATE process_quality_control_detail')
      expect(detailParams).toEqual([
        '11:00',
        ...readingParams({ time_slot: '11:00', fruit_press_oil_loss_in_sludge_percent: 0.7 }),
        expect.any(String),
        'detail-existing-1',
      ])
    })
    ...
    it('upserts multiple detail rows sequentially, in details order (mixed insert + update)', async () => {
      ...
      const headerData = makeHeaderData()
      const details: ProcessQualityControlDetailFormRow[] = [
        { ...makeDetailFormRow(), id: 'detail-existing-2', time_slot: '07:00', fruit_press_oil_loss_in_sludge_percent: 0.1 },
        makeDetailFormRow({ time_slot: '13:00', fruit_press_oil_loss_in_sludge_percent: 0.2 }),
      ]
      ...
      expect(firstRowSql).toContain('UPDATE process_quality_control_detail')
      expect(secondRowSql).toContain('INSERT INTO process_quality_control_detail')
    })
    ...
    it('DELETEs every id in detailIdsToDelete, after the upserts', async () => {
      ...
      expect(deleteSql1).toContain('DELETE FROM process_quality_control_detail WHERE id = ?')
      expect(deleteParams1).toEqual(['detail-removed-1'])
      expect(deleteSql2).toContain('DELETE FROM process_quality_control_detail WHERE id = ?')
    ...
    it('nulls/strips checked_by for a non-supervisor role', async () => {
      ...
      // checked_by is the 4th positional SET param (index 3): process_qc_id,
    ...
    it('throws when getDraftWithDetails() re-read after save unexpectedly finds nothing', async () => {
      ...
      await expect(saveDraft('draft-8', headerData, details, [], 'operator')).rejects.toThrow(
        'Gagal memuat ulang data process quality control setelah disimpan.',
      )
    })
  })
  ...
  describe('pauseDraftWithFormData()', () => {
    it('saves with status=draft_paused even when details is empty (no validation gate)', async () => {
      ...
      const headerData = makeHeaderData({ process_qc_id: '' })
    ...
    it('upserts detail rows and applies pending deletions, same contract as saveDraft()', async () => {
      ...
      expect(deleteSql).toContain('DELETE FROM process_quality_control_detail WHERE id = ?')
    ...
    it('throws when getDraftWithDetails() re-read after pause unexpectedly finds nothing', async () => {
      ...
      await expect(pauseDraftWithFormData('draft-1', headerData, [], [], 'operator')).rejects.toThrow(
        'Gagal menyimpan progres process quality control setelah pause.',
      )
    })
  })

  describe('deleteDraft()', () => {
    it('deletes process_quality_control_detail rows before deleting the process_quality_control_record row', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      ...
      expect(run).toHaveBeenNthCalledWith(
        1,
        expect.stringContaining('DELETE FROM process_quality_control_detail WHERE process_quality_control_record_id = ?'),
        ['draft-1'],
      )
      expect(run).toHaveBeenNthCalledWith(
        2,
        expect.stringContaining('DELETE FROM process_quality_control_record WHERE id = ?'),
        ['draft-1'],
      )
    })
  })
