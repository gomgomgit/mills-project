...
/**
 * DataBrowserDepricarping — screen-051--data-browser-depricarping-web /
 * "Data Browser Depricarping" (Livewire web page, route name
 * `data.depricarping`, /data/depricarping).
 *
 * Reuses DepricarpingRecordService — the exact same service the API
 * controller (App\Http\Controllers\Api\DepricarpingRecordController) uses —
 * so filtering/pagination rules stay identical between the web and API
 * entry points (mirrors screen-050's DataBrowserPressing pattern).
 *
 * Filter state mirrors the tech-spec exactly: date_from, date_to,
 * business_unit_id.
 *
 * Pagination: calls DepricarpingRecordService::listRecords() directly and
 * keeps its own $page/$perPage state, rather than Livewire's
 * WithPagination trait — same approach as DataBrowserPressing.
 *
 * Export: dispatched as a plain browser-navigable link (GET
 * /api/depricarping-records/export?...), NOT an AJAX/blob download —
 * session cookie carries auth on a same-origin navigation.
 */
#[Layout('data.depricarping')]
class DataBrowserDepricarping extends Component
{
    ...
    protected function exportUrl(string $format): string
    {
        ...
        return url('/api/depricarping-records/export').'?'.http_build_query($query);
    }
    ...
    public function render()
    {
        ...
        return view('livewire.data.data-browser-depricarping', [
            'records' => $result['data'],
            'meta' => $result['meta'],
