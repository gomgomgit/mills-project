...
/**
 * MonitorProcessWaterView — screen-062--monitor-process-water /
 * usecase-067--monitor-process-water (mounted at
 * /stations/process-water/monitor, meta.public = false — requires an
 ...
 * local (offline) `process_water_record` table (and its child
 * `process_water_detail` table, for the "filled slot" counter) via
 * processWaterRecordRepo.ts. api_contracts.endpoints is empty for this
 * screen.
 *
 ...
 * deleteDraft() calls) — those actions live in Form Process Water
 * (screen-072).
 *
 * Counter card labels per this screen's tech spec: "Jumlah Process Water
 * Record" (COUNT of today's process_water_record rows, any status) /
 * "Jumlah Baris Time-Slot Tercatat" (COUNT of process_water_detail rows
 ...
 * (Home > Production Process Activity > Process Water, the first two
 * tappable).
 *
 * Navigation:
 *  - 'New Data' -> processWaterRecordRepo.createDraft(userId) (INSERTs a
 *    new status='draft_ongoing' header row ONLY — no process_water_detail
 *    rows are pre-created, see that function's doc comment), then ->
 *    route name `monitor-process-water`'s sibling `process-water-form`
 *    with the new draft's id.
 *  - Tapping a list item -> route name `process-water-form` with that
 *    item's EXISTING id, directly — no status-transition call.
 *  - 'Load Data' -> route name `data-preview-process-water` with NO id
 *    param (list mode).
...
import {
  ...
  type ProcessWaterDraftListItem,
  type ProcessWaterTodaySummary,
...
const drafts = ref<ProcessWaterDraftListItem[]>([])
...
// today.
const todaySummary = ref<ProcessWaterTodaySummary>({ countProcessWaterRecord: 0, detailRowCount: 0 })
...
async function loadDrafts() {
  ...
  try {
    drafts.value = await processWaterRecordRepo.getDrafts(userId)
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal memuat daftar draft process water lokal.'
  } finally {
    loading.value = false
  }
}
...
async function loadTodaySummary() {
  ...
  try {
    todaySummary.value = await processWaterRecordRepo.getTodaySummary(userId)
  } catch {
...
/**
 * Process Water ID fallback text for a draft whose `process_water_id` has
 * not been filled in yet.
 */
function draftLabel(draft: ProcessWaterDraftListItem): string {
  return draft.process_water_id?.trim() ? draft.process_water_id : 'Process Water ID belum diisi'
}
...
async function onNewData() {
  ...
  try {
    const id = await processWaterRecordRepo.createDraft(userId)
    router.push({ name: 'process-water-form', params: { id } })
  } catch (err) {
    error.value = err instanceof Error ? err.message : 'Gagal membuat draft process water baru.'
  }
}
...
function onDraftClick(draft: ProcessWaterDraftListItem) {
  router.push({ name: 'process-water-form', params: { id: draft.id } })
}
...
// business_logic step 8 — breadcrumb taps. 'Process Water' is the current
...
<template>
  <main class="monitor-process-water-view">
    ...
    <div class="monitor-process-water-header">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="breadcrumb-separator" aria-hidden="true">/</span>
        <span class="breadcrumb-current" aria-current="page">Process Water</span>
      </nav>
      <h1 class="screen-title">Monitor Process Water</h1>
    </div>
    ...
    <section class="today-summary-section" aria-label="Hari Ini">
      ...
      <div class="today-summary-grid">
        <div class="today-summary-card">
          <span class="today-summary-label">Jumlah Process Water Record</span>
          <span class="today-summary-value" data-testid="counter-count-process-water-record">{{ todaySummary.countProcessWaterRecord }}</span>
        </div>
        <div class="today-summary-card">
    ...
    <p v-if="loading" class="status-text">Memuat daftar draft process water lokal…</p>
    <p v-else-if="error" class="status-text status-text--error" role="alert">{{ error }}</p>

    <section v-else class="draft-list-section" aria-label="Daftar Draft Process Water">
      <p v-if="drafts.length === 0" class="empty-state" data-testid="draft-list-empty">
        Belum ada draft process water tersimpan.
      </p>
