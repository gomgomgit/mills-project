...
/**
 * DataBrowserEffluentPlantTest (Feature/Livewire) —
 * screen-095--data-browser-effluent-plant-web /
 * usecase-088--data-browser-effluent-plant-web.
 *
 * Component tests for App\Livewire\Data\DataBrowserEffluentPlant, mirroring
 * tests/Feature/Livewire/DataBrowserThreshingTest.php's structure exactly.
 */
...
it('success: shows filtered rows and export links after setting filter properties', function () {
    $recordWithFilledRows = EffluentPlantRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();
    EffluentPlantDetail::factory()->forRecord($recordWithFilledRows)->timeSlot('07:00')->filled()->create();
    EffluentPlantDetail::factory()->forRecord($recordWithFilledRows)->timeSlot('08:00')->filled()->create();

    EffluentPlantRecord::factory()->forStation($this->station)->onDate('2026-08-06')->create();
    EffluentPlantRecord::factory()->forStation($this->station)->onDate('2026-09-01')->create();

    Livewire::actingAs($this->user)
        ->test(DataBrowserEffluentPlant::class)
...
it('Ekspor Gagal: export links are always built from the current filters', function () {
    Livewire::actingAs($this->user)
        ...
        ->assertViewHas('exportCsvUrl', fn ($url) => str_contains($url, '/api/effluent-plant-records/export')
...
it('Klik Baris Membuka Detail: rows render with a clickable-row link to the real detail route', function () {
    $record = EffluentPlantRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();

    Livewire::actingAs($this->user)
        ->test(DataBrowserEffluentPlant::class)
        ->assertSeeHtml("onclick=\"window.location.href='".route('data.effluent-plant.detail', ['id' => $record->id])."'\"");
});
