...
/**
 * DataBrowserClarificationTest (Feature/Livewire) —
 * screen-099--data-browser-clarification-web /
 * usecase-112--data-browser-clarification-web.
 *
 * Component tests for App\Livewire\Data\DataBrowserClarification, mirroring
 * tests/Feature/Livewire/DataBrowserBoilerRoomTest.php's structure
 * exactly.
 */
...
it('success: shows filtered rows and export links after setting filter properties', function () {
    $recordWithFilledRows = ClarificationRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();
    ClarificationDetail::factory()->forRecord($recordWithFilledRows)->timeSlot('07:00')->filled()->create();
    ClarificationDetail::factory()->forRecord($recordWithFilledRows)->timeSlot('08:00')->filled()->create();

    ClarificationRecord::factory()->forStation($this->station)->onDate('2026-08-06')->create();
    ClarificationRecord::factory()->forStation($this->station)->onDate('2026-09-01')->create();

    Livewire::actingAs($this->user)
        ->test(DataBrowserClarification::class)
...
it('Ekspor Gagal: export links are always built from the current filters', function () {
    Livewire::actingAs($this->user)
        ...
        ->assertViewHas('exportCsvUrl', fn ($url) => str_contains($url, '/api/clarification-records/export')
...
it('Klik Baris Membuka Detail: rows render with a clickable-row link to the real detail route', function () {
    $record = ClarificationRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();

    Livewire::actingAs($this->user)
        ->test(DataBrowserClarification::class)
        ->assertSeeHtml("onclick=\"window.location.href='".route('data.clarification.detail', ['id' => $record->id])."'\"");
});
