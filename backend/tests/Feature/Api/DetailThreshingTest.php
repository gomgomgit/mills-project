...
/**
 * DetailThreshingTest (Feature/Api) — screen-053--detail-threshing-web /
 * usecase-053--detail-threshing-web.
 *
 * Integration tests for GET /api/threshing-records/{id}, mirroring
 * tests/Feature/Api/DetailCagesTrackTest.php's structure exactly.
 */
...
it('berhasil: returns the full record with resolved names and its details grid', function () {
    ...
    ThreshingDetail::factory()->forRecord($record)->timeSlot('09:00')->filled()->create();

    $response = $this->actingAs($this->supervisor, 'web')->getJson("/api/threshing-records/{$record->id}");
...
it('returns details ordered by canonical time-slot, not creation order', function () {
    $record = ThreshingRecord::factory()->forStation($this->station)->create();
    ThreshingDetail::factory()->forRecord($record)->timeSlot('14:00')->create();
    ThreshingDetail::factory()->forRecord($record)->timeSlot('08:00')->create();
    ThreshingDetail::factory()->forRecord($record)->timeSlot('00:00')->create();

    $response = $this->actingAs($this->admin, 'web')->getJson("/api/threshing-records/{$record->id}");
...
it('Mill Management and Admin can also access the detail endpoint', function () {
    ...
    $this->actingAs($this->millManagement, 'web')->getJson("/api/threshing-records/{$record->id}")->assertOk();
    $this->actingAs($this->admin, 'web')->getJson("/api/threshing-records/{$record->id}")->assertOk();
});

it('returns 403 for the Operator role (route-level role gate)', function () {
    $record = ThreshingRecord::factory()->forStation($this->station)->create();

    $this->actingAs($this->operator, 'web')->getJson("/api/threshing-records/{$record->id}")->assertStatus(403);
});

it('returns 404 when the id does not exist', function () {
    $this->actingAs($this->admin, 'web')->getJson('/api/threshing-records/00000000-0000-0000-0000-000000000000')->assertStatus(404);
});
...
it('rejects unauthenticated requests', function () {
    ...
    $this->getJson("/api/threshing-records/{$record->id}")->assertStatus(401);
});
