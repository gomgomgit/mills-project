 ...
 * EngineRoomRecordController) and the Livewire components (App\Livewire\
 * Data\{DataBrowserEngineRoom,DetailEngineRoom,FormEngineRoom}), mirroring
 * StorageTankRecordService's structure exactly — Engine Room follows the
 * same "hourly grid" pattern as Storage Tank/Effluent Plant/Process Water/
 ...
 *
 * UNLIKE Threshing/Pressing/Depricarping/Kernel Plant: Engine Room has NO
 ...
 * engine_room_detail row counts as filled when at least one of its 27
 * non-time_slot columns is non-null — like Storage Tank, this station has
 * NO identifying/context columns (no shift/inspector_id equivalent), so all
 * 27 remaining columns are reading/status/text columns and all participate
 * in the "filled" check. This is the LARGEST field count of any station in
 * this project (task brief originally said 26; the migration — ground
 * truth — has 27; corrected here as a derived assumption).
 *
 * IMPORTANT — enum empty-string-vs-null: 2 of the 27 columns
 * (diesel_gen_1_status, diesel_gen_2_status) are backed by a SQLite CHECK
 * constraint (Laravel enum()) that rejects an empty string.
 * normalizeDetails() coerces '' to null for these columns specifically
 * before validation/persistence — the same mitigation Kernel Dispatch/
 * Effluent Plant/Storage Tank needed for their own enum columns.
 */
class EngineRoomRecordService
{
    public const EXPORT_ROW_LIMIT = 50000;

    protected const FORM_FIELDS = ['engine_room_id', 'date', 'note'];
     ...
     * engineRoomRecordRepo.ts's canonicalTimeSlots() exactly (`for i in
     ...
     * engine_room_detail rows inside a DB transaction.
     */
    public function create(array $data, User $actor): array
    {
        ...
        $record = DB::transaction(function () use ($attributes, $details) {
            // EngineRoomRecord may carry the same "minimal satu detail row
        ...
        $record->load(['station', 'createdBy', 'checkedBy', 'acknowledgedBy', 'engineRoomDetails']);
     ...
     * then upsert its engine_room_detail rows (insert rows without an id,
     ...
     * StorageTankRecordService::update() exactly.
     */
    public function update(string $id, array $data, User $actor): array
    {
        $record = EngineRoomRecord::findOrFail($id);
        ...
        return collect($rawDetails)
            ->map(function ($row) {
                ...
                foreach (self::DETAIL_FIELDS as $field) {
                    ...
                    // ''. Same fix Kernel Dispatch/Effluent Plant/Storage
                    // Tank needed for their own enum columns.
    ...
    protected function validateForm(array $attributes): void
    {
        Validator::make($attributes, [
            'engine_room_id' => ['required', 'string'],
            'date' => ['required', 'date'],
            'note' => ['nullable', 'string'],
        ], [
            'engine_room_id.required' => 'Engine Room ID wajib diisi.',
     ...
     * StorageTankRecordService::validateDetails() exactly.
     */
    protected function validateDetails(array $details): void
    {
        ...
        if ($validRows->isEmpty()) {
            throw ValidationException::withMessages([
                'details' => 'Minimal satu baris Engine Room Detail (Time-Slot terpilih + minimal 1 kolom bacaan terisi) harus diisi.',
            ]);
        }
        ...
        if ($slots->contains(fn ($slot) => ! array_key_exists($slot, $canonicalOrder))) {
            throw ValidationException::withMessages([
                'details' => 'Time-Slot Engine Room Detail harus salah satu dari 24 slot kanonis (07:00-06:00).',
            ]);
        }

        if ($slots->duplicates()->isNotEmpty()) {
            throw ValidationException::withMessages([
                'details' => 'Time-Slot tiap baris Engine Room Detail tidak boleh duplikat.',
            ]);
        }
        ...
        foreach ($sortedIndexes as $index => $slotIndex) {
            if ($index > 0 && $slotIndex <= $sortedIndexes[$index - 1]) {
                throw ValidationException::withMessages([
                    'details' => 'Time-Slot tiap baris Engine Room Detail harus lebih besar dari baris sebelumnya (urutan menaik).',
                ]);
            }
        }
    }
     ...
     * 27 non-time_slot columns — this station has no identifying/context
     ...
     * no longer present — mirrors StorageTankRecordService::upsertDetails()
     * exactly.
     */
    protected function upsertDetails(EngineRoomRecord $record, array $details): void
    {
        $validRows = collect($details)->filter(
            fn ($row) => $row['time_slot'] !== null && $row['time_slot'] !== '' && $this->isRowFilled($row)
        );
        ...
        foreach ($validRows as $row) {
            $detailAttributes = ['engine_room_record_id' => $record->id, 'time_slot' => $row['time_slot']];
            ...
            if (! empty($row['id']) && EngineRoomDetail::where('id', $row['id'])->where('engine_room_record_id', $record->id)->exists()) {
                ...
                $detail = EngineRoomDetail::create($detailAttributes);
                $keptIds[] = $detail->id;
            }
        }

        EngineRoomDetail::where('engine_room_record_id', $record->id)
     ...
     * StorageTankRecordService::applyVerification() exactly.
     */
    ...
    public function listRecords(array $filters, int $page, int $perPage): array
    {
        ...
        $query = $this->buildFilteredQuery($filters)
            ->withCount(['engineRoomDetails as filled_slot_count' => function (Builder $detailQuery) use ($readingFields) {
        ...
        $formatted['data'] = collect($formatted['data'])
            ->map(fn (EngineRoomRecord $record) => $this->toListRow($record))
            ->all();
    ...
    protected function fileMetaFor(string $format): array
    {
        ...
        if ($format === 'excel') {
            return [
                ...
                "engine-room-records_{$timestamp}.xlsx",
            ];
        }

        return [
            'text/csv',
            "engine-room-records_{$timestamp}.csv",
        ];
    }
    ...
    /**
     * Maps an EngineRoomRecord (with filled_slot_count pre-loaded via
     * withCount()) to the list endpoint's success_schema row shape.
     */
    protected function toListRow(EngineRoomRecord $record): array
    {
        return [
            'id' => $record->id,
            'engine_room_id' => $record->engine_room_id,
     ...
     * and the engineRoomDetails grid, sorted into canonical time-slot
     * order (not alphabetical — '00:00' would otherwise sort before
     * '07:00').
     */
    public function getDetail(string $id): array
    {
        $record = EngineRoomRecord::with([
            'station',
            'createdBy',
            'checkedBy',
            'acknowledgedBy',
    ...
    /**
     * Maps an EngineRoomRecord to the detail endpoint's success_schema —
     ...
     */
    protected function toDetailRow(EngineRoomRecord $record): array
    {
        $canonicalOrder = array_flip(self::canonicalTimeSlots());

        $details = $record->engineRoomDetails
            ->sortBy(fn (EngineRoomDetail $row) => $canonicalOrder[$row->time_slot] ?? 999)
            ->values()
            ->map(function (EngineRoomDetail $row) {
