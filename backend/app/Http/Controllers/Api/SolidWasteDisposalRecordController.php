...
/**
 * SolidWasteDisposalRecordController — screen-091--data-browser-solid-waste-disposal-web,
 * screen-101--detail-solid-waste-disposal-web, screen-111--form-solid-waste-disposal-web.
 * Mirrors CagesTrackRecordController's structure exactly, delegating all
 * business logic to SolidWasteDisposalRecordService (shared with the
 * Livewire components) so both web and API entry points apply the same
 * rules.
 */
class SolidWasteDisposalRecordController extends Controller
{
    public function __construct(protected SolidWasteDisposalRecordService $service) {}
    ...
    {
        $data = $request->only([
            'production_line_id', 'solid_waste_disposal_id', 'date', 'note',
    ...
    {
        $data = $request->only([
            'solid_waste_disposal_id', 'date', 'note', 'checked', 'acknowledged', 'details',
        ]);
