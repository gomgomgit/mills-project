...
/**
 * DataBrowserProcessQualityControl — screen-100--data-browser-process-quality-control-web /
 * "Data Browser Process Quality Control" (Livewire web page, route name
 * `data.process-quality-control`, /data/process-quality-control).
 *
 * Reuses ProcessQualityControlRecordService — the exact same service the API
 * controller (App\Http\Controllers\Api\ProcessQualityControlRecordController) uses
 * — so filtering/pagination rules stay identical between the web and API
 * entry points (mirrors DataBrowserClarification's pattern).
 *
 * Filter state mirrors the tech-spec exactly: date_from, date_to,
 * business_unit_id.
 *
 * Pagination: calls ProcessQualityControlRecordService::listRecords() directly and
 * keeps its own $page/$perPage state, rather than Livewire's
 * WithPagination trait — same approach as DataBrowserClarification.
 *
 * Export: dispatched as a plain browser-navigable link (GET
 * /api/process-quality-control-records/export?...), NOT an AJAX/blob download —
 * session cookie carries auth on a same-origin navigation.
 */
#[Layout('data.process-quality-control')]
class DataBrowserProcessQualityControl extends Component
{
    ...
    protected function exportUrl(string $format): string
    {
        ...
        return url('/api/process-quality-control-records/export').'?'.http_build_query($query);
    }
    ...
    public function render()
    {
        ...
        return view('livewire.data.data-browser-process-quality-control', [
            'records' => $result['data'],
            'meta' => $result['meta'],
