/**
 * DataPreviewDepricarpingView.spec.ts —
 * screen-047--data-preview-depricarping /
 * usecase-047--data-preview-depricarping.
 *
 * Mirrors DataPreviewPressingView.spec.ts's dual-mode (list/detail) mocking
 * strategy — `useRoute()` is mocked with a reactive-enough shape so
 * `route.params.id` can be swapped between tests (list vs detail mode).
 */
import { beforeEach, describe, expect, it, vi } from 'vitest'
import { mount, flushPromises } from '@vue/test-utils'
import DataPreviewDepricarpingView from '@/views/DataPreviewDepricarpingView.vue'
import {
  canonicalTimeSlots,
...
function makeAllDetailRows(): DepricarpingDetailRow[] {
  return canonicalTimeSlots().map((timeSlot, index) => ({
    id: `detail-${index}`,
    depricarping_record_id: 'rec-1',
    time_slot: timeSlot,
...
describe('DataPreviewDepricarpingView — list mode', () => {
  it('defaults the date filter to today and shows only today-dated records initially', async () => {
    ...
    const wrapper = mount(DataPreviewDepricarpingView)
    await flushPromises()
  ...
  it('navigates to Form Depricarping when a draft/pause item is tapped', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'depricarping-form', params: { id: 'rec-draft' } })
  })
  ...
  it('navigates to detail mode when a saved/synced item is tapped', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'data-preview-depricarping', params: { id: 'rec-saved' } })
  })

  it('navigates to Monitor Depricarping when Back is pressed in list mode', async () => {
    const wrapper = mount(DataPreviewDepricarpingView)
    await flushPromises()
...
describe('DataPreviewDepricarpingView — detail mode', () => {
  it('renders the record read-only with all of its detail rows (a full 24, when all canonical slots were added) and the 4-column operational target table', async () => {
    ...
    expect(wrapper.find('[data-testid="depricarping-detail-rows-list"]').exists()).toBe(true)
    expect(wrapper.findAll('[data-testid^="depricarping-detail-row-"]')).toHaveLength(24)
  ...
  it('shows the empty-state message when a record has zero detail rows', async () => {
    ...
    expect(wrapper.find('[data-testid="depricarping-detail-rows-empty"]').exists()).toBe(true)
  ...
  it('navigates back to list mode (not Monitor Depricarping) when Back is pressed in detail mode', async () => {
    routeState.params = { id: 'rec-1' }
    getDraftWithDetailsMock.mockResolvedValue({ record: makeRecord(), details: makeAllDetailRows() })
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'data-preview-depricarping' })
