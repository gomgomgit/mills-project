...
/**
 * DataBrowserStorageTankTest (Browser/Playwright) —
 * screen-096--data-browser-storage-tank-web /
 * usecase-094--data-browser-storage-tank-web.
 *
 * Playwright spec, mirrors tests/Browser/DataBrowserEffluentPlantTest.php's
 * conventions exactly.
 *
 ...
 * (sttest-admin01 / Passw0rd!), a business unit with at least one Storage
 * Tank record dated within 2026-08-01..2026-08-15.
 */
...
test.describe('Data Browser Storage Tank (Web)', () => {
  ...
  test('apply date range and business unit filters, click export and choose format', async ({ page }) => {
    ...
    expect(download.suggestedFilename()).toContain('storage-tank-records');
  });
  ...
  test('apply filters with no matching data, empty state shown', async ({ page }) => {
    ...
    await expect(page.locator('.st-empty__title')).toContainText('Tidak ada data');
  });
  ...
  test('start date later than end date, validation message displayed', async ({ page }) => {
    ...
    await expect(page.locator('.st-alert')).toContainText('Rentang tanggal tidak valid');
  });

  // Scenario: "Klik Baris Membuka Detail"
  test('click a table row, browser navigates to Detail Storage Tank', async ({ page }) => {
    await login(page, 'sttest-admin01', PASSWORD);
    await page.goto(`${BASE_URL}/data/storage-tank`);
