/**
 * DataPreviewProcessQualityControlView.spec.ts —
 * screen-090--data-preview-process-quality-control / usecase-117--data-preview-process-quality-control.
 *
 * Mirrors DataPreviewClarificationView.spec.ts's dual-mode (list/detail)
 ...
 * This is the station with the MOST non-time_slot columns in the whole
 * project — NO operational-target reference table, so no such assertion
 * appears here.
 */
import { beforeEach, describe, expect, it, vi } from 'vitest'
import { mount, flushPromises } from '@vue/test-utils'
import DataPreviewProcessQualityControlView from '@/views/DataPreviewProcessQualityControlView.vue'
import {
  canonicalTimeSlots,
...
function makeAllDetailRows(): ProcessQualityControlDetailRow[] {
  return canonicalTimeSlots().map((timeSlot, index) => ({
    id: `detail-${index}`,
    process_quality_control_record_id: 'rec-1',
    time_slot: timeSlot,
    shift: null,
...
describe('DataPreviewProcessQualityControlView — list mode', () => {
  it('defaults the date filter to today and shows only today-dated records initially', async () => {
    ...
    const wrapper = mount(DataPreviewProcessQualityControlView)
    await flushPromises()
  ...
  it('navigates to Form Process Quality Control when a draft/pause item is tapped', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'process-quality-control-form', params: { id: 'rec-draft' } })
  })
  ...
  it('navigates to Monitor Process Quality Control when Back is pressed in list mode', async () => {
    const wrapper = mount(DataPreviewProcessQualityControlView)
    await flushPromises()
...
describe('DataPreviewProcessQualityControlView — detail mode', () => {
  it('renders the record read-only with all of its detail rows (a full 24, when all canonical slots were added)', async () => {
    ...
    expect(wrapper.find('[data-testid="process-quality-control-detail-rows-list"]').exists()).toBe(true)
    expect(wrapper.findAll('[data-testid^="process-quality-control-detail-row-"]')).toHaveLength(24)
  })
  ...
  it('shows the empty-state message when a record has zero detail rows', async () => {
    ...
    expect(wrapper.find('[data-testid="process-quality-control-detail-rows-empty"]').exists()).toBe(true)
  ...
  it('navigates back to list mode (not Monitor Process Quality Control) when Back is pressed in detail mode', async () => {
    routeState.params = { id: 'rec-1' }
    getDraftWithDetailsMock.mockResolvedValue({ record: makeRecord(), details: makeAllDetailRows() })
