/**
 * effluentPlantRecordRepo.spec.ts — screen-065--monitor-effluent-plant /
 * screen-075--form-effluent-plant / screen-085--data-preview-effluent-plant.
 *
 * Unit tests for src/services/effluentPlantRecordRepo.ts — covers the
 * authoritative unit_test_cases for these 3 screens' repo layer
 * (mobile-only / local-SQLite-only, no REST api_contract). Mirrors
 * processWaterRecordRepo.spec.ts's structure/pattern exactly, since
 * Effluent Plant follows the same hourly-grid (dynamic add-row/remove-row,
 * 1..24 rows) pattern as Process Water — except this station has NO
 * identifying/context columns (no shift/inspector_id equivalent); all 19
 * non-time_slot columns are reading/status columns, including 3 enum
 * status columns (biogas_flare_status, dosing_pump_1_status,
...
function makeHeaderData(overrides: Partial<EffluentPlantHeaderFormData> = {}): EffluentPlantHeaderFormData {
  return {
    effluent_plant_id: 'EP-01',
...
function makeDetailFormRow(overrides: Partial<EffluentPlantDetailFormRow> = {}): EffluentPlantDetailFormRow {
  return {
    time_slot: '07:00',
    anaerobic_pond_1_ph: null,
    anaerobic_pond_1_temp_c: null,
...
function makeValidDetailRows(overrides: Partial<EffluentPlantDetailFormRow>[] = []): EffluentPlantDetailFormRow[] {
  if (overrides.length === 0) {
    return [makeDetailFormRow({ time_slot: '07:00', anaerobic_pond_1_ph: 7.1 })]
  }

  return overrides.map((o) => makeDetailFormRow(o))
}

// Positional order of the 19 non-time_slot detail columns as written by
// applyDetailRowChanges() — used to build expected INSERT/UPDATE param
// arrays without repeating all 19 `null`s by hand in every test.
function readingParams(overrides: Partial<EffluentPlantDetailFormRow> = {}): (string | number | null)[] {
...
describe('effluentPlantRecordRepo', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  describe('createDraft()', () => {
    it('inserts a new draft_ongoing effluent_plant_record with date auto-filled, and inserts NO effluent_plant_detail rows', async () => {
      ...
      expect(headerSql).toContain('INSERT INTO effluent_plant_record')
  ...
  describe('getDrafts()', () => {
    it('returns the draft/pause list, most-recently-updated first', async () => {
      const rows = [
        { id: 'draft-newest', status: 'draft_paused' as const, effluent_plant_id: 'EP-002', updated_at: '2026-08-31T10:00:00.000Z' },
        { id: 'draft-oldest', status: 'draft_ongoing' as const, effluent_plant_id: 'EP-001', updated_at: '2026-08-31T08:00:00.000Z' },
      ]
  ...
  describe('getTodaySummary()', () => {
    it("computes today's record count and a plain COUNT of effluent_plant_detail rows belonging to those records (not filtered by filled-ness)", async () => {
      vi.mocked(query)
      ...
      expect(result).toEqual({ countEffluentPlantRecord: 2, detailRowCount: 5 })
      expect(query).toHaveBeenNthCalledWith(1, expect.stringContaining('FROM effluent_plant_record'), [USER_ID])
      ...
      expect(countSql).toContain('FROM effluent_plant_detail')
      expect(countSql).toContain('WHERE effluent_plant_record_id IN (?, ?)')
    ...
    it('returns zero counters and skips the detail query when no records match today', async () => {
      ...
      expect(result).toEqual({ countEffluentPlantRecord: 0, detailRowCount: 0 })
  ...
  describe('getAllRecords()', () => {
    it('returns every record for the user regardless of status', async () => {
      ...
      expect(result).toEqual(rows)
      expect(query).toHaveBeenCalledWith(expect.stringContaining('FROM effluent_plant_record WHERE created_by = ?'), [
        USER_ID,
      ])
      expect(query).toHaveBeenCalledWith(expect.stringContaining('ORDER BY updated_at DESC'), [USER_ID])
    })
  ...
  describe('saveDraft()', () => {
    it('throws EffluentPlantDetailRequiredError and calls neither run() nor query() when details is empty', async () => {
    ...
    it('throws EffluentPlantDetailRequiredError when every row lacks a selected time_slot or has zero reading columns filled', async () => {
      const headerData = makeHeaderData()
      const details: EffluentPlantDetailFormRow[] = [
    ...
    it('treats a row with only an enum status column filled as satisfying the minimum-one-row rule', async () => {
      ...
      const headerData = makeHeaderData()
      const details = [makeDetailFormRow({ time_slot: '07:00', biogas_flare_status: 'fault' })]
    ...
    it('updates the header with status=saved, INSERTs a new (id-less) detail row, and returns the freshly saved draft', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      const saved = makeRecord({ id: 'draft-4', effluent_plant_id: 'EP-01', status: 'saved' })
      const savedDetails = [makeDetailRow({ id: 'detail-new-1', effluent_plant_record_id: 'draft-4' })]
      vi.mocked(query).mockResolvedValueOnce([saved]).mockResolvedValueOnce(savedDetails)

      const headerData = makeHeaderData()
      const details = makeValidDetailRows([{ time_slot: '09:00', anaerobic_pond_1_ph: 7.2 }])
      ...
      const [headerSql, headerParams] = vi.mocked(run).mock.calls[0]
      expect(headerSql).toContain('UPDATE effluent_plant_record')
      expect(headerSql).toContain("status = 'saved'")
      expect(headerParams).toEqual(
        expect.arrayContaining(['EP-01', expect.any(String), null, null, expect.any(String), 'draft-4']),
      )

      const [detailSql, detailParams] = vi.mocked(run).mock.calls[1]
      expect(detailSql).toContain('INSERT INTO effluent_plant_detail')
      expect(detailParams).toEqual([
        expect.any(String), // freshly generated detail id
        'draft-4',
        '09:00',
        ...readingParams({ time_slot: '09:00', anaerobic_pond_1_ph: 7.2 }),
        expect.any(String),
        expect.any(String),
      ])
    ...
    it('UPDATEs an existing detail row (has an id) instead of inserting it', async () => {
      ...
      const headerData = makeHeaderData()
      const details: EffluentPlantDetailFormRow[] = [
        { ...makeDetailFormRow(), id: 'detail-existing-1', time_slot: '11:00', anaerobic_pond_1_ph: 7.5 },
      ]
      ...
      expect(run).toHaveBeenCalledTimes(2)
      const [detailSql, detailParams] = vi.mocked(run).mock.calls[1]
      expect(detailSql).toContain('UPDATE effluent_plant_detail')
      expect(detailParams).toEqual([
        '11:00',
        ...readingParams({ time_slot: '11:00', anaerobic_pond_1_ph: 7.5 }),
        expect.any(String),
        'detail-existing-1',
      ])
    })
    ...
    it('upserts multiple detail rows sequentially, in details order (mixed insert + update)', async () => {
      ...
      const headerData = makeHeaderData()
      const details: EffluentPlantDetailFormRow[] = [
        { ...makeDetailFormRow(), id: 'detail-existing-2', time_slot: '07:00', anaerobic_pond_1_ph: 1 },
        makeDetailFormRow({ time_slot: '13:00', anaerobic_pond_1_ph: 2 }),
      ]
      ...
      expect(firstRowSql).toContain('UPDATE effluent_plant_detail')
      expect(secondRowSql).toContain('INSERT INTO effluent_plant_detail')
    })
    ...
    it('DELETEs every id in detailIdsToDelete, after the upserts', async () => {
      ...
      expect(deleteSql1).toContain('DELETE FROM effluent_plant_detail WHERE id = ?')
      expect(deleteParams1).toEqual(['detail-removed-1'])
      expect(deleteSql2).toContain('DELETE FROM effluent_plant_detail WHERE id = ?')
      expect(deleteParams2).toEqual(['detail-removed-2'])
    })

    it('coerces empty-string enum values to null when upserting a detail row', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      vi.mocked(query)
      ...
      // positions: id, recordId, time_slot, then the 19 reading params —
      // biogas_flare_status is index 9, dosing_pump_1_status index 17,
      // sludge_dewatering_status index 19 (0-based) of the full array.
      expect(detailParams[9]).toBeNull()
      expect(detailParams[17]).toBeNull()
      expect(detailParams[19]).toBeNull()
    })
    ...
    it('nulls/strips checked_by for a non-supervisor role', async () => {
      ...
      // checked_by is the 4th positional SET param (index 3): effluent_plant_id,
    ...
    it('throws when getDraftWithDetails() re-read after save unexpectedly finds nothing', async () => {
      ...
      await expect(saveDraft('draft-8', headerData, details, [], 'operator')).rejects.toThrow(
        'Gagal memuat ulang data effluent plant setelah disimpan.',
      )
    })
  })
  ...
  describe('pauseDraftWithFormData()', () => {
    it('saves with status=draft_paused even when details is empty (no validation gate)', async () => {
      ...
      const headerData = makeHeaderData({ effluent_plant_id: '' })
    ...
    it('upserts detail rows and applies pending deletions, same contract as saveDraft()', async () => {
      ...
      expect(deleteSql).toContain('DELETE FROM effluent_plant_detail WHERE id = ?')
    ...
    it('throws when getDraftWithDetails() re-read after pause unexpectedly finds nothing', async () => {
      ...
      await expect(pauseDraftWithFormData('draft-1', headerData, [], [], 'operator')).rejects.toThrow(
        'Gagal menyimpan progres effluent plant setelah pause.',
      )
    })
  })

  describe('deleteDraft()', () => {
    it('deletes effluent_plant_detail rows before deleting the effluent_plant_record row', async () => {
      vi.mocked(run).mockResolvedValue({ changes: 1 })
      ...
      expect(run).toHaveBeenNthCalledWith(
        1,
        expect.stringContaining('DELETE FROM effluent_plant_detail WHERE effluent_plant_record_id = ?'),
        ['draft-1'],
      )
      expect(run).toHaveBeenNthCalledWith(2, expect.stringContaining('DELETE FROM effluent_plant_record WHERE id = ?'), [
        'draft-1',
      ])
    })
  })
