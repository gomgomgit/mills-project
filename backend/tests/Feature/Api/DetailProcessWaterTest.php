...
/**
 * DetailProcessWaterTest (Feature/Api) — screen-102--detail-process-water-web /
 * usecase-071--detail-process-water-web.
 *
 * Integration tests for GET /api/process-water-records/{id}, mirroring
...
it('berhasil: returns the full record with resolved names and its details grid', function () {
    ...
    ProcessWaterDetail::factory()->forRecord($record)->timeSlot('09:00')->filled()->create();

    $response = $this->actingAs($this->supervisor, 'web')->getJson("/api/process-water-records/{$record->id}");
...
it('returns details ordered by canonical time-slot, not creation order', function () {
    $record = ProcessWaterRecord::factory()->forStation($this->station)->create();
    ProcessWaterDetail::factory()->forRecord($record)->timeSlot('14:00')->create();
    ProcessWaterDetail::factory()->forRecord($record)->timeSlot('08:00')->create();
    ProcessWaterDetail::factory()->forRecord($record)->timeSlot('00:00')->create();

    $response = $this->actingAs($this->admin, 'web')->getJson("/api/process-water-records/{$record->id}");
...
it('Mill Management and Admin can also access the detail endpoint', function () {
    ...
    $this->actingAs($this->millManagement, 'web')->getJson("/api/process-water-records/{$record->id}")->assertOk();
    $this->actingAs($this->admin, 'web')->getJson("/api/process-water-records/{$record->id}")->assertOk();
});

it('returns 403 for the Operator role (route-level role gate)', function () {
    $record = ProcessWaterRecord::factory()->forStation($this->station)->create();

    $this->actingAs($this->operator, 'web')->getJson("/api/process-water-records/{$record->id}")->assertStatus(403);
});

it('returns 404 when the id does not exist', function () {
    $this->actingAs($this->admin, 'web')->getJson('/api/process-water-records/00000000-0000-0000-0000-000000000000')->assertStatus(404);
});
...
it('rejects unauthenticated requests', function () {
    ...
    $this->getJson("/api/process-water-records/{$record->id}")->assertStatus(401);
});
