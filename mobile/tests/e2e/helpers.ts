...
export async function login(page: Page, user: { username: string; password: string } = USERS.operator): Promise<void> {
  ...
  // Native <option> elements never report as "visible" to Playwright's
  // actionability checks (no layout box) — wait for "attached" (present in
  // the DOM once GET /api/business-units resolves) instead.
  await page.locator('#business_unit_id option').nth(1).waitFor({ state: 'attached', timeout: 15_000 })
