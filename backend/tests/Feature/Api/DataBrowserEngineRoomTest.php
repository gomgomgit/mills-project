...
/**
 * DataBrowserEngineRoomTest (Feature/Api) — screen-097--data-browser-engine-room-web
 * / usecase-100--data-browser-engine-room-web.
 *
 * Integration tests for GET /api/engine-room-records and GET
 * /api/engine-room-records/export, mirroring
 * tests/Feature/Api/DataBrowserStorageTankTest.php's structure exactly.
 */
...
it('success: lists filtered/paginated records then exports them as csv', function () {
    $recordWithFilledRow = EngineRoomRecord::factory()->forStation($this->station)->onDate('2026-08-05')->create();
    EngineRoomDetail::factory()->forRecord($recordWithFilledRow)->timeSlot('07:00')->filled()->create();

    EngineRoomRecord::factory()->forStation($this->station)->onDate('2026-08-06')->count(2)->create();
    EngineRoomRecord::factory()->forStation($this->station)->onDate('2026-09-01')->create();

    $listResponse = $this->actingAs($this->admin, 'web')->getJson('/api/engine-room-records?'.http_build_query([
    ...
    $exportResponse = $this->actingAs($this->admin, 'web')->get('/api/engine-room-records/export?'.http_build_query([
        'date_from' => '2026-08-01',
        'date_to' => '2026-08-15',
        'business_unit_id' => $this->businessUnit->id,
        'format' => 'csv',
...
it('Tidak Ada Data Sesuai Filter: returns 200 with an empty data list and meta.total = 0', function () {
    ...
    $response = $this->actingAs($this->supervisor, 'web')->getJson('/api/engine-room-records?'.http_build_query([
...
it('Rentang Tanggal Tidak Valid: returns 422 INVALID_DATE_RANGE when date_from is after date_to', function () {
    $response = $this->actingAs($this->millManagement, 'web')->getJson('/api/engine-room-records?'.http_build_query([
...
it('Klik Baris Membuka Detail: baseline list call succeeds (row-click detail nav is a FE concern)', function () {
    ...
    $response = $this->actingAs($this->admin, 'web')->getJson('/api/engine-room-records?'.http_build_query(['page' => 1, 'per_page' => 20]));
...
it('Ekspor Gagal: returns 422 EXPORT_FAILED when the filtered dataset exceeds the export row limit', function () {
    $limit = app(EngineRoomRecordService::class)::EXPORT_ROW_LIMIT;
    $total = $limit + 1;
    ...
    while ($inserted < $total) {
        ...
        \Illuminate\Support\Facades\DB::table('engine_room_records')->insert($rows);
        $inserted += $batch;
    }

    $response = $this->actingAs($this->admin, 'web')->getJson('/api/engine-room-records/export?format=excel');
...
it('returns 401 for both endpoints when there is no authenticated session', function () {
    $this->getJson('/api/engine-room-records')->assertStatus(401);
    $this->getJson('/api/engine-room-records/export?format=csv')->assertStatus(401);
});

it('returns 403 for both endpoints when the authenticated user is an operator', function () {
    $this->actingAs($this->operator, 'web')->getJson('/api/engine-room-records')->assertStatus(403);
    $this->actingAs($this->operator, 'web')->getJson('/api/engine-room-records/export?format=csv')->assertStatus(403);
});
