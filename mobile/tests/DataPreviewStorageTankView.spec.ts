/**
 * DataPreviewStorageTankView.spec.ts — screen-086--data-preview-storage-tank
 * / usecase-093--data-preview-storage-tank.
 *
 * Mirrors DataPreviewEffluentPlantView.spec.ts's dual-mode (list/detail)
...
import DataPreviewStorageTankView from '@/views/DataPreviewStorageTankView.vue'
import { canonicalTimeSlots, type StorageTankDetailRow, type StorageTankRecord } from '@/services/storageTankRecordRepo'
...
function makeAllDetailRows(): StorageTankDetailRow[] {
  return canonicalTimeSlots().map((timeSlot, index) => ({
    id: `detail-${index}`,
    storage_tank_record_id: 'rec-1',
    time_slot: timeSlot,
...
describe('DataPreviewStorageTankView — list mode', () => {
  it('defaults the date filter to today and shows only today-dated records initially', async () => {
    ...
    const wrapper = mount(DataPreviewStorageTankView)
    await flushPromises()
  ...
  it('navigates to Form Storage Tank when a draft/pause item is tapped', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'storage-tank-form', params: { id: 'rec-draft' } })
  })
  ...
  it('navigates to detail mode when a saved/synced item is tapped', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'data-preview-storage-tank', params: { id: 'rec-saved' } })
  })

  it('navigates to Monitor Storage Tank when Back is pressed in list mode', async () => {
    const wrapper = mount(DataPreviewStorageTankView)
    await flushPromises()
...
describe('DataPreviewStorageTankView — detail mode', () => {
  it('renders the record read-only with all of its detail rows (a full 24, when all canonical slots were added)', async () => {
    ...
    expect(wrapper.find('[data-testid="storage-tank-detail-rows-list"]').exists()).toBe(true)
    expect(wrapper.findAll('[data-testid^="storage-tank-detail-row-"]')).toHaveLength(24)
  })
  ...
  it('shows the empty-state message when a record has zero detail rows', async () => {
    ...
    expect(wrapper.find('[data-testid="storage-tank-detail-rows-empty"]').exists()).toBe(true)
  ...
  it('navigates back to list mode (not Monitor Storage Tank) when Back is pressed in detail mode', async () => {
    routeState.params = { id: 'rec-1' }
    getDraftWithDetailsMock.mockResolvedValue({ record: makeRecord(), details: makeAllDetailRows() })
