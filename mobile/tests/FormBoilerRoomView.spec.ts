/**
 * FormBoilerRoomView.spec.ts — screen-078--form-boiler-room /
 * usecase-104--form-boiler-room.
 *
 * Mirrors FormEngineRoomView.spec.ts's structure/pattern exactly — Boiler
 * Room follows the same hourly-grid, dynamic add-row/remove-row pattern as
 * Engine Room, but with 15 non-time_slot columns and 2 enum status columns
 * (blowdown_executed, sootblowing_executed). UNLIKE FormThreshingView.spec.ts:
 * this station has NO operational-target reference table, so those tests
 * are omitted.
 *
 ...
 *   - required-field validation (Boiler Room ID)
 *   - "at least one valid row" validation (Time-Slot + reading filled)
 *   - Simpan success (status=saved), navigates to Monitor Boiler Room
...
import FormBoilerRoomView from '@/views/FormBoilerRoomView.vue'
import {
...
// Mirrors FormEngineRoomView.spec.ts's helpers exactly.
...
describe('FormBoilerRoomView', () => {
  it('loads existing detail rows (however many) ordered by canonical time-slot on mount', async () => {
    ...
    const wrapper = mount(FormBoilerRoomView)
    await flushPromises()

    const rows = wrapper.findAll('[data-testid="boiler-room-detail-row"]')
  ...
  it('adds one row at a time via "Tambah baris"', async () => {
    ...
    expect(wrapper.findAll('[data-testid="boiler-room-detail-row"]')).toHaveLength(1)
  ...
  it('shows inline error and does not save when Boiler Room ID is empty on Simpan', async () => {
    const wrapper = mount(FormBoilerRoomView)
    await flushPromises()
    ...
    expect(saveDraftMock).not.toHaveBeenCalled()
    expect(wrapper.text()).toContain('Boiler Room ID wajib diisi')
  })
  ...
  it('shows a grid-specific error and does not save when there are zero valid detail rows', async () => {
    ...
    await wrapper.find('#field-boiler-room-id').setValue('BR-01')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()
  ...
  it('saves successfully and navigates to Monitor Boiler Room when Boiler Room ID and a valid detail row exist', async () => {
    ...
    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    await chooseSearchableOption(wrapper, 'time-slot-select-0', '07:00')
    await wrapper.find('#steam-pressure-0').setValue('12.5')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()
    ...
    expect(headerData.boiler_room_id).toBe('BR-01')
  ...
  it('falls back to the grid error (defense-in-depth) when the repo throws BoilerRoomDetailRequiredError', async () => {
    saveDraftMock.mockRejectedValue(new BoilerRoomDetailRequiredError())
    ...
    await wrapper.find('[data-testid="add-detail-row-button"]').trigger('click')
    await chooseSearchableOption(wrapper, 'time-slot-select-0', '07:00')
    await wrapper.find('#steam-pressure-0').setValue('1')
    await wrapper.find('[data-testid="save-button"]').trigger('click')
    await flushPromises()
  ...
  it('shows a confirm dialog on Back when the form is dirty', async () => {
    ...
    await wrapper.find('#field-boiler-room-id').setValue('BR-99')
