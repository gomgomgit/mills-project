/**
 * MonitorStorageTankView.spec.ts — screen-066--monitor-storage-tank /
 * usecase-091--monitor-storage-tank.
 *
 ...
 *   4. 'New Data' creates a new draft and navigates to Form Storage Tank
 *   5. tapping a list item navigates with the EXISTING draft id
 *   6. 'Load Data' navigates to Data Preview Storage Tank (list mode, no id)
...
import MonitorStorageTankView from '@/views/MonitorStorageTankView.vue'
import type { StorageTankDraftListItem } from '@/services/storageTankRecordRepo'
...
function makeDraft(overrides: Partial<StorageTankDraftListItem> & { id: string }): StorageTankDraftListItem {
  return {
    status: 'draft_ongoing',
    storage_tank_id: 'ST-001',
...
beforeEach(() => {
  ...
  getTodaySummaryMock.mockResolvedValue({ countStorageTankRecord: 0, detailRowCount: 0 })
})

describe('MonitorStorageTankView', () => {
  it('loads and renders a populated draft/pause list', async () => {
    getDraftsMock.mockResolvedValue([
      makeDraft({ id: 'draft-1', storage_tank_id: 'ST-001' }),
      makeDraft({ id: 'draft-2', storage_tank_id: 'ST-002' }),
    ])

    const wrapper = mount(MonitorStorageTankView)
    await flushPromises()
  ...
  it('creates a new draft and navigates to Form Storage Tank when New Data is pressed', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'storage-tank-form', params: { id: 'new-draft-id' } })
  })

  it('navigates to Form Storage Tank with the existing draft id when a list item is tapped, without any status-update call', async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith({ name: 'storage-tank-form', params: { id: 'draft-existing' } })
    expect(createDraftMock).not.toHaveBeenCalled()
  })

  it('navigates to Data Preview Storage Tank in list mode with no id when Load Data is pressed', async () => {
    const wrapper = mount(MonitorStorageTankView)
    await flushPromises()

    await wrapper.find('[data-testid="load-data-button"]').trigger('click')

    expect(pushMock).toHaveBeenCalledWith({ name: 'data-preview-storage-tank' })
  })
  ...
  it("renders the Hari Ini counter cards from getTodaySummary()'s resolved values", async () => {
    getTodaySummaryMock.mockResolvedValue({ countStorageTankRecord: 3, detailRowCount: 7 })

    const wrapper = mount(MonitorStorageTankView)
    await flushPromises()

    expect(wrapper.find('[data-testid="counter-count-storage-tank-record"]').text()).toBe('3')
  ...
  it('renders zero counters when there is no data today', async () => {
    ...
    expect(wrapper.find('[data-testid="counter-count-storage-tank-record"]').text()).toBe('0')
